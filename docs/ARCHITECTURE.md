...
| **Masters** (32 keys)             | working               | Cross-ref of `masterTables.ts` ↔ `masterStorage.mjs` is **32 perfect matches**. New 2026-05-10: `designation_master` (20 seeded — MD/CEO/CFO down to Analyst, with `level` 1–5 and JSON `approvalLimit`). `MasterDataContext` now hydrates `designations` alongside `vendorGroups` and `locations`. Realistic Indian mid-market org seeds: department_master 16 rows (Finance/HR/IT/Ops/Sales/Procurement/Legal/Admin/Engineering/CS), location_master 12 rows (incl. Mumbai/Delhi/Bangalore/Chennai/Hyderabad/Pune offices + Mumbai/Delhi warehouses), gl_code_master 51 rows (full Indian COA: cash 1xxx, AP/TDS/GST liabilities 2xxx, revenue 4xxx, expenses 5xxx). user_master grew from 5 → 15 — all share the same bcrypt hash for `Demo@123`, all on `tenant-default-001` / `entity-ptpl-001`. Earlier critical fixes (item_master 500, vendor_group_master end-to-end, KitBundlePRForm hardcodes, GL/location seeds) all still in place. The `init.sql` now contains CREATE statements for `location_master`, `gl_code_master`, `vendor_group_master`, and `designation_master` so fresh-DB provisioning matches production |
...
| Masters               |    31 | `item_master`, `master_records` + 26 `erp_master_*` (LIKE-cloned) + 6 per-master DBs (payment_method, tds_section, location, gl_code, vendor_group, designation)                 |
