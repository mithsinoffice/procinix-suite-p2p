...
| **Masters** (31 keys)             | working               | Cross-ref of `masterTables.ts` ↔ `masterStorage.mjs` is **31 perfect matches** post-fix. **All 4 critical master bugs fixed 2026-05-10**: (1) `item_master` GET no longer 500s — handler adapts the non-`erp_master_*` schema to the canonical `record_code`/`record_name`/`payload` shape inline; (2) `vendor_group_master` end-to-end wired (DB table + 5 seeds + API + `MasterDataContext.vendorGroups` from API + `VendorMasterCreate.tsx` dropdown that sets `vendor_group_id`/`vendor_group_name` on save); (3) `location_master` seeded with 5 rows, exposed via `MasterDataContext.locations`, consumed by `KitBundlePRForm`; (4) `gl_code_master` seeded with 12 rows. Per-master DBs (`payment_method_master`, `tds_section_master`, `location_master`, `gl_code_master`, `vendor_group_master`) live in migrations rather than `init.sql` |
...
| Masters               |    30 | `item_master`, `master_records` + 26 `erp_master_*` (LIKE-cloned) + 5 per-master DBs (payment_method, tds_section, location, gl_code, vendor_group)                              |
