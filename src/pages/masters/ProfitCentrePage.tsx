...
import {
  ...
  FormPageHeader, FormFooter, WorkflowBanner, MasterPageHeader, ApiSelect,
...
interface ProfitCentre { id: string; code: string; name: string; entityId: string; description?: string; status: string; createdAt: string }
...
function ProfitCentreForm({ record, onClose, onSaved }: { record?: ProfitCentre; onClose: () => void; onSaved: () => void }) {
  ...
  function validate() {
    ...
    if (!form.entityId) e.entityId = 'Entity is required'
    setErrors(e)
  ...
  return (
    <div className="max-w-4xl mx-auto px-6 py-6 space-y-4">
      <FormPageHeader
        title={record ? `Edit — ${record.name}` : 'Create Profit Centre'}
        subtitle="Entity-scoped profit centres for financial reporting"
        onBack={onClose}
      />
      <FormSection title="Identity">
        <FormField label="Entity" required error={errors.entityId}>
          <ApiSelect
            ...
            queryKey={['entities']}
            value={String(form.entityId ?? '')}
            onChange={v => set('entityId', v)}
            valueKey="id" labelKey="name"
            placeholder="Select entity…"
          />
        </FormField>
        <FormField label="Code" required hint="Unique per entity e.g. PC-0001" error={errors.code}>
          <FormInput value={String(form.code ?? '')} placeholder="PC-0001"
            onChange={e => set('code', e.target.value)} onBlur={validate} disabled={!!record} />
        </FormField>
        <FormField label="Name" required error={errors.name}>
          <FormInput value={String(form.name ?? '')} placeholder="Corporate HQ"
...
export default function ProfitCentrePage() {
  const qc = useQueryClient()
  const [formOpen, setFormOpen]   = useState(false)
  const [edit, setEdit]           = useState<ProfitCentre | null>(null)
  ...
  const { data: rows = [], isLoading, refetch } = useQuery({
    queryKey: ['profitCentre', activeTab, search, entityFilter],
    staleTime: 30_000,
    gcTime: 0,
    retry: false,
    refetchOnMount: true,
    queryFn: () => {
      ...
      return http.get<ProfitCentre[]>(`/api/masters/profit-centres?${p}`)
    },
  })

  const entityMap = Object.fromEntries((entities as Entity[]).map(e => [e.id, e.name]))

  if (formOpen) {
    return (
      <div className="flex flex-col h-full overflow-y-auto">
        <ProfitCentreForm record={edit ?? undefined} onClose={() => { setFormOpen(false); setEdit(null) }} onSaved={() => refetch()} />
      </div>
    )
  }
  ...
  return (
    <div className="flex flex-col h-full">
      <MasterPageHeader
        ...
        description="Entity-scoped profit centres for financial reporting and cost allocation"
        onRefresh={() => qc.invalidateQueries({ queryKey: ['profitCentre'] })}
        actions={
          <>
            <select value={entityFilter} onChange={e => setEntityFilter(e.target.value)}
              ...
              {(entities as Entity[]).map(e => <option key={e.id} value={e.id}>{e.name}</option>)}
            </select>
            <input type="search" placeholder="Search profit centres…" value={search}
      ...
      <MasterTabs active={activeTab} onChange={setActiveTab} apiPath="/api/masters/profit-centres" />
      ...
      <div className="flex-1 overflow-auto">
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-16">
            <p className="text-sm text-muted-foreground">No profit centres found</p>
          </div>
        ) : (
          <table className="w-full text-sm">
            <thead className="border-b border-border bg-muted/40">
              <tr>
                {['Code', 'Name', 'Entity', 'Description', 'Status', 'Actions'].map(h => (
                  <th key={h} className="px-4 py-2.5 text-left text-xs font-medium text-muted-foreground">{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {rows.map((r: ProfitCentre) => (
                <tr key={r.id} className="border-b border-border hover:bg-muted/20">
                  <td className="px-4 py-3 font-mono text-xs font-medium">{r.code}</td>
                  <td className="px-4 py-3 font-medium">{r.name}</td>
                  <td className="px-4 py-3 text-xs text-muted-foreground">{entityMap[r.entityId] ?? r.entityId}</td>
      ...
      <AuditTrailDrawer open={!!audit} onClose={() => setAudit(null)}
        entityType="profitCentre" entityId={audit?.id ?? ''} entityName={audit?.name ?? ''} />
    </div>
  )
}
