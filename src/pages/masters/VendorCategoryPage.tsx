...
interface VendorCategory { id: string; code: string; name: string; description?: string; status: string; createdAt: string }

function VendorCategoryForm({ record, onClose, onSaved }: { record?: VendorCategory; onClose: () => void; onSaved: () => void }) {
  ...
  const save = useMutation({
    mutationFn: () => record
      ? http.put<any>(`/api/masters/vendor-categories/${record.id}`, form)
      : http.post<any>('/api/masters/vendor-categories', form),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['vendorCategory'] }); onSaved(); onClose() },
  })
  ...
  return (
    <div className="max-w-4xl mx-auto px-6 py-6 space-y-4">
      <FormPageHeader
        title={record ? `Edit — ${record.name}` : 'Create Vendor Category'}
        subtitle="Classify vendors by category for reporting and routing"
        onBack={onClose}
      />
      <FormSection title="Identity">
        <FormField label="Code" required hint="Unique code e.g. VCT-0001" error={errors.code}>
          <FormInput value={String(form.code ?? '')} placeholder="VCT-0001"
            onChange={e => set('code', e.target.value)} onBlur={validate} disabled={!!record} />
        </FormField>
        <FormField label="Name" required error={errors.name}>
          <FormInput value={String(form.name ?? '')} placeholder="Information Technology"
...
export default function VendorCategoryPage() {
  ...
  const [edit, setEdit]           = useState<VendorCategory | null>(null)
  ...
  const { data: rows = [], isLoading, refetch } = useQuery({
    queryKey: ['vendorCategory', activeTab, search],
    staleTime: 30_000,
    gcTime: 0,
    retry: false,
    refetchOnMount: true,
    queryFn: () => {
      ...
      return http.get<VendorCategory[]>(`/api/masters/vendor-categories?${p}`)
    },
  })
  ...
  if (formOpen) {
    return (
      <div className="flex flex-col h-full overflow-y-auto">
        <VendorCategoryForm record={edit ?? undefined} onClose={() => { setFormOpen(false); setEdit(null) }} onSaved={() => refetch()} />
      </div>
    )
  }
  ...
  return (
    <div className="flex flex-col h-full">
      <MasterPageHeader
        ...
        description="Classify vendors by category for reporting and approval routing"
        onRefresh={() => qc.invalidateQueries({ queryKey: ['vendorCategory'] })}
        actions={
          <>
      ...
      <MasterTabs active={activeTab} onChange={setActiveTab} apiPath="/api/masters/vendor-categories" />
      ...
      <div className="flex-1 overflow-auto">
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-16">
            <p className="text-sm text-muted-foreground">No vendor categories found</p>
          </div>
        ) : (
      ...
      <AuditTrailDrawer open={!!audit} onClose={() => setAudit(null)}
        entityType="vendorCategory" entityId={audit?.id ?? ''} entityName={audit?.name ?? ''} />
    </div>
  )
}
