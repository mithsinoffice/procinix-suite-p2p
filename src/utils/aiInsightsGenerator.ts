...
export function generateAIInsights(
  ...
  if (invoiceData.invoiceNumber && invoiceData.vendorCode) {
    const exactDuplicate = historicalInvoices.find(
      inv => inv.invoiceNumber === invoiceData.invoiceNumber && 
             inv.vendorCode === invoiceData.vendorCode
    );
  ...
  if (invoiceData.invoiceAmount > 0 && invoiceData.invoiceDate && invoiceData.vendorCode) {
    ...
    if (similarInvoices.length > 0 && !insights.find(i => i.id === 'duplicate-exact')) {
      insights.push({
        ...
        evidence: similarInvoices.slice(0, 3).map(inv => 
  ...
  if (poData && grnData && invoiceData.lineItems.length > 0) {
    ...
    invoiceData.lineItems.forEach(lineItem => {
      ...
      if (lineItem.qty > lineItem.grnQty) {
        ...
        mismatches.push(`${lineItem.itemName}: Invoice qty (${lineItem.qty}) exceeds GRN qty (${lineItem.grnQty}) by ${variance.toFixed(1)}%`);
      ...
      if (lineItem.unitPrice !== lineItem.poRate) {
        ...
        mismatches.push(`${lineItem.itemName}: Rate variance of ${variance.toFixed(1)}% (PO: ₹${lineItem.poRate}, Invoice: ₹${lineItem.unitPrice})`);
  ...
  if (invoiceData.vendorGSTIN && vendorData) {
    const isValidFormat = /^[0-9]{2}[A-Z]{5}[0-9]{4}[A-Z]{1}[1-9A-Z]{1}Z[0-9A-Z]{1}$/.test(invoiceData.vendorGSTIN);
    ...
    if (!isValidFormat) {
      insights.push({
        ...
        explanation: 'The GSTIN provided does not match the standard 15-character format required by GST regulations.',
        confidence: 100,
        recommendedActions: [
          {
            label: 'Verify GSTIN',
      ...
      insights.push({
        ...
        explanation: 'The GSTIN on this invoice differs from the GSTIN registered in the vendor master.',
        confidence: 88,
        recommendedActions: [
          {
  ...
  if (vendorData && vendorData.bankAccountChanged) {
    ...
    const daysSinceChange = Math.floor((new Date().getTime() - changeDate.getTime()) / (1000 * 60 * 60 * 24));
  ...
  if (vendorData && vendorData.averageInvoiceAmount && invoiceData.invoiceAmount > 0) {
    const variance = ((invoiceData.invoiceAmount - vendorData.averageInvoiceAmount) / vendorData.averageInvoiceAmount) * 100;

    if (Math.abs(variance) > 200) { // 200% deviation
      insights.push({
        id: 'unusual-amount',
        severity: 'warning',
        category: 'fraud',
  ...
  if (vendorData && vendorData.paymentTerms) {
    ...
    insights.push({
      ...
      evidence: [
        ...
        `Due Date: ${dueDate}`,
        vendorData.isMSME ? 'MSME Compliance: Must pay within 45 days' : 'Standard payment terms apply'
      ]
    });
  }
 ...
 */
function suggestTDSSection(vendorCategory: string, invoiceData: InvoiceData): {
  section: string;
  rate: number;
  rationale: string;
} {
  ...
  return categoryMap[vendorCategory] || {
    section: '194Q',
    rate: 0.1,
    rationale: 'Purchase of goods from vendors with turnover > 10 crores attracts TDS under Section 194Q at 0.1%'
  };
}
