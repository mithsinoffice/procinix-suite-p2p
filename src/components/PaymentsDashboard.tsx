...
export function PaymentsDashboard() {
  ...
  const { user } = useAuth();
  const [liveInvoices, setLiveInvoices] = useState<PaymentInvoice[]>([]);
  const [paidByDay, setPaidByDay] = useState<Record<string, number>>({});
  ...
  const loadDashboard = useCallback(async () => {
    ...
    setDashError(null);
    try {
      const payload = await fetchPaymentsDashboard(
        user.tenantId,
        user.currentPlatformEntityId ?? undefined
      );
      setLiveInvoices(payload.invoices);
      setPaidByDay(payload.paidByDay || {});
  ...
  const payments = useMemo(() => {
    if (user?.tenantId && liveReady) return liveInvoices;
    return mockPayments;
  }, [user?.tenantId, liveReady, liveInvoices]);
  ...
  const dueToday = useMemo(
    () =>
      payments
        .filter((p) => p.dueDate === todayStr && p.status !== 'paid')
  ...
  const earlyPaymentDiscount = useMemo(
    () =>
      payments
        .filter(
          (p) => p.earlyPaymentDiscount && p.discountDate && new Date(p.discountDate) >= today
        )
        .reduce((sum, p) => sum + (p.earlyPaymentDiscount || 0), 0),
    [payments, today]
  );

  const criticalPaymentsCount = useMemo(
    () => payments.filter((p) => p.priority === 'critical' && p.status !== 'paid').length,
    [payments]
  );

  const highPriorityPayments = useMemo(
    () =>
      payments
        .filter(
          (p) =>
            (p.priority === 'critical' || p.category === 'Statutory' || p.category === 'Payroll') &&
            p.status !== 'paid'
        )
        .slice(0, 3),
    [payments]
  ...
  const suspiciousPayments = useMemo(
    () =>
      payments.filter((p) => p.riskFlag === 'duplicate' || p.riskFlag === 'suspicious').slice(0, 2),
    [payments]
  );
  ...
  const agingData = useMemo(() => getAgingData(payments), [payments]);
  const trendData = useMemo(() => {
    if (user?.tenantId && liveReady) return buildPaymentTrendData(payments, paidByDay);
    return getPaymentTrend();
  }, [user?.tenantId, liveReady, payments, paidByDay]);
  const categoryData = useMemo(() => getCategoryBreakdown(payments), [payments]);
  const projectionData = useMemo(() => getProjectedOutflow(payments), [payments]);

  const COLORS = ['var(--color-teal)', '#3B82F6', '#F59E0B', '#10B981', '#007D87', '#EF4444'];

  const aiTop = useMemo(() => {
    const open = payments.filter((p) => p.status !== 'paid');
    const top = [...open].sort((a, b) => b.amount - a.amount).slice(0, 5);
    const sum = top.reduce((s, p) => s + p.amount, 0);
    return { count: top.length, sum };
  }, [payments]);
    ...
    <div style={{ backgroundColor: 'var(--color-cloud)', minHeight: '100vh' }}>
        ...
        <div className="flex items-center justify-between mb-2">
          <div>
            ...
            {user?.tenantId ? (
              <p className="text-xs mt-2" style={{ color: 'var(--color-teal)', fontWeight: 600 }}>
                Live data: open payables for your tenant (rejected invoices excluded).
              </p>
            ) : null}
            {dashError ? (
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              disabled={dashLoading || !user?.tenantId}
              className="flex items-center gap-2 px-4 py-2 rounded-lg transition-all"
              style={{
                backgroundColor: '#FFFFFF',
                border: '1px solid var(--color-silver)',
                color: 'var(--color-mercury-grey)',
                opacity: dashLoading || !user?.tenantId ? 0.6 : 1,
              }}
            >
        ...
        >
          ...
          <span>
            ...
            {lastRefresh
              ...
              : user?.tenantId
                ? '—'
                : 'Demo dataset (sign in with tenant for live data)'}
          </span>
        </div>
      </div>
      ...
      <div className="px-8 py-6">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p style={{ fontSize: '14px', color: 'var(--color-mercury-grey)', margin: 0 }}>
                  {user?.tenantId && liveReady
                    ? `Top ${aiTop.count} open payables by amount total ${formatCompactCurrency(aiTop.sum)} (see Payment proposal for batching).`
            ...
            <div className="flex items-center gap-6">
              <div className="text-center">
                <div style={{ fontSize: '24px', fontWeight: '700', color: 'var(--color-teal)' }}>
                  {aiTop.count}
                </div>
              ...
              <div className="text-center">
                <div style={{ fontSize: '24px', fontWeight: '700', color: 'var(--color-teal)' }}>
                  {formatCompactCurrency(aiTop.sum)}
                </div>
                <div style={{ fontSize: '12px', color: 'var(--color-mercury-grey)' }}>
                  Total Amount
                </div>
              </div>
              <button
        ...
        <div className="grid grid-cols-1 md:grid-cols-6 gap-4">
          ...
          >
            <div className="flex items-start justify-between mb-3">
              ...
              <span
                ...
                style={{ backgroundColor: '#FEF3C7', color: '#F59E0B', fontWeight: '600' }}
              >
                TODAY
              </span>
            </div>
            ...
            <div className="text-xs" style={{ color: 'var(--color-mercury-grey)' }}>
              {payments.filter((p) => p.dueDate === todayStr && p.status !== 'paid').length}{' '}
              payments
            </div>
          </div>
          ...
          {/* Overdue */}
          <div className="bg-white rounded-lg p-4" style={{ border: '2px solid #EF4444' }}>
            <div className="flex items-start justify-between mb-3">
              <div
                className="w-10 h-10 rounded-lg flex items-center justify-center"
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div
              ...
              style={{ backgroundColor: 'var(--color-error-light)', border: '1px solid #FECACA' }}
            >
      ...
      <div className="px-8 pb-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          >
            <h3
              className="mb-4"
              style={{ color: 'var(--color-ink)', fontWeight: '700', fontSize: '15px' }}
            >
              Aging Analysis
            </h3>
              ...
              <BarChart data={agingData}>
                ...
                <YAxis
                  ...
                  label={{
                    ...
                    angle: -90,
                    position: 'insideLeft',
                    style: { fill: 'var(--color-mercury-grey)', fontSize: 12 },
                  }}
                />
                <Tooltip
                  contentStyle={{
                ...
                <tbody>
                  {criticalPayments.map((payment, index) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-4 text-center">{getRiskBadge(payment.riskFlag)}</td>
