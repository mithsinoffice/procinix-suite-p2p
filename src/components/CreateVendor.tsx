...
export function CreateVendor() {
  ...
  const { addVendor, updateVendor, getVendorById, entities, vendors, vendorGroups } =
    useMasterData();
  const [paymentTermsMaster] = useIncrementalMasterRecords<{
    id: string;
    code: string;
    description: string;
    creditDays: string;
  ...
  const activePaymentTerms = useMemo(
    () => paymentTermsMaster.filter((t) => t.status === 'Active'),
    [paymentTermsMaster]
  );
  ...
  const [cinValidationStatus, setCinValidationStatus] = useState<
    'PENDING' | 'VALIDATED' | 'FAILED'
  >('PENDING');
  const [udyamValidationStatus, setUdyamValidationStatus] = useState<
    'PENDING' | 'VALIDATED' | 'FAILED'
  >('PENDING');
  const [gstValidationStatus, setGstValidationStatus] = useState<{
    [key: string]: 'PENDING' | 'VALIDATED' | 'FAILED';
  }>({});
    ...
    setGstDetails(
      v.addresses.length > 0
        ? v.addresses.map((a) => ({
            ...
            detailedAddress:
              [a.addressLine1, a.addressLine2].filter(Boolean).join(', ') || a.addressLine1,
          }))
        : v.gstin
          ? [
              {
  ...
  const buildPayload = (workflowOverride?: string): VendorMaster | null => {
    ...
    return {
      ...
      msmeCategory: isMSMERegistered
        ? (msmeClassification as 'Micro' | 'Small' | 'Medium')
        : undefined,
      status: masterStatus,
      ...
      bankAccounts: bankAccounts.map((bank) => ({
        ...
        verifiedDate:
          bank.verificationStatus === 'Validated'
      ...
      approvedBy:
        approvalStatus === 'Approved'
          ? internalSPOC || existing?.approvedBy || 'System'
          : existing?.approvedBy,
      approvedDate:
        approvalStatus === 'Approved'
    ...
    >
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-3 gap-6">
          ...
          <div
            className="col-span-2 bg-white rounded-lg p-6 space-y-6"
            style={{ border: '1px solid var(--color-silver)' }}
          >
            ...
            <div>
              ...
              {gstDetails.length === 0 ? (
                ...
                >
                  <p style={{ color: 'var(--color-mercury-grey)' }}>No GST registrations added.</p>
                </div>
              ) : (
                      ...
                      <div className="grid grid-cols-5 gap-4">
                        <div>
                          ...
                          >
                            ...
                            {gstValidationStatus[gst.id] === 'PENDING'
                              ? 'Validate GSTIN'
                              : gstValidationStatus[gst.id]}
                          </button>
                        </div>
            ...
            <div>
              ...
              {bankAccounts.length === 0 ? (
                ...
                >
                  <p style={{ color: 'var(--color-mercury-grey)' }}>No bank accounts added.</p>
                </div>
              ) : (
                <div className="space-y-4">
        ...
        >
          <FormSection title="Financial Mapping & Accounts" columns={2}>
            <PxFormField
              label="Mapped Services (Comma separated)"
              filled={mappedServices.length > 0 && mappedServices[0] !== ''}
            >
              <input
                type="text"
            ...
            <PxFormField
              label="Mapped Departments (Comma separated)"
              filled={mappedDepartments.length > 0 && mappedDepartments[0] !== ''}
            >
              <input
                type="text"
                value={mappedDepartments.join(', ')}
                onChange={(e) =>
                  setMappedDepartments(e.target.value.split(',').map((d) => d.trim()))
                }
          ...
          <div>
            ...
            {entityMappings.length === 0 ? (
              ...
              >
                <p style={{ color: 'var(--color-mercury-grey)' }}>No entity mappings added.</p>
              </div>
            ) : (
              <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-4 gap-4">
                      <div>
                        ...
                        >
                          ...
                          {entities
                            .filter((ent: any) => ent.isActive !== false)
                            .map((ent: any) => (
                              <option key={ent.id} value={ent.name || ent.legalName}>
                                {ent.name || ent.legalName}
                              </option>
                            ))}
                        </select>
                      </div>
                        ...
                        <input
                          ...
                          onChange={(e) => {
                            setEntityMappings(
                              entityMappings.map((m) =>
                                m.id === mapping.id
                                  ? { ...m, defaultGLAccountCode: e.target.value }
                                  : m
                              )
                            );
                          }}
                        ...
                        <select
                          ...
                          onChange={(e) => {
                            setEntityMappings(
                              entityMappings.map((m) =>
                                m.id === mapping.id
                                  ? { ...m, paymentTermsOverride: e.target.value }
                                  : m
                              )
                            );
                          }}
                        ...
                        >
                          ...
                          {activePaymentTerms.map((term) => (
                            <option key={term.id} value={term.code}>
                              {term.code} - {term.description} ({term.creditDays} days)
                            </option>
                          ))}
                          {mapping.paymentTermsOverride &&
                            !activePaymentTerms.some(
                              (t) => t.code === mapping.paymentTermsOverride
                            ) && (
                              <option value={mapping.paymentTermsOverride}>
                                {mapping.paymentTermsOverride}
                              </option>
                            )}
                        </select>
        ...
        >
          ...
          >
            <p className="text-sm" style={{ color: '#FF9800' }}>
              ℹ️ ERP Vendor Code will be auto-generated once the vendor is approved and synced to
              ERP system.
            </p>
          </div>
        </div>
