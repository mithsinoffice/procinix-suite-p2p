...
export function WeekForecast13() {
  ...
  const getRiskColor = (risk: string) => {
    switch (risk) {
      ...
      case 'high':
        return { bg: 'var(--color-warning-light)', color: '#E65100', border: '#FF9800' };
      case 'critical':
        return {
        ...
        <div className="grid grid-cols-4 gap-6 mb-8">
          ...
          >
            ...
            <p className="text-2xl" style={{ color: 'var(--color-ink)' }}>
              {formatCurrency(weeks[0].opening)}
            </p>
          </div>
          <div
            className="bg-white p-6 rounded-lg"
          ...
          >
            ...
            <p className="text-2xl" style={{ color: 'var(--color-ink)' }}>
              {formatCurrency(weeks[12].closing)}
            </p>
          </div>
        </div>
                ...
                {weeks.map((week) => {
                  ...
                  return (
                    <tr
                      key={week.week}
                      style={{
                        backgroundColor: belowBuffer ? 'var(--color-error-light)' : '#FFFFFF',
                      }}
                    >
                      <td
                      ...
                      <td
                        className="px-6 py-4 text-sm text-right"
                        style={{
                          color: belowBuffer ? 'var(--color-error-dark)' : 'var(--color-ink)',
                          fontWeight: '600',
                        }}
                      >
        ...
        >
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-sm" style={{ color: 'var(--color-mercury-grey)', margin: 0 }}>
                Cash drops to {formatCurrency(44820000)} (below minimum buffer of{' '}
                {formatCurrency(minimumBuffer)}) due to GST payment + delayed collections.
                <br />
                <strong>AI Recommendation:</strong> Defer non-critical vendor payments or utilize
                ₹10 Cr overdraft facility.
              </p>
            </div>
          </div>
        </div>
