...
export function POInvoiceExceptionModal({ isOpen, onClose, onSubmit, lineItem }: POInvoiceExceptionModalProps) {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-xl w-full max-w-3xl mx-4 max-h-[90vh] overflow-y-auto" style={{ border: '1px solid #E1E6EA' }}>
        {/* Header */}
        <div className="flex items-center justify-between p-6" style={{ borderBottom: '1px solid #E1E6EA' }}>
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg" style={{ color: '#0A0F14', fontWeight: '700' }}>Request Rate Exception Approval</h2>
              <p className="text-sm" style={{ color: '#6E7A82' }}>Invoice rate exceeds PO rate - requires CFO approval</p>
            </div>
          </div>
        ...
        <div className="p-6">
          {/* Item Details Card */}
          <div className="bg-gray-50 rounded-lg p-4 mb-6" style={{ border: '1px solid #E1E6EA' }}>
            <div className="flex items-start justify-between">
              <div>
                <p className="text-sm" style={{ color: '#6E7A82', fontWeight: '500' }}>Item</p>
                <p className="text-base mt-1" style={{ color: '#0A0F14', fontWeight: '600' }}>{lineItem.itemName}</p>
                <p className="text-sm" style={{ color: '#6E7A82' }}>{lineItem.itemCode}</p>
              </div>
              <div className="text-right">
                <p className="text-sm" style={{ color: '#6E7A82', fontWeight: '500' }}>Quantity</p>
                <p className="text-base mt-1" style={{ color: '#0A0F14', fontWeight: '600' }}>{lineItem.quantity}</p>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-3 gap-4 mb-6">
            <div className="bg-white rounded-lg p-4" style={{ border: '2px solid #E1E6EA' }}>
              <div className="flex items-center gap-2 mb-2">
                <FileText className="w-4 h-4" style={{ color: '#00A9B7' }} />
                <p className="text-xs" style={{ color: '#6E7A82', fontWeight: '600' }}>PO RATE</p>
              </div>
              <p className="text-2xl" style={{ color: '#0A0F14', fontWeight: '700' }}>
                ₹{lineItem.poRate.toFixed(2)}
              </p>
              <p className="text-xs mt-1" style={{ color: '#6E7A82' }}>Original agreed rate</p>
            </div>

            <div className="bg-white rounded-lg p-4" style={{ border: '2px solid #D97706' }}>
              <div className="flex items-center gap-2 mb-2">
                <DollarSign className="w-4 h-4" style={{ color: '#D97706' }} />
                <p className="text-xs" style={{ color: '#6E7A82', fontWeight: '600' }}>REQUESTED RATE</p>
              </div>
              <p className="text-2xl" style={{ color: '#D97706', fontWeight: '700' }}>
                ₹{lineItem.requestedRate.toFixed(2)}
              </p>
              <p className="text-xs mt-1" style={{ color: '#6E7A82' }}>Invoice rate entered</p>
            </div>

            <div className="bg-red-50 rounded-lg p-4" style={{ border: '2px solid #FF4E5B' }}>
              <div className="flex items-center gap-2 mb-2">
                <TrendingUp className="w-4 h-4" style={{ color: '#FF4E5B' }} />
                <p className="text-xs" style={{ color: '#FF4E5B', fontWeight: '600' }}>VARIANCE</p>
              </div>
              <p className="text-2xl" style={{ color: '#FF4E5B', fontWeight: '700' }}>
                +₹{formData.varianceAmount.toFixed(2)}
              </p>
              <p className="text-xs mt-1" style={{ color: '#FF4E5B' }}>
          ...
          {/* Total Impact */}
          <div className="bg-red-50 rounded-lg p-4 mb-6" style={{ border: '1px solid #FF4E5B' }}>
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm" style={{ color: '#6E7A82', fontWeight: '500' }}>Total Financial Impact</p>
              ...
              <div className="text-right">
                ...
                <p className="text-xs" style={{ color: '#FF4E5B' }}>Additional cost</p>
              </div>
            </div>
          </div>
          ...
          <div className="mb-6">
            <label className="block text-sm mb-2" style={{ color: '#0A0F14', fontWeight: '600' }}>
              Exception Reason <span style={{ color: '#FF4E5B' }}>*</span>
            </label>
            <select
          ...
          <div className="mb-6">
            <label className="block text-sm mb-2" style={{ color: '#0A0F14', fontWeight: '600' }}>
              Detailed Explanation <span style={{ color: '#FF4E5B' }}>*</span>
            </label>
            <textarea
              value={formData.comments}
              onChange={(e) => setFormData({ ...formData, comments: e.target.value })}
              className="w-full px-4 py-3 rounded-lg text-sm"
              style={{ border: '1px solid #E1E6EA', color: '#0A0F14', minHeight: '120px' }}
          ...
          <div className="mb-6">
            ...
            <div className="border-2 border-dashed rounded-lg p-6 text-center" style={{ borderColor: '#E1E6EA' }}>
              <Upload className="w-8 h-8 mx-auto mb-2" style={{ color: '#6E7A82' }} />
              <p className="text-sm mb-2" style={{ color: '#0A0F14' }}>Upload supporting documents</p>
              <p className="text-xs mb-4" style={{ color: '#6E7A82' }}>
                Vendor rate sheet, email correspondence, market analysis, etc.
              </p>
              <label className="inline-block px-4 py-2 rounded-lg cursor-pointer transition-colors" style={{ backgroundColor: '#00A9B7', color: '#FFFFFF' }}>
                <input
                  type="file"
                  multiple
            ...
            {attachments.length > 0 && (
              <div className="mt-4 space-y-2">
                {attachments.map((file, index) => (
                  <div key={index} className="flex items-center justify-between p-3 rounded-lg" style={{ backgroundColor: '#F6F9FC', border: '1px solid #E1E6EA' }}>
                    <div className="flex items-center gap-3">
                      <FileText className="w-4 h-4" style={{ color: '#00A9B7' }} />
                      <div>
                        <p className="text-sm" style={{ color: '#0A0F14', fontWeight: '500' }}>{file.name}</p>
                    ...
                    >
                      <X className="w-4 h-4" style={{ color: '#FF4E5B' }} />
                    </button>
                  </div>
                ))}
              </div>
          ...
          {/* Approval Info */}
          <div className="bg-blue-50 rounded-lg p-4" style={{ border: '1px solid #00A9B7' }}>
            <p className="text-sm" style={{ color: '#0A0F14', fontWeight: '600' }}>Approval Routing</p>
        ...
        {/* Footer */}
        <div className="flex items-center justify-end gap-3 p-6" style={{ borderTop: '1px solid #E1E6EA' }}>
          <button
            onClick={onClose}
