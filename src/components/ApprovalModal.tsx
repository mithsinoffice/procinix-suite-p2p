    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-lg w-full max-w-3xl max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div className="border-b px-6 py-4 flex items-center justify-between flex-shrink-0" style={{ borderColor: '#E1E6EA' }}>
          <div>
            <h2 className="text-xl" style={{ color: '#0A0F14' }}>Review Changes - {recordType}</h2>
            <p className="text-sm mt-1" style={{ color: '#6E7A82' }}>Record ID: {recordId}</p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-6 flex-1 overflow-y-auto">
          <h3 className="text-lg mb-4" style={{ color: '#0A0F14' }}>Proposed Changes</h3>
          
          {changes.length === 0 ? (
            <p style={{ color: '#6E7A82' }}>No changes detected</p>
          ) : (
            <div className="space-y-4">
              {changes.map((change, index) => (
                ...
                >
                  <div className="mb-2">
                    <span className="text-sm" style={{ color: '#6E7A82' }}>Field:</span>
                    <span className="ml-2" style={{ color: '#0A0F14' }}>{change.field}</span>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-4 mt-3">
                    ...
                    >
                      <p className="text-xs mb-1" style={{ color: '#FF4E5B' }}>Previous Value</p>
                      <p className="text-sm" style={{ color: '#0A0F14' }}>
                        {change.oldValue || <em style={{ color: '#6E7A82' }}>(empty)</em>}
                      </p>
                    </div>
                    ...
                    <div
                      className="p-3 rounded-lg"
                      style={{ backgroundColor: '#E8F7F8', border: '1px solid #B3E5EA' }}
                    >
                      <p className="text-xs mb-1" style={{ color: '#00A9B7' }}>New Value</p>
                      <p className="text-sm" style={{ color: '#0A0F14' }}>
                        {change.newValue || <em style={{ color: '#6E7A82' }}>(empty)</em>}
                      </p>
                    </div>
                  </div>
                </div>
        ...
        >
          ...
          <button
            ...
            onMouseLeave={(e) => e.currentTarget.style.backgroundColor = '#FF4E5B'}
          >
