...
import { Sparkles, TrendingUp, CheckCircle, Library, Plus, X, Play, Check, ChevronDown, ChevronUp, Loader2 } from 'lucide-react';
...
interface FieldLogicStepProps {
  ...
  externalRuleToAdd?: { fieldName: string; rule: { type: string; config: string; severity: string } } | null;
  onExternalRuleAdded?: () => void;
}

type RuleType = 'Required' | 'Format' | 'Duplicate' | 'CrossReference' | 'Math' | 'Threshold' | 'Blacklist' | 'Custom';
...
export function FieldLogicStep({ selectedModule = 'Accounts Payable', selectedForm = 'AP Invoice', onFieldSelect, externalRuleToAdd, onExternalRuleAdded }: FieldLogicStepProps) {
  ...
  const [testResultsMap, setTestResultsMap] = useState<Record<string, Array<{ rule: string; passed: boolean; reason?: string }>>>({});
  ...
  const [globalRules, setGlobalRules] = useState({
    preExecution: [
      'Confidence score must be ≥ threshold',
  ...
  const loadFormFields = (formName: string) => {
    const f = (name: string, dataType: string, required: boolean, category: string, rules: Field['rules'] = [], aiSuggestions: Field['aiSuggestions'] = []): Field =>
      ({ name, dataType, required, category, rules, aiSuggestions });
    const req = (id: string): Field['rules'][0] => ({ id, type: 'Required', config: 'Cannot be empty', severity: 'Block', color: '#00A9B7' });
    const math = (id: string, c: string): Field['rules'][0] => ({ id, type: 'Math', config: c, severity: 'Block', color: '#10B981' });

    const invoiceFields: Field[] = [
      f('invoice_number', 'Text', true, 'Header', [req('1')], [{ title: 'Duplicate check', description: 'Prevent duplicate invoice numbers' }]),
      f('invoice_date', 'Date', true, 'Header', [req('2')], [{ title: 'Future date check', description: 'Warn if > 7 days in future' }]),
      f('due_date', 'Date', true, 'Header', [req('3')], [{ title: 'Due date calc', description: 'Auto from invoice date + payment terms' }]),
      f('payment_terms', 'Text', false, 'Header', [], [{ title: 'Terms lookup', description: 'Validate vendor payment terms' }]),
      f('vendor_name', 'Text', true, 'Vendor', [req('4')], [{ title: 'Vendor master lookup', description: 'Cross-reference with vendor master' }]),
      f('vendor_code', 'Text', false, 'Vendor'),
      f('vendor_group', 'Text', false, 'Vendor'),
      f('vendor_gstin', 'Text', false, 'Vendor', [], [{ title: 'GST format', description: 'Validate 15-char GSTIN pattern' }, { title: 'State code match', description: 'Match GSTIN state with vendor address' }]),
      f('vendor_pan', 'Text', false, 'Vendor', [], [{ title: 'PAN format', description: 'Validate 10-char PAN pattern' }]),
      f('vendor_state', 'Text', false, 'Vendor'),
      f('vendor_location', 'Text', false, 'Vendor'),
      f('entity', 'Text', true, 'Entity', [req('5')], [{ title: 'Entity lookup', description: 'Must exist and be active in entity master' }]),
      f('billing_location', 'Text', true, 'Entity', [req('6')]),
      f('bill_to_gstin', 'Text', false, 'Entity', [], [{ title: 'GSTIN match', description: 'Bill-to GSTIN must match entity registration' }]),
      f('department', 'Text', true, 'Entity', [req('7')], [{ title: 'Dept lookup', description: 'Validate against department master' }]),
      f('sub_department', 'Text', false, 'Entity'),
      f('base_amount', 'Currency', true, 'Amount', [req('8'), math('9', 'Must be > 0')]),
      f('gross_amount', 'Currency', true, 'Amount', [req('10'), math('11', 'Must be > 0')], [{ title: 'Cross-field check', description: 'Gross = base + total GST' }]),
      ...
      f('item_name', 'Text', true, 'Line Item', [req('12')], [{ title: 'Item master lookup', description: 'Cross-reference with item/product master' }]),
      f('item_code', 'Text', false, 'Line Item', [], [{ title: 'Item code check', description: 'Verify item code exists in item master' }]),
      f('item_description', 'Text', false, 'Line Item'),
      f('gl_code', 'Text', false, 'Line Item', [], [{ title: 'GL code validation', description: 'Verify account code in chart of accounts' }]),
      f('quantity', 'Number', true, 'Line Item', [req('13'), math('14', 'Must be > 0')]),
      f('rate', 'Currency', true, 'Line Item', [req('15'), math('16', 'Must be > 0')]),
      f('line_amount', 'Currency', true, 'Line Item', [req('17')], [{ title: 'Line calc', description: 'Amount = qty x rate' }]),
      f('gst_percent', 'Number', false, 'Line Item Tax', [], [{ title: 'GST rate check', description: 'Verify GST % is valid (0, 5, 12, 18, 28)' }]),
      ...
      f('total_gst', 'Currency', false, 'Line Item Tax', [], [{ title: 'GST calc check', description: 'Total GST = CGST + SGST + IGST + cess' }]),
      f('line_gross_amount', 'Currency', false, 'Line Item Tax', [], [{ title: 'Gross calc', description: 'Line gross = amount + total GST' }]),
      f('tds_section', 'Text', false, 'TDS', [], [{ title: 'TDS section check', description: 'Validate TDS section code (194C, 194J, etc.)' }]),
      f('tds_percent', 'Number', false, 'TDS', [], [{ title: 'TDS rate check', description: 'Verify TDS % matches section + vendor category' }]),
      f('tds_amount', 'Currency', false, 'TDS'),
      f('lower_tds', 'Boolean', false, 'TDS', [], [{ title: 'Lower TDS cert', description: 'If true, verify vendor has valid lower TDS certificate' }]),
      f('sec_206ab', 'Text', false, 'TDS', [], [{ title: '206AB check', description: 'Verify vendor ITR filing status for higher TDS' }]),
      f('net_payable', 'Currency', false, 'TDS', [], [{ title: 'Net payable', description: 'Net = gross - TDS' }]),
      f('cost_center', 'Text', false, 'Allocation', [], [{ title: 'Cost centre lookup', description: 'Validate against cost centre master' }]),
      f('profit_center', 'Text', false, 'Allocation', [], [{ title: 'Profit centre lookup', description: 'Validate against profit centre master' }]),
      f('ship_to', 'Text', false, 'Allocation'),
      f('project_code', 'Text', false, 'Allocation'),
      f('ocr_item', 'Text', false, 'OCR'),
      f('ocr_confidence', 'Text', false, 'OCR', [], [{ title: 'Confidence threshold', description: 'Flag items with confidence < 80%' }]),
    ];
    ...
    const schemas: Record<string, Field[]> = {
      ...
      'Purchase Order': [
        f('po_number', 'Text', true, 'Identity', [req('1')], [{ title: 'Auto-number', description: 'Auto-generate sequential PO number' }]),
        f('po_date', 'Date', true, 'Identity', [req('2')]),
        f('vendor_name', 'Text', true, 'Vendor', [req('3')], [{ title: 'Vendor lookup', description: 'Must exist in vendor master' }]),
        f('vendor_code', 'Text', true, 'Vendor', [req('4')]),
        f('entity', 'Text', true, 'Identity', [req('5')]),
        f('currency', 'Text', true, 'Identity', [req('6')]),
        f('delivery_date', 'Date', true, 'Logistics', [req('7')], [{ title: 'Lead time', description: 'Delivery date > PO date + lead time' }]),
      ...
      'Purchase Requisition': [
        ...
        f('estimated_amount', 'Currency', false, 'Financial', [], [{ title: 'Budget check', description: 'Verify against department budget' }]),
      ...
      'Goods Receipt Note': [
        ...
        f('po_number', 'Text', true, 'Reference', [req('3')], [{ title: 'PO check', description: 'PO must exist and be open' }]),
        f('vendor_name', 'Text', true, 'Vendor', [req('4')]),
        f('entity', 'Text', true, 'Identity', [req('5')]),
        f('received_qty', 'Number', true, 'Quantity', [req('6'), math('7', 'Must be > 0')], [{ title: 'Over-receipt', description: 'Warn if received > ordered' }]),
      ...
      'Debit Note': [
        ...
        f('original_invoice', 'Text', false, 'Reference', [], [{ title: 'Invoice ref', description: 'Original invoice must exist' }]),
      ],
      'Payment Proposal': [
      ...
      'Vendor Onboarding': [
        ...
        f('vendor_code', 'Text', true, 'Identity', [req('2')], [{ title: 'Duplicate', description: 'No existing vendor with same code' }]),
        f('gstin', 'Text', false, 'Compliance', [], [{ title: 'GSTIN format', description: 'Validate 15-char pattern' }]),
        f('pan', 'Text', false, 'Compliance', [], [{ title: 'PAN format', description: 'Validate 10-char pattern' }]),
        f('msme_number', 'Text', false, 'Compliance'),
        f('bank_account', 'Text', false, 'Payment'),
        f('ifsc_code', 'Text', false, 'Payment', [], [{ title: 'IFSC check', description: 'Validate IFSC format' }]),
        f('entity', 'Text', true, 'Identity', [req('3')]),
        f('payment_terms', 'Text', false, 'Payment'),
        f('contact_email', 'Text', false, 'Contact', [], [{ title: 'Email format', description: 'Validate email' }]),
      ...
      '__placeholder__': [
        {
          ...
          category: 'Identity',
          rules: [
            { id: '1', type: 'Required', config: 'Cannot be empty', severity: 'Block', color: '#00A9B7' }
          ],
          aiSuggestions: [
        ...
        },
        {
          ...
          category: 'Identity',
          rules: [
            { id: '2', type: 'Required', config: 'Cannot be empty', severity: 'Block', color: '#00A9B7' }
          ],
          aiSuggestions: [
            { title: 'Check future dates', description: 'Warn if invoice date is > 7 days in future' }
          ]
        },
        {
          name: 'vendor_name',
          ...
          category: 'Vendor',
          rules: [
            { id: '3', type: 'Required', config: 'Cannot be empty', severity: 'Block', color: '#00A9B7' }
          ]
        },
        {
          name: 'vendor_gstin',
        ...
        },
        {
          ...
          category: 'Financial',
          rules: [
            { id: '4', type: 'Required', config: 'Cannot be empty', severity: 'Block', color: '#00A9B7' },
        ...
        },
        {
          ...
          category: 'Financial',
          rules: [
            { id: '6', type: 'Required', config: 'Cannot be empty', severity: 'Block', color: '#00A9B7' }
          ]
        },
        {
          name: 'total_amount',
          ...
          category: 'Financial',
          rules: [
            { id: '7', type: 'Required', config: 'Cannot be empty', severity: 'Block', color: '#00A9B7' },
  ...
  const rulesNeeded = Math.max(0, fields.length * 2 - fields.reduce((sum, f) => sum + f.rules.length, 0));

  const handleReviewRules = () => {
    setReviewData({
      predicted_accuracy: 94.5,
      gaps: [
        { field: 'vendor_gstin', missing_rule: 'Format validation', reason: 'GSTIN format should be validated', priority: 'High' },
        { field: 'invoice_date', missing_rule: 'Date range check', reason: 'Future dates should be flagged', priority: 'Medium' }
      ],
      redundancies: [],
      recommendations: [
  ...
  void selectedModule;
  void activeTab; void setActiveTab; void generateInput; void setGenerateInput;
  void reviewData; void setReviewData; void handleReviewRules;
    ...
    <div className="flex gap-6 h-full">
      ...
      <div className="flex-1 overflow-auto">
        ...
        >
          ...
          <span style={{ color: '#0A0F14', fontWeight: '600', fontSize: '13px', whiteSpace: 'nowrap' }}>
            Estimated accuracy:
          </span>
          <span style={{ color: accuracyColor, fontWeight: '700', fontSize: '16px', whiteSpace: 'nowrap' }}>
            {accuracy.toFixed(1)}%
          </span>
          <div className="flex-1 h-2 rounded-full overflow-hidden" style={{ backgroundColor: '#E1E6EA' }}>
            <div
          ...
          <button
            ...
            style={{ backgroundColor: '#8B5CF6', color: '#FFFFFF', fontSize: '11px', fontWeight: '600' }}
          >
        ...
        {showTemplates && (
          <div className="mb-5">
            <div className="flex items-center justify-between mb-3">
              <h3 style={{ color: '#0A0F14', fontWeight: '600', fontSize: '15px' }}>Rule Templates</h3>
              <button onClick={() => setShowTemplates(false)} style={{ color: '#6E7A82', fontSize: '12px' }}>Close</button>
            </div>
        ...
        <div className="mb-5 bg-white rounded-xl p-5" style={{ border: '1px solid #E1E6EA', borderLeft: '4px solid #8B5CF6' }}>
          <h3 className="mb-4" style={{ color: '#0A0F14', fontWeight: '600', fontSize: '14px' }}>Global Rules</h3>
          <div className="grid grid-cols-3 gap-5">
            {/* Pre-execution */}
            <div>
              <div style={{ color: '#6E7A82', fontSize: '10px', textTransform: 'uppercase', letterSpacing: '0.5px', marginBottom: '8px', fontWeight: '600' }}>
              ...
              <ul className="space-y-1.5">
                {globalRules.preExecution.map((rule, idx) => (
                  <li key={idx} className="flex items-start gap-2">
                    ...
                    <span style={{ color: '#0A0F14', fontSize: '12px', lineHeight: '1.4' }}>{rule}</span>
                  </li>
                ))}
              </ul>
            </div>
            ...
            <div>
              ...
              <ul className="space-y-1.5">
                {globalRules.crossField.map((rule, idx) => (
                  <li key={idx} className="flex items-start gap-2">
                    <CheckCircle className="w-3.5 h-3.5 flex-shrink-0 mt-0.5" style={{ color: '#8B5CF6' }} />
                    <span style={{ color: '#0A0F14', fontSize: '12px', lineHeight: '1.4' }}>{rule}</span>
                  </li>
                ))}
              </ul>
            ...
            <div>
              ...
              <ul className="space-y-1.5">
                {globalRules.postValidation.map((rule, idx) => (
                  <li key={idx} className="flex items-start gap-2">
                    <CheckCircle className="w-3.5 h-3.5 flex-shrink-0 mt-0.5" style={{ color: '#00A9B7' }} />
                    <span style={{ color: '#0A0F14', fontSize: '12px', lineHeight: '1.4' }}>{rule}</span>
                  </li>
                ))}
              </ul>
        ...
        >
          {/* Table header banner */}
          <div className="px-5 py-3 flex items-center justify-between" style={{ borderBottom: '1px solid #E1E6EA', backgroundColor: '#F6F9FC' }}>
          ...
          >
            <div />
            <span style={{ color: '#6E7A82', fontSize: '10px', textTransform: 'uppercase', letterSpacing: '0.5px', fontWeight: '700', paddingLeft: '12px' }}>Field Name</span>
            <span style={{ color: '#6E7A82', fontSize: '10px', textTransform: 'uppercase', letterSpacing: '0.5px', fontWeight: '700' }}>Type</span>
            <span style={{ color: '#6E7A82', fontSize: '10px', textTransform: 'uppercase', letterSpacing: '0.5px', fontWeight: '700' }}>Req</span>
            <span style={{ color: '#6E7A82', fontSize: '10px', textTransform: 'uppercase', letterSpacing: '0.5px', fontWeight: '700' }}>Category</span>
            <span style={{ color: '#6E7A82', fontSize: '10px', textTransform: 'uppercase', letterSpacing: '0.5px', fontWeight: '700' }}>Rules</span>
            <span style={{ color: '#6E7A82', fontSize: '10px', textTransform: 'uppercase', letterSpacing: '0.5px', fontWeight: '700' }}>Actions</span>
          </div>
          ...
          {Object.entries(groupedFields).map(([category, categoryFields]) => (
            <div key={category}>
              ...
              >
                ...
                <span style={{ color: '#00A9B7', fontSize: '10px', textTransform: 'uppercase', letterSpacing: '1px', fontWeight: '700', whiteSpace: 'nowrap' }}>
                  {category}
                </span>
                <div style={{ flex: 1, height: '1px', backgroundColor: '#D1D9DE' }} />
              </div>
                  ...
                  <div key={field.name}>
                    ...
                    <div
                      ...
                      style={{
                        gridTemplateColumns: '3px 1fr 70px 40px 80px 1fr 120px',
                        minHeight: '44px',
                        backgroundColor: selectedFieldForParent === field.name ? '#F0FDFA' : rowIdx % 2 === 0 ? '#FFFFFF' : '#FAFBFC',
                        borderBottom: '1px solid #F0F3F5',
                        cursor: 'pointer',
                      }}
                      onMouseEnter={(e) => { if (selectedFieldForParent !== field.name) e.currentTarget.style.backgroundColor = '#F0FAFA'; }}
                      onMouseLeave={(e) => { if (selectedFieldForParent !== field.name) e.currentTarget.style.backgroundColor = rowIdx % 2 === 0 ? '#FFFFFF' : '#FAFBFC'; }}
                      onClick={() => {
                    ...
                    >
                      ...
                      <div
                        ...
                        style={{ backgroundColor: selectedFieldForParent === field.name ? '#00A9B7' : hasRules ? '#00A9B720' : 'transparent', width: '3px', transition: 'background-color 0.15s ease' }}
                      />
                      ...
                      <div className="py-2">
                        <span
                          ...
                          style={{ backgroundColor: '#F0F3F5', color: '#6E7A82', fontSize: '11px', fontWeight: '500' }}
                        >
                          {field.dataType}
                        </span>
                      </div>
                        ...
                        {field.rules.length > 0 ? (
                          field.rules.map((rule) => (
                            <span
                              ...
                              style={{ backgroundColor: `${rule.color}15`, fontSize: '10px', fontWeight: '600', color: rule.color }}
                            >
                              <span style={{ width: '5px', height: '5px', borderRadius: '50%', backgroundColor: rule.color, display: 'inline-block' }} />
                              {rule.type}
                              <button
                                onClick={(e) => {
                                  e.stopPropagation();
                          ...
                          <span style={{ color: '#B8C1C8', fontSize: '11px', fontStyle: 'italic' }}>(no rules)</span>
                        )}
                      </div>
                      ...
                      >
                        {isAiLoading ? (
                          <div className="flex items-center gap-2 py-2">
                            <Loader2 className="w-4 h-4 animate-spin" style={{ color: '#4A3BAF' }} />
                            <span style={{ color: '#4A3BAF', fontSize: '12px' }}>Generating AI suggestions...</span>
                          </div>
                        ) : (
                          <div>
                            <div className="flex items-center justify-between mb-2">
                              <div className="flex items-center gap-1.5">
                                ...
                                <span style={{ color: '#4A3BAF', fontSize: '11px', fontWeight: '600' }}>AI Suggestions for {field.name}</span>
                              </div>
                              <button
                            ...
                            <div className="flex gap-2">
                              {(field.aiSuggestions || []).slice(0, 3).map((suggestion, idx) => (
                                <div key={idx} className="flex-1 bg-white p-3 rounded-lg" style={{ border: '1px solid #E1E6EA' }}>
                                  <div style={{ color: '#0A0F14', fontWeight: '600', fontSize: '12px', marginBottom: '3px' }}>
                                    {suggestion.title}
                                  </div>
                              ...
                              {(!field.aiSuggestions || field.aiSuggestions.length === 0) && (
                                <div style={{ color: '#9AA6AF', fontSize: '12px', padding: '8px 0' }}>No suggestions available for this field.</div>
                              )}
                            </div>
                          </div>
                        )}
                    ...
                    {isExpanded && (
                      ...
                      >
                        <div className="grid grid-cols-2 gap-5">
                          ...
                          <div>
                            <div style={{ color: '#6E7A82', fontSize: '10px', textTransform: 'uppercase', letterSpacing: '0.5px', marginBottom: '6px', fontWeight: '600' }}>
                              Custom Logic
                            </div>
                            <textarea
                              placeholder="Describe logic in plain English..."
                              value={customLogicMap[field.name] || ''}
                              onChange={(e) => setCustomLogicMap(prev => ({ ...prev, [field.name]: e.target.value }))}
                              rows={3}
                          ...
                          <div>
                            ...
                            <div className="flex gap-2 mb-2">
                              <input
                                ...
                                onChange={(e) => setTestValueMap(prev => ({ ...prev, [field.name]: e.target.value }))}
                            ...
                            {(testResultsMap[field.name] || []).length > 0 && (
                              <div className="space-y-1">
                                {testResultsMap[field.name].map((result, idx) => (
                                  <div key={idx} className="flex items-start gap-2 p-2 rounded text-xs bg-white">
                                    {result.passed ? (
