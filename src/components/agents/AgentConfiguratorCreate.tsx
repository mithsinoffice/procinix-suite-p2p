...
import {
  Check, X, Save, ArrowRight, FileText, Database, Sparkles,
  CheckCircle, AlertCircle, TrendingUp, Send, Mail, Bell, Zap,
...
export function AgentConfiguratorCreate() {
  ...
  useEffect(() => {
    if (!agentId) return;
    let cancelled = false;
    mysqlApiRequest<{ success: boolean; agent: any; rules: any[]; actions: any[] }>(`/agents/${agentId}`)
      .then((res) => {
        if (cancelled) return;
  ...
  const doAutoSave = useCallback(async () => {
    ...
    try {
      const payload = {
        name: agentName, type: agentType, purpose, target_accuracy: Number(targetAccuracy),
        fallback_action: fallbackAction, application_on: applicationType === 'master' ? 'Master' : 'Form',
        module, form_name: formName, entity_scope: entityScope, trigger_event: triggerEvent, status: 'Draft',
      };
      if (savedIdRef.current) {
        await mysqlApiRequest(`/agents/${savedIdRef.current}`, { method: 'PUT', body: JSON.stringify(payload) });
      } else {
        const res = await mysqlApiRequest<{ success: boolean; data?: { id: string }; id?: string }>('/agents', { method: 'POST', body: JSON.stringify(payload) });
  ...
  }, [agentName, agentType, purpose, targetAccuracy, fallbackAction, applicationType, module, formName, entityScope, triggerEvent, navigate]);
  ...
  useEffect(() => {
    const t = setTimeout(() => { isLoadingRef.current = false; }, 1500);
  ...
  const activeEntities = useMemo(
    () => (entities as Array<{ id: string; name?: string; legalName?: string; isActive?: boolean }>).filter((e) => e.isActive !== false),
    [entities],
  );
  ...
  const [selectedFieldAI, setSelectedFieldAI] = useState<{ name: string; type: string } | null>(null);
  ...
  const [externalRuleToAdd, setExternalRuleToAdd] = useState<{ fieldName: string; rule: { type: string; config: string; severity: string } } | null>(null);
  ...
  const fetchSuggestions = async (fieldName: string, fieldType: string) => {
    ...
    setAiSuggestions([]);
    try {
      const res = await mysqlApiRequest<{ success: boolean; suggestions: any[] }>('/agents/ai/suggest-rules', {
        method: 'POST',
        body: JSON.stringify({
  ...
  const fetchGeneratedRules = async () => {
    ...
    setAiGeneratedRules([]);
    try {
      const res = await mysqlApiRequest<{ success: boolean; suggestions: any[] }>('/agents/ai/generate-rules', {
        method: 'POST',
        body: JSON.stringify({
      ...
      if (!res.suggestions?.length) setAiError('No rules generated. Try a more specific description.');
    } catch (err: any) {
  ...
  const fetchReviewRules = async () => {
    ...
    setAiReview(null);
    try {
      const res = await mysqlApiRequest<{ success: boolean; review: any }>('/agents/ai/review-rules', {
        method: 'POST',
        body: JSON.stringify({
          rules: fieldRules,
          form_context: formName,
  ...
  const renderStep1 = () => (
    <div className="flex-1 p-8 overflow-auto">
      ...
      <div className="grid grid-cols-2 gap-6 mb-6">
        <div>
          <label className="block mb-2" style={{ color: '#6E7A82', fontSize: '11px', textTransform: 'uppercase', letterSpacing: '0.5px' }}>
            Agent Name *
          </label>
          <input
            type="text"
      ...
      <div className="mb-6">
        <label className="block mb-3" style={{ color: '#6E7A82', fontSize: '11px', textTransform: 'uppercase', letterSpacing: '0.5px' }}>
          Application On *
        </label>
        <div className="grid grid-cols-2 gap-4">
          ...
          >
            <FileText className="w-8 h-8 mb-3" style={{ color: applicationType === 'form' ? '#00A9B7' : '#6E7A82' }} />
            <div className="mb-1" style={{ color: '#0A0F14', fontWeight: '600', fontSize: '15px' }}>
              Form
            </div>
            <div style={{ color: '#6E7A82', fontSize: '13px' }}>
              Apply to transactional forms
            </div>
          </div>
          <div
          ...
          >
            <Database className="w-8 h-8 mb-3" style={{ color: applicationType === 'master' ? '#00A9B7' : '#6E7A82' }} />
            <div className="mb-1" style={{ color: '#0A0F14', fontWeight: '600', fontSize: '15px' }}>
              Master
            </div>
            <div style={{ color: '#6E7A82', fontSize: '13px' }}>
      ...
      <div className="grid grid-cols-2 gap-6">
        <div>
          ...
          >
            ...
            {activeEntities.map((e) => <option key={e.id} value={e.name || e.legalName || e.id}>{e.name || e.legalName}</option>)}
          </select>
        </div>
        <div>
          <label className="block mb-2" style={{ color: '#6E7A82', fontSize: '11px', textTransform: 'uppercase', letterSpacing: '0.5px' }}>
    ...
    <div className="flex-1 p-8 overflow-auto">
      ...
      <div className="mb-8">
        <div className="flex items-center gap-3 mb-4">
          ...
          <span style={{ color: '#10B981', fontSize: '11px', textTransform: 'uppercase', letterSpacing: '0.5px', fontWeight: '600', whiteSpace: 'nowrap' }}>
            On Success
          </span>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          {[
            { id: 'approve', icon: CheckCircle, title: 'Auto-approve', desc: 'Automatically approve the record', color: '#10B981' },
            { id: 'route', icon: Send, title: 'Route to next step', desc: 'Continue workflow', color: '#00A9B7' },
            { id: 'notify', icon: Bell, title: 'Send notification', desc: 'Notify stakeholders', color: '#F59E0B' },
            { id: 'log', icon: FileText, title: 'Log success', desc: 'Record in audit trail', color: '#6B7280' }
          ].map(action => {
      ...
      <div className="mb-8">
        <div className="flex items-center gap-3 mb-4">
          ...
          <div style={{ width: '100%', height: '0.5px', backgroundColor: '#E1E6EA' }} />
          <span style={{ color: '#EF4444', fontSize: '11px', textTransform: 'uppercase', letterSpacing: '0.5px', fontWeight: '600', whiteSpace: 'nowrap' }}>
            On Failure
          </span>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          {[
            { id: 'reject', icon: X, title: 'Auto-reject', desc: 'Reject the record', color: '#EF4444' },
            { id: 'escalate', icon: AlertCircle, title: 'Escalate', desc: 'Route to supervisor', color: '#F59E0B' }
          ].map(action => {
            const Icon = action.icon;
      ...
      <div className="bg-white rounded-xl p-6" style={{ border: '1px solid #E1E6EA' }}>
        ...
        <div className="space-y-2">
          {['Validate all fields', 'Check business rules', 'Execute success actions', 'Log activity'].map((step, idx) => (
            <div key={idx} className="flex items-center gap-3 p-3 rounded-lg" style={{ backgroundColor: '#F6F9FC' }}>
              <div
    ...
    <div className="flex-1 p-8 overflow-auto">
      ...
      <div className="grid grid-cols-2 gap-6 mb-8">
        <div className="bg-white rounded-xl p-6" style={{ border: '1px solid #E1E6EA' }}>
          <div className="flex items-center gap-3 mb-4">
            ...
            <span style={{ color: '#6E7A82', fontSize: '11px', textTransform: 'uppercase', letterSpacing: '0.5px' }}>
              Overall Accuracy
            </span>
          </div>
          ...
          <div className="h-2 rounded-full overflow-hidden" style={{ backgroundColor: '#E1E6EA' }}>
            <div className="h-full rounded-full" style={{ width: '99.2%', backgroundColor: '#10B981' }} />
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl p-6" style={{ border: '1px solid #E1E6EA' }}>
          ...
          <div className="h-2 rounded-full overflow-hidden" style={{ backgroundColor: '#E1E6EA' }}>
            <div className="h-full rounded-full" style={{ width: '85%', backgroundColor: '#00A9B7' }} />
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl mb-8" style={{ border: '1px solid #E1E6EA' }}>
        ...
        <table className="w-full">
          ...
          <tbody>
            {[
              { rule: 'Required validation', field: 'Invoice Number', pass: 98, fail: 2, status: 'pass' },
              { rule: 'Format validation', field: 'GSTIN', pass: 95, fail: 5, status: 'pass' },
              { rule: 'Range check', field: 'Amount', pass: 100, fail: 0, status: 'pass' },
              { rule: 'Duplicate check', field: 'Invoice Number', pass: 97, fail: 3, status: 'warning' }
            ].map((result, idx) => (
              <tr key={idx} style={{ borderBottom: '0.5px solid #E1E6EA' }}>
                ...
                <td className="px-6 py-4 text-center" style={{ color: '#10B981', fontSize: '13px', fontWeight: '600' }}>
                  {result.pass}
                </td>
                <td className="px-6 py-4 text-center" style={{ color: '#EF4444', fontSize: '13px', fontWeight: '600' }}>
                  {result.fail}
                </td>
      ...
      {/* Activation Card */}
      <div className="bg-white rounded-xl p-8" style={{ border: '2px solid #10B981', backgroundColor: '#ECFDF5' }}>
            ...
            onClick={async () => {
              ...
              try {
                ...
                if (!id) {
                  ...
                  const res = await mysqlApiRequest<{ success: boolean; data?: { id: string }; id?: string }>('/agents', { method: 'POST', body: JSON.stringify(payload) });
                  const newId = res.data?.id || res.id;
                  if (!newId) { window.alert('Failed to save agent.'); return; }
                  savedIdRef.current = newId;
                  id = newId;
                }
      ...
      <div style={{ backgroundColor: '#FFFFFF', borderBottom: '1px solid #E1E6EA', flexShrink: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', padding: '12px 24px', gap: 24 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexShrink: 0 }}>
            <button onClick={() => navigate('/agent-configurator')} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#9AA6AF', padding: 4 }}>
              <X size={18} />
            </button>
            <h2 style={{ fontSize: 16, fontWeight: 700, color: '#0A0F14', margin: 0 }}>{isEdit ? 'Edit Agent' : 'Create Agent'}</h2>
          </div>
              ...
              <div key={step.number} style={{ display: 'flex', alignItems: 'center', flex: 1 }}>
                <button
                  ...
                  style={{
                    ...
                    cursor: 'pointer', padding: '6px 12px', borderRadius: 8, whiteSpace: 'nowrap',
                ...
                >
                  <div style={{
                    ...
                    backgroundColor: step.completed ? '#10B981' : currentStep === step.number ? '#007D87' : '#E1E6EA',
                    color: step.completed || currentStep === step.number ? '#FFFFFF' : '#9AA6AF',
                  }}>
                    {step.completed ? <Check size={13} /> : step.number}
                  </div>
                  <span style={{
                    fontSize: 13, fontWeight: currentStep === step.number ? 600 : 400,
                    color: currentStep === step.number ? '#007D87' : step.completed ? '#10B981' : '#9AA6AF',
                  }}>
                    {step.label}
                  </span>
                </button>
                {idx < steps.length - 1 && (
                  <div style={{ flex: 1, height: 2, backgroundColor: step.completed ? '#10B981' : '#E1E6EA', margin: '0 4px', minWidth: 16 }} />
                )}
              </div>
            ))}
          </div>
          <div style={{ flexShrink: 0, display: 'flex', alignItems: 'center', gap: 6 }}>
            <div style={{ width: 6, height: 6, borderRadius: '50%', backgroundColor: autoSaved ? '#10B981' : '#E1E6EA' }} />
            <span style={{ fontSize: 11, color: '#9AA6AF' }}>Auto-saved</span>
          </div>
        </div>
        <div style={{ height: 3, backgroundColor: '#E1E6EA' }}>
          <div style={{ height: '100%', width: `${(currentStep / 4) * 100}%`, backgroundColor: '#00A9B7', transition: 'width 0.3s ease' }} />
        </div>
      </div>
    ...
    <div className="flex flex-1 overflow-hidden">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <div className="flex-1 overflow-hidden flex">
          ...
          {currentStep === 1 && (
            <div className="bg-white p-6 overflow-auto" style={{ width: '320px', borderLeft: '0.5px solid #E1E6EA' }}>
              ...
              <div className="space-y-3">
                {[
                  ...
                  <div key={idx} className="p-4 rounded-lg" style={{ border: '1px solid #C7B5F3' }}>
          ...
          {currentStep === 2 && (
            <div style={{ width: 320, borderLeft: '1px solid #E1E6EA', backgroundColor: '#FFFFFF', display: 'flex', flexDirection: 'column', overflow: 'hidden', flexShrink: 0 }}>
              {/* Tab bar */}
              <div style={{ display: 'flex', borderBottom: '1px solid #E1E6EA', flexShrink: 0 }}>
                {(['suggest', 'generate', 'review'] as const).map(tab => (
                  <button key={tab} onClick={() => setAiTab(tab)} style={{
                    flex: 1, padding: '10px 0', fontSize: 12, fontWeight: aiTab === tab ? 700 : 400,
              ...
              {selectedFieldAI ? (
                <div style={{ padding: '12px 16px', borderBottom: '1px solid #E1E6EA', backgroundColor: '#F0FDFA', flexShrink: 0 }}>
                  <span style={{ fontSize: 11, color: '#9AA6AF', textTransform: 'uppercase' as const, letterSpacing: '0.5px' }}>Suggestions for</span>
                  <div style={{ fontSize: 14, fontWeight: 600, color: '#0A0F14', marginTop: 2 }}>{selectedFieldAI.name}</div>
                  <div style={{ fontSize: 11, color: '#6E7A82', marginTop: 1 }}>Type: {selectedFieldAI.type}</div>
                </div>
              ) : (
                <div style={{ padding: 24, textAlign: 'center' as const, color: '#9AA6AF', fontSize: 13 }}>
              ...
              <div style={{ flex: 1, overflow: 'auto', padding: 16 }}>
                ...
                {/* Error state */}
                {aiError && !aiLoading && (
                  <div style={{ padding: 12, backgroundColor: '#FEF2F2', borderRadius: 8, marginBottom: 12, border: '1px solid #FECACA' }}>
                    ...
                    {selectedFieldAI && aiSuggestions.length > 0 ? (
                      <div style={{ display: 'flex', flexDirection: 'column' as const, gap: 10 }}>
                        {aiSuggestions.map((s: any, idx: number) => (
                          <div key={idx} style={{ padding: 12, borderRadius: 8, border: '1px solid #E1E6EA', backgroundColor: '#FAFBFC' }}>
                            <div style={{ color: '#0A0F14', fontWeight: 600, fontSize: 13, marginBottom: 4 }}>
                              {s.description || s.suggested?.ruleType || 'Rule suggestion'}
                            </div>
                            {s.suggested?.fieldName && (
                              <div style={{ color: '#6E7A82', fontSize: 11, marginBottom: 2 }}>Field: {s.suggested.fieldName}</div>
                            )}
                            {s.suggested?.ruleType && (
                              <div style={{ color: '#6E7A82', fontSize: 11, marginBottom: 2 }}>Type: {s.suggested.ruleType}</div>
                            )}
                            {s.suggested?.severity && (
                              <div style={{ color: '#6E7A82', fontSize: 11, marginBottom: 6 }}>Severity: {s.suggested.severity}</div>
                            )}
                            {s.confidence != null && (
                              <div style={{ display: 'flex', alignItems: 'center', gap: 4, marginBottom: 8 }}>
                                <div style={{ flex: 1, height: 4, borderRadius: 2, backgroundColor: '#E1E6EA' }}>
                                  <div style={{ height: '100%', borderRadius: 2, backgroundColor: '#00A9B7', width: `${(s.confidence * 100)}%` }} />
                                </div>
                                <span style={{ fontSize: 10, color: '#6E7A82' }}>{Math.round(s.confidence * 100)}%</span>
                              </div>
                            )}
                            <button
                              onClick={() => {
                                setExternalRuleToAdd({
                                  ...
                                  rule: {
                                    ...
                                    severity: s.suggested?.severity === 'Error' ? 'Block' : s.suggested?.severity === 'Warning' ? 'Warning' : 'Info',
                                  },
                                });
                              }}
                              style={{ width: '100%', padding: '6px 0', borderRadius: 6, backgroundColor: '#007D87', color: '#FFFFFF', fontSize: 12, fontWeight: 600, border: 'none', cursor: 'pointer' }}
                            >
                              Apply Rule
                            </button>
                          </div>
                      ...
                      <div style={{ color: '#9AA6AF', fontSize: 12, textAlign: 'center' as const, padding: '16px 0' }}>No suggestions yet. Select a field.</div>
                    ) : null}
                  </div>
                )}
                ...
                {aiTab === 'generate' && !aiLoading && (
                  <div>
                    <textarea
                      placeholder={selectedFieldAI ? `Describe validation rules for "${selectedFieldAI.name}" in plain English...` : 'Select a field first, then describe your rules...'}
                      value={generatePrompt}
                      onChange={(e) => setGeneratePrompt(e.target.value)}
                      rows={4}
                      disabled={!selectedFieldAI}
                      style={{ width: '100%', padding: '10px 12px', borderRadius: 8, border: '1px solid #E1E6EA', fontSize: 13, resize: 'none' as const, marginBottom: 10, fontFamily: 'inherit', boxSizing: 'border-box' as const }}
                    />
                    <button
                      ...
                      style={{
                        ...
                        backgroundColor: !selectedFieldAI || !generatePrompt.trim() ? '#E1E6EA' : '#007D87',
                        color: !selectedFieldAI || !generatePrompt.trim() ? '#9AA6AF' : '#FFFFFF',
                        fontSize: 13, fontWeight: 600, border: 'none',
                        cursor: !selectedFieldAI || !generatePrompt.trim() ? 'not-allowed' : 'pointer', marginBottom: 16,
                      }}
                    >
                    ...
                    {aiGeneratedRules.length > 0 && (
                      <div style={{ display: 'flex', flexDirection: 'column' as const, gap: 10 }}>
                        {aiGeneratedRules.map((r: any, idx: number) => (
                          <div key={idx} style={{ padding: 12, borderRadius: 8, border: '1px solid #D1FAE5', backgroundColor: '#F0FDF4' }}>
                            <div style={{ color: '#0A0F14', fontWeight: 600, fontSize: 13, marginBottom: 4 }}>
                              {r.description || r.suggested?.ruleType || 'Generated rule'}
                            </div>
                            {r.suggested?.ruleType && (
                              <div style={{ color: '#6E7A82', fontSize: 11, marginBottom: 6 }}>Type: {r.suggested.ruleType} | Severity: {r.suggested.severity || 'Error'}</div>
                            )}
                            <div style={{ display: 'flex', gap: 6 }}>
                              <button
                                onClick={() => {
                                  setExternalRuleToAdd({
                                    fieldName: r.suggested?.fieldName || selectedFieldAI?.name || '',
                                    rule: {
                                      type: r.suggested?.ruleType || 'Custom',
                                      config: r.description || r.suggested?.ruleType || '',
                                      severity: r.suggested?.severity === 'Error' ? 'Block' : r.suggested?.severity === 'Warning' ? 'Warning' : 'Info',
                                    },
                                  });
                                  setAiGeneratedRules(prev => prev.filter((_, i) => i !== idx));
                                }}
                                style={{ flex: 1, padding: '6px 0', borderRadius: 6, backgroundColor: '#10B981', color: '#FFFFFF', fontSize: 11, fontWeight: 600, border: 'none', cursor: 'pointer' }}
                              >
                                Accept
                              </button>
                              <button
                                onClick={() => setAiGeneratedRules(prev => prev.filter((_, i) => i !== idx))}
                                style={{ flex: 1, padding: '6px 0', borderRadius: 6, backgroundColor: '#FFFFFF', color: '#EF4444', fontSize: 11, fontWeight: 600, border: '1px solid #EF4444', cursor: 'pointer' }}
                              >
                                Reject
                              </button>
                            </div>
                ...
                {aiTab === 'review' && !aiLoading && (
                  <div>
                    <button
                      ...
                      style={{ width: '100%', padding: '10px 0', borderRadius: 8, backgroundColor: '#007D87', color: '#FFFFFF', fontSize: 13, fontWeight: 600, border: 'none', cursor: 'pointer', marginBottom: 16 }}
                    >
                      Review All Rules
                    </button>
                    ...
                    {aiReview && (
                      <div style={{ display: 'flex', flexDirection: 'column' as const, gap: 12 }}>
                        {/* Predicted Accuracy */}
                        <div style={{ padding: 14, borderRadius: 8, backgroundColor: '#ECFDF5', border: '1px solid #10B981' }}>
                          <div style={{ color: '#065F46', fontSize: 11, textTransform: 'uppercase' as const, letterSpacing: '0.5px', marginBottom: 4 }}>Predicted Accuracy</div>
                          <div style={{ color: '#10B981', fontWeight: 700, fontSize: 28 }}>{aiReview.predicted_accuracy ?? '--'}%</div>
                        </div>

                        {/* Gaps */}
                        {aiReview.gaps && aiReview.gaps.length > 0 && (
                          <div>
                            <div style={{ color: '#6E7A82', fontSize: 11, textTransform: 'uppercase' as const, letterSpacing: '0.5px', marginBottom: 8, fontWeight: 600 }}>Gaps Detected</div>
                            <div style={{ display: 'flex', flexDirection: 'column' as const, gap: 8 }}>
                              {aiReview.gaps.map((gap: any, idx: number) => (
                                <div key={idx} style={{ padding: 10, borderRadius: 8, backgroundColor: '#FEF3C7', border: '1px solid #F59E0B' }}>
                                  <div style={{ display: 'flex', alignItems: 'start', gap: 6, marginBottom: 6 }}>
                                    <AlertTriangle size={14} style={{ color: '#F59E0B', flexShrink: 0, marginTop: 1 }} />
                                    <div style={{ flex: 1 }}>
                                      <div style={{ color: '#92400E', fontWeight: 600, fontSize: 12 }}>{gap.field}: {gap.missing_rule}</div>
                                      <div style={{ color: '#92400E', fontSize: 11 }}>{gap.recommendation || gap.reason || ''}</div>
                                      {gap.severity && <div style={{ color: '#78350F', fontSize: 10, marginTop: 2 }}>Severity: {gap.severity}</div>}
                                    </div>
                                  </div>
                                  <button
                                    onClick={() => {
                                      ...
                                      if (!fieldName) {
                                        window.alert('Cannot apply: no field specified in this gap. Please add the rule manually in Step 2 (Field Logic).');
                                        return;
                                      }
                                      setExternalRuleToAdd({
                                        fieldName,
                                        rule: {
                                          type: 'Custom',
                                          config: gap.missing_rule || gap.recommendation || gap.description || '',
                                          severity: gap.severity === 'High' ? 'Block' : gap.severity === 'Medium' ? 'Warning' : 'Info',
                                        },
                                      });
                        ...
                        {aiReview.recommendations && aiReview.recommendations.length > 0 && (
                          <div>
                            <div style={{ color: '#6E7A82', fontSize: 11, textTransform: 'uppercase' as const, letterSpacing: '0.5px', marginBottom: 8, fontWeight: 600 }}>Recommendations</div>
                            <div style={{ display: 'flex', flexDirection: 'column' as const, gap: 4 }}>
                              {aiReview.recommendations.map((rec: string, idx: number) => (
                                <div key={idx} style={{ padding: '8px 10px', borderRadius: 6, backgroundColor: '#F0F9FF', border: '1px solid #BAE6FD', fontSize: 12, color: '#0C4A6E' }}>
                                  {rec}
                                </div>
                              ))}
                            </div>
                        ...
                        {aiReview.fraud_risks && aiReview.fraud_risks.length > 0 && (
                          <div>
                            <div style={{ color: '#6E7A82', fontSize: 11, textTransform: 'uppercase' as const, letterSpacing: '0.5px', marginBottom: 8, fontWeight: 600 }}>Fraud Risks</div>
                            <div style={{ display: 'flex', flexDirection: 'column' as const, gap: 4 }}>
                              {aiReview.fraud_risks.map((risk: string, idx: number) => (
                                <div key={idx} style={{ padding: '8px 10px', borderRadius: 6, backgroundColor: '#FEF2F2', border: '1px solid #FECACA', fontSize: 12, color: '#991B1B' }}>
                                  {risk}
                                </div>
                              ))}
                            </div>
        ...
        >
            ...
            <button
              ...
              onClick={async () => {
                try {
                  ...
                  if (isEdit && agentId) {
                    await mysqlApiRequest(`/agents/${agentId}`, { method: 'PUT', body: JSON.stringify(payload) });
                  } else {
                    const res = await mysqlApiRequest<{ success: boolean; data?: { id: string }; id?: string }>('/agents', { method: 'POST', body: JSON.stringify(payload) });
                    const newId = res.data?.id || res.id;
                    if (newId) { savedIdRef.current = newId; navigate(`/agent-configurator/edit/${newId}`, { replace: true }); }
                  }
                  setAutoSaved(true);
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs" style={{ color: '#6E7A82' }}>Auto-saved</span>
          </div>
