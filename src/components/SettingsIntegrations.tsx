...
import { Sparkles, ShieldCheck, Mail, Send, Lock, Eye, EyeOff, Save, CheckCircle2 } from 'lucide-react';
...
const GROUPS: { key: SettingRow['group']; title: string; subtitle: string; icon: ReactNode }[] = [
  { key: 'ai', title: 'AI & OCR', subtitle: 'Google Gemini is the only supported provider.', icon: <Sparkles style={{ width: 20, height: 20, color: 'var(--color-teal)' }} /> },
  { key: 'kyc', title: 'KYC (Ongrid Gridlines)', subtitle: 'PAN, GSTIN, penny-drop, Udyam verification.', icon: <ShieldCheck style={{ width: 20, height: 20, color: 'var(--color-teal)' }} /> },
  { key: 'imap', title: 'Email Ingestion (IMAP)', subtitle: 'Inbound invoice mailbox polled by the poller.', icon: <Mail style={{ width: 20, height: 20, color: 'var(--color-teal)' }} /> },
  { key: 'smtp', title: 'Outbound Email (SMTP)', subtitle: 'Used for vendor invitations and transactional email.', icon: <Send style={{ width: 20, height: 20, color: 'var(--color-teal)' }} /> },
  { key: 'security', title: 'Security & CORS', subtitle: 'API secret key and allowed origins.', icon: <Lock style={{ width: 20, height: 20, color: 'var(--color-teal)' }} /> },
];

const inputStyle: CSSProperties = {
  width: '100%', padding: '9px 12px', fontSize: 13,
  border: '1.5px solid var(--color-silver)', borderRadius: 8,
  outline: 'none', color: 'var(--color-ink)', backgroundColor: '#fff',
};
  ...
  const saveGroup = async (group: string) => {
    ...
    try {
      for (const r of toSave) {
        ...
        const json = await res.json();
        if (!json.success) throw new Error(`Failed to save ${r.key}: ${json.error || 'unknown error'}`);
      }
      setDrafts((prev) => {
        const next = { ...prev };
        ...
        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
          <div style={{ flex: 1, position: 'relative' }}>
            <input
              ...
              value={isDirty ? drafts[row.key] : (revealed[row.key] ? (row.value || '') : (row.value || ''))}
              placeholder={row.hasValue ? '(stored — start typing to replace)' : (row.default || 'Enter value')}
              onChange={(e) => setDraft(row.key, e.target.value)}
              style={inputStyle}
            />
          </div>
          <button
            ...
            style={{
              padding: '7px 10px', borderRadius: 6, border: '1px solid var(--color-silver)',
              background: '#fff', cursor: 'pointer', color: 'var(--color-mercury-grey)',
            }}
            title={revealed[row.key] ? 'Hide' : 'Reveal'}
          >
            {revealed[row.key] ? <EyeOff style={{ width: 14, height: 14 }} /> : <Eye style={{ width: 14, height: 14 }} />}
          </button>
        </div>
      );
    }
          ...
          return (
            <FormSection
              ...
              action={
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  {justFlashed && (
                    <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4, fontSize: 12, color: '#007D60' }}>
            ...
            >
              {groupRows.map((row) => (
                ...
                >
                  {renderField(row)}
                  <div style={{ display: 'flex', gap: 8, marginTop: 4, fontSize: 11, color: 'var(--color-mercury-grey)' }}>
                    <span>
                      Source: <strong style={{
                        color: row.source === 'database' ? '#007D60' : row.source === 'environment' ? '#92400E' : '#DC2626',
                      }}>{row.source}</strong>
                    </span>
                    {row.updatedAt && (
