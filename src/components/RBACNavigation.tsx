  ...
  const renderNavItem = (module: NavModule, level: number = 0) => {
    ...
    if (hasChildren && !isCollapsed) {
      ...
      const accessibleChildren =
        module.children?.filter((child) => hasAccess(child.path) !== 'none') || [];
            ...
            <div key={section.id} style={{ borderBottom: '1px solid #3a4a52' }}>
              ...
              {isExpanded && (
                <div className="py-2">
                  {/* Core Modules */}
                  {accessibleModules.length > 0 && (
                    <div>{accessibleModules.map((module) => renderNavItem(module))}</div>
                  )}
                  ...
                  {accessibleMasters.length > 0 && (
                    <>
                      ...
                      <div>{accessibleMasters.map((module) => renderNavItem(module))}</div>
                    </>
                  )}
                  ...
                  {accessibleReports.length > 0 && (
                    <>
                      ...
                      <div>{accessibleReports.map((module) => renderNavItem(module))}</div>
                    </>
                  )}
                </div>
              )}
