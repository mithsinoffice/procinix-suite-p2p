...
export function BudgetConsumptionControl() {
  ...
  const kpis = filteredBudgets.reduce(
    (acc, b) => ({
  ...
  const chartData = departments.map((dept) => {
    const deptBudgets = budgets.filter(
      (b) => b.dimensions.department === dept && b.status === 'Approved'
    );
    return {
      name: dept,
  ...
  const getControlStatus = (budget: any) => {
    ...
    if (utilization >= 95)
      return { label: 'Hard Stop', color: 'bg-red-100 text-red-700', icon: Shield };
    if (utilization >= 85)
      return { label: 'Soft Warning', color: 'bg-yellow-100 text-yellow-700', icon: AlertTriangle };
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-[var(--color-mercury-grey)] text-sm">
                Real-time budget tracking with procurement control enforcement
              </p>
            </div>
        ...
        <div className="grid grid-cols-5 gap-4 mb-6">
          <div className="bg-white rounded-lg border border-[var(--color-silver)] p-4">
            ...
            <div className="text-2xl text-[var(--color-ink)]">
              ₹{(kpis.originalBudget / 1000000).toFixed(1)}M
            </div>
          </div>
          ...
          <div className="bg-white rounded-lg border border-[var(--color-silver)] p-4">
            ...
            <div className="text-2xl text-[var(--color-ink)]">
              ₹{(kpis.committed / 1000000).toFixed(1)}M
            </div>
            <p className="text-xs text-[var(--color-mercury-grey)] mt-1">
              {((kpis.committed / kpis.originalBudget) * 100).toFixed(1)}%
            </p>
          </div>
          ...
          <div className="bg-white rounded-lg border border-[var(--color-silver)] p-4">
            ...
            <div className="text-2xl text-[var(--color-ink)]">
              ₹{(kpis.actual / 1000000).toFixed(1)}M
            </div>
            <p className="text-xs text-[var(--color-mercury-grey)] mt-1">
              {((kpis.actual / kpis.originalBudget) * 100).toFixed(1)}%
            </p>
          </div>
          ...
          <div className="bg-white rounded-lg border border-[var(--color-silver)] p-4">
            ...
            <p className="text-xs text-[var(--color-mercury-grey)] mt-1">
              {((kpis.available / kpis.originalBudget) * 100).toFixed(1)}%
            </p>
          </div>
        ...
        <div className="grid grid-cols-2 gap-6 mb-6">
          ...
          <div className="bg-white rounded-lg border border-[var(--color-silver)] p-6">
            <h2 className="text-[var(--color-ink)] mb-4">
              Budget vs Committed vs Actual by Department
            </h2>
                    ...
                    <tr key={budget.id} className="hover:bg-[var(--color-cloud)]">
                      <td className="px-4 py-3">
                        <div>
                          ...
                          <div className="text-xs text-[var(--color-mercury-grey)] mt-1">
                            {budget.budgetNumber} • {budget.dimensions.department}
                          </div>
                        </div>
                      </td>
                      ...
                      <td className="px-4 py-3 text-center">
                        <div className="flex items-center justify-center gap-2">
                          ...
                          <span
                            className={`text-sm ${getUtilizationColor(budget.utilizationPercent)}`}
                          >
                      ...
                      <td className="px-4 py-3 text-center">
                        <span
                          className={`inline-flex items-center gap-1 px-2 py-1 rounded text-xs ${controlStatus.color}`}
                        >
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <div className="text-sm text-blue-700 space-y-1">
                <p>
                  • <strong>Hard Stop (≥95%):</strong> PO creation blocked automatically
                </p>
                <p>
                  • <strong>Soft Warning (≥85%):</strong> Warning shown with override option for
                  authorized users
                </p>
                <p>
                  • <strong>Normal (&lt;85%):</strong> POs can be created without restrictions
                </p>
                <p>
                  • Budget is committed at PO creation and updated to actual upon invoice posting
                </p>
              </div>
            </div>
          </div>
