...
export function ProfitCentreMaster() {
  ...
  const [profitCentres, setProfitCentres] = useIncrementalMasterRecords<ProfitCentre>(
    'profit_centre_master',
    [
      {
        id: '1',
        profitCentreCode: 'PC-NORTH-001',
        profitCentreName: 'North Region Sales',
        department: 'Sales',
        headOfPC: 'Rahul Mehta',
      ...
      {
        id: '2',
        profitCentreCode: 'PC-SOUTH-001',
        profitCentreName: 'South Region Sales',
        department: 'Sales',
        headOfPC: 'Lakshmi Iyer',
      ...
      {
        id: '3',
        profitCentreCode: 'PC-EAST-001',
        profitCentreName: 'East Region Operations',
      ...
      {
        id: '4',
        profitCentreCode: 'PC-WEST-001',
        profitCentreName: 'West Region Manufacturing',
      ...
      {
        id: '5',
        profitCentreCode: 'PC-CORP-001',
        profitCentreName: 'Corporate Services',
        department: 'Finance',
        headOfPC: 'Sandeep Kumar',
  ...
  const filteredProfitCentres = useMemo(() => {
    return profitCentres.filter((pc) => {
      ...
      const matchesApproval =
        approvalFilter.length === 0 || approvalFilter.includes(pc.approvalStatus);
  ...
  const regions = ['North', 'South', 'East', 'West', 'Central', 'Corporate'];
  ...
  const handleReview = (profitCentre: ProfitCentre) => {
    ...
    if (profitCentre.originalData) {
      ...
      if (orig.profitCentreCode !== profitCentre.profitCentreCode)
        changes.push({
          field: 'Profit Centre Code',
          oldValue: orig.profitCentreCode,
          newValue: profitCentre.profitCentreCode,
        });
      if (orig.profitCentreName !== profitCentre.profitCentreName)
        changes.push({
          field: 'Profit Centre Name',
          oldValue: orig.profitCentreName,
          newValue: profitCentre.profitCentreName,
        });
      if (orig.department !== profitCentre.department)
        changes.push({
          field: 'Department',
          oldValue: orig.department,
          newValue: profitCentre.department,
        });
      if (orig.headOfPC !== profitCentre.headOfPC)
        changes.push({
          field: 'Head of PC',
          oldValue: orig.headOfPC,
          newValue: profitCentre.headOfPC,
        });
      if (orig.revenueTarget !== profitCentre.revenueTarget)
        changes.push({
          field: 'Revenue Target',
          oldValue: `₹${orig.revenueTarget.toLocaleString('en-IN')}`,
          newValue: `₹${profitCentre.revenueTarget.toLocaleString('en-IN')}`,
        });
      if (orig.region !== profitCentre.region)
        changes.push({ field: 'Region', oldValue: orig.region, newValue: profitCentre.region });
      if (orig.status !== profitCentre.status)
        changes.push({ field: 'Status', oldValue: orig.status, newValue: profitCentre.status });
    }
      ...
      >
        <FormSection title="Profit Centre Details" columns={2}>
          <PxFormField
            ...
            required
            filled={!!profitCentreCode.trim()}
            hint="Unique identifier for the profit centre"
          >
            <input
              type="text"
          ...
          <PxFormField label="Head of PC" required filled={!!headOfPC.trim()}>
            <input
              ...
              placeholder="Head of profit centre"
              className="px-input"
            />
          </PxFormField>
          <PxFormField
            ...
            required
            filled={!!revenueTarget.trim()}
            hint="Annual revenue target in INR"
          >
            <input
              type="number"
              value={revenueTarget}
    ...
    <MasterPageShell masterName="Profit Centre Master" description="Manage profit centres">
      ...
      >
        <table className="w-full">
          <thead style={{ backgroundColor: 'var(--color-cloud)' }}>
            <tr>
              <th
                className="text-left px-6 py-4 text-xs"
                style={{
              ...
              <th
                className="text-center px-6 py-4 text-xs"
                style={{
                  color: 'var(--color-mercury-grey)',
                  fontWeight: '600',
                  textTransform: 'uppercase',
                  ...
                  <div className="flex items-center justify-center gap-2">
                    {profitCentre.approvalStatus === 'Pending Approval' && (
                      <button
                        ...
                        onMouseEnter={(e) =>
                          (e.currentTarget.style.backgroundColor = 'var(--color-teal)20')
                        }
                        onMouseLeave={(e) =>
                          (e.currentTarget.style.backgroundColor = 'var(--color-teal)10')
                        }
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label
                    ...
                    style={{ color: 'var(--color-mercury-grey)' }}
                  >
                    Profit Centre Code <span style={{ color: 'var(--color-error)' }}>*</span>
                  </label>
                  <input
                    type="text"
                    value={profitCentreCode}
                ...
                <div>
                  <label
                    ...
                    style={{ color: 'var(--color-mercury-grey)' }}
                  >
                    Profit Centre Name <span style={{ color: 'var(--color-error)' }}>*</span>
                  </label>
                  <input
                    type="text"
                    value={profitCentreName}
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label
                    ...
                    style={{ color: 'var(--color-mercury-grey)' }}
                  >
                    Head of PC <span style={{ color: 'var(--color-error)' }}>*</span>
                  </label>
                  <input
                    type="text"
                    value={headOfPC}
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label
                    ...
                    style={{ color: 'var(--color-mercury-grey)' }}
                  >
                    Revenue Target (₹) <span style={{ color: 'var(--color-error)' }}>*</span>
                  </label>
                  <input
                    type="number"
                    value={revenueTarget}
      ...
      {showApprovalModal && currentReviewRecord && (
        <ApprovalModal
          ...
          onApprove={(comments) => {
            void handleApprovalAction('approve', comments);
          }}
          onReject={(comments) => {
            void handleApprovalAction('reject', comments);
          }}
          onMoreInfo={(comments) => {
            void handleApprovalAction('moreinfo', comments);
          }}
        />
      )}
    </MasterPageShell>
