...
export function LocationMaster() {
  ...
  const [records, setRecords] = useIncrementalMasterRecords<Location>('location_master', [
    ...
    {
      id: '2',
      locationCode: 'BLR-ST-01',
      locationName: 'Bangalore Store',
      locationType: 'Store',
    ...
    {
      id: '3',
      locationCode: 'PUN-ST-01',
      locationName: 'Pune Store',
      locationType: 'Store',
      ...
      contactPhone: '9881012345',
      status: 'Active',
      approvalStatus: 'Pending Approval',
    },
    {
      id: '4',
      locationCode: 'DEL-HUB-01',
      locationName: 'Delhi Hub',
      locationType: 'Warehouse',
      address: 'Sector 18, Okhla Industrial Estate',
      city: 'New Delhi',
      state: 'Delhi',
      pincode: '110020',
      parentEntity: 'Subko Coffee',
  ...
  const filteredRecords = useMemo(() => {
    return records.filter((r) => {
      const haystack = [r.locationCode, r.locationName, r.locationType, r.city, r.state]
        .join(' ')
        .toLowerCase();
  ...
  const handleEdit = (rec: Location) => {
    setIsEditMode(true);
    setEditingId(rec.id);
    setLocationCode(rec.locationCode || '');
    setLocationName(rec.locationName || '');
    setLocationType(rec.locationType || 'Warehouse');
    setAddress(rec.address || '');
    setCity(rec.city || '');
    setState(rec.state || '');
    setPincode(rec.pincode || '');
    setParentEntity(rec.parentEntity || '');
  ...
  const handleReview = (rec: Location) => {
    ...
    if (rec.originalData) {
      ...
      if (o.locationCode !== rec.locationCode)
        changes.push({
          field: 'Location Code',
          oldValue: o.locationCode,
          newValue: rec.locationCode,
        });
      if (o.locationName !== rec.locationName)
        changes.push({
          field: 'Location Name',
          oldValue: o.locationName,
          newValue: rec.locationName,
        });
      if (o.locationType !== rec.locationType)
        changes.push({
          field: 'Location Type',
          oldValue: o.locationType,
          newValue: rec.locationType,
        });
      if (o.city !== rec.city)
        changes.push({ field: 'City', oldValue: o.city, newValue: rec.city });
      if (o.state !== rec.state)
        changes.push({ field: 'State', oldValue: o.state, newValue: rec.state });
  ...
  return (
    <MasterPageShell
      masterName="Location Master"
      description="Manage warehouses, stores, offices and factory locations"
    >
