...
export function ARMasters() {
  const masters = [
    { name: 'Customer Master', count: 124, icon: Users, route: '/ar/masters/customers', color: 'var(--color-teal)' },
    { name: 'Item / Service Master', count: 856, icon: FileText, route: '/ar/masters/items', color: 'var(--color-success-dark)' },
    { name: 'Tax Master (GST)', count: 12, icon: DollarSign, route: '/ar/masters/tax', color: 'var(--color-warning-dark)' },
    { name: 'Payment Terms', count: 8, icon: Calendar, route: '/ar/masters/payment-terms', color: '#7B1FA2' },
    { name: 'Reason Codes', count: 15, icon: Tag, route: '/ar/masters/reason-codes', color: 'var(--color-error-dark)' },
    ...
    <div style={{ backgroundColor: 'var(--color-cloud)', minHeight: '100vh' }}>
      <div className="bg-white px-8 py-6" style={{ borderBottom: '1px solid var(--color-silver)' }}>
        <div className="flex items-center justify-between mb-4">
          <div>
            <h1 className="text-2xl mb-2" style={{ color: 'var(--color-ink)', margin: 0 }}>AR Masters</h1>
      ...
      <div className="p-8">
        <div className="grid grid-cols-3 gap-6">
          {masters.map((master, idx) => {
            ...
            return (
              <div key={idx} className="bg-white rounded-lg p-6 cursor-pointer hover:shadow-lg transition-shadow" style={{ border: '1px solid var(--color-silver)' }}>
                <div className="flex items-start justify-between mb-4">
                  ...
                  <span className="text-2xl" style={{ color: 'var(--color-ink)', fontWeight: '600' }}>{master.count}</span>
                </div>
                <h3 className="text-base mb-2" style={{ color: 'var(--color-ink)', fontWeight: '600', margin: 0 }}>{master.name}</h3>
                <p className="text-sm" style={{ color: 'var(--color-mercury-grey)' }}>Manage {master.name.toLowerCase()} data</p>
              </div>
            );
          })}
        </div>
