...
export function AdvancesHub() {
  ...
  const stats = {
    ...
    readyForPayment: advanceRequests.filter(
      (r) =>
        r.status === 'Approved' && (r.paymentStatus === 'Pending' || r.paymentStatus === 'In Queue')
    ).length,
  ...
  const features = [
    ...
    },
    {
      ...
      stats: [{ label: 'Open Balances', value: stats.utilizationOpen }],
    },
  ];
        ...
        <div className="px-6 py-4">
          <div>
            ...
            <p className="text-[var(--color-mercury-grey)] text-sm">
              Comprehensive vendor advance request and payment management
            </p>
          </div>
        </div>
      </div>
                  ...
                  <div className="flex items-center gap-6">
                    {module.stats.map((stat, idx) => (
                      <div key={idx}>
                        ...
                        <div className={`${module.textColor}`}>{stat.value}</div>
                      </div>
                    ))}
                  </div>
                </div>
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-[var(--color-mercury-grey)] mb-3">
                The Advance Management module enables comprehensive handling of vendor advances with
              ...
              <ul className="space-y-2 text-sm text-[var(--color-mercury-grey)]">
                <li className="flex items-start gap-2">
                  ...
                  <span>
                    <strong>PO-based Advances:</strong> Request advances against specific PO
                    milestones with automatic eligibility validation
                  </span>
                </li>
                <li className="flex items-start gap-2">
                  <CheckCircle className="w-4 h-4 text-[var(--color-teal)] mt-0.5 flex-shrink-0" />
                  <span>
                    <strong>On-Account Advances:</strong> Raise general vendor advances without PO
                    linkage
                  </span>
                </li>
                <li className="flex items-start gap-2">
                  <CheckCircle className="w-4 h-4 text-[var(--color-teal)] mt-0.5 flex-shrink-0" />
                  <span>
                    <strong>TDS Calculation:</strong> Automatic TDS computation with section-wise
                    rate application
                  </span>
                </li>
                <li className="flex items-start gap-2">
                  <CheckCircle className="w-4 h-4 text-[var(--color-teal)] mt-0.5 flex-shrink-0" />
                  <span>
                    <strong>Approval Workflow:</strong> Configurable approval chains with SLA
                ...
                <li className="flex items-start gap-2">
                  <CheckCircle className="w-4 h-4 text-[var(--color-teal)] mt-0.5 flex-shrink-0" />
                  <span>
                    <strong>Payment Integration:</strong> Seamless flow into payment processing with
                ...
                <li className="flex items-start gap-2">
                  <CheckCircle className="w-4 h-4 text-[var(--color-teal)] mt-0.5 flex-shrink-0" />
                  <span>
                    <strong>Utilization Tracking:</strong> Real-time tracking of advance adjustments
                    against invoices with audit trail
                  </span>
                </li>
              </ul>
            </div>
