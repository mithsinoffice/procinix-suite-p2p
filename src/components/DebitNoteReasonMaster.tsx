...
export function DebitNoteReasonMaster() {
  ...
  const [reasons, setReasons] = useIncrementalMasterRecords<DebitNoteReason>(
  ...
  const filteredReasons = useMemo(() => {
    return reasons.filter((r) => {
      ...
      const matchesApproval =
        approvalFilter.length === 0 || approvalFilter.includes(r.approvalStatus ?? 'Approved');
  ...
  const handleReview = (reason: DebitNoteReason) => {
    ...
    if (original) {
      if (original.code !== reason.code)
        changes.push({ field: 'Reason Code', oldValue: original.code, newValue: reason.code });
      if (original.name !== reason.name)
        changes.push({ field: 'Reason Name', oldValue: original.name, newValue: reason.name });
      if (original.description !== reason.description)
        changes.push({
          field: 'Description',
          oldValue: original.description,
          newValue: reason.description,
        });
      if (original.status !== reason.status)
        changes.push({ field: 'Status', oldValue: original.status, newValue: reason.status });
    }
  ...
  if (showForm) {
    return (
      <FormShell
        masterName="Debit Note Reason Master"
      ...
      >
        <FormSection title="Debit Note Reason Details" columns={2}>
          <PxFormField
            ...
            required
            filled={!!code.trim()}
            hint="Uppercase code (e.g., DNR-SHORT)"
          >
            <input
              type="text"
              value={code}
              onChange={(e) => setCode(e.target.value.toUpperCase())}
              placeholder="e.g., DNR-SHORT"
              className="px-input"
            />
          </PxFormField>
  ...
  return (
    <MasterPageShell
      masterName="Debit Note Reason Master"
      description="Manage debit note reason codes"
    >
              ...
              <div className="grid grid-cols-2 gap-6">
                <div>
                  <label
                    ...
                    style={{ color: 'var(--color-mercury-grey)' }}
                  >
                    Reason Code <span style={{ color: 'var(--color-error)' }}>*</span>
                  </label>
                ...
                <div>
                  <label
                    ...
                    style={{ color: 'var(--color-mercury-grey)' }}
                  >
                    Reason Name <span style={{ color: 'var(--color-error)' }}>*</span>
                  </label>
                  <div className="relative">
                    <Type
                      className="absolute left-3 top-1/2 transform -translate-y-1/2 w-4 h-4"
                ...
                >
                  ...
                  <td className="px-6 py-4">
                    <span
                      ...
                      style={{
                        backgroundColor:
                          reason.status === 'Active' ? 'var(--color-teal-tint)' : '#E5E7EB',
                        color:
                  ...
                  <td className="px-6 py-4">
                    <span
                      className="px-3 py-1 rounded-full text-sm"
                      style={getApprovalBadgeStyle(reason.approvalStatus)}
                    >
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-2">
                      ...
                      {(reason.approvalStatus === 'Pending Approval' ||
                        reason.approvalStatus === 'Changes Requested' ||
