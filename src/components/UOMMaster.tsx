...
export function UOMMaster() {
  ...
  const [uoms, setUOMs] = useIncrementalMasterRecords<UOM>('uom_master', [
    ...
    },
    {
      ...
      code: 'BOX',
      name: 'Box',
      description: 'Standard box or carton',
      status: 'Active',
      approvalStatus: 'Approved',
    },
  ]);
  ...
  const filteredUOMs = useMemo(() => {
    return uoms.filter((uom) => {
      ...
      const matchesApproval =
        approvalFilter.length === 0 || approvalFilter.includes(uom.approvalStatus ?? 'Approved');
      return matchesSearch && matchesStatus && matchesApproval;
    });
  }, [uoms, searchTerm, statusFilter, approvalFilter]);

  const handleSubmit = (
    approvalStatus: NonNullable<UOM['approvalStatus']> = 'Pending Approval'
  ) => {
  ...
  const handleReview = (uom: UOM) => {
    ...
    if (original) {
      if (original.code !== uom.code)
        changes.push({ field: 'UOM Code', oldValue: original.code, newValue: uom.code });
      if (original.name !== uom.name)
        changes.push({ field: 'UOM Name', oldValue: original.name, newValue: uom.name });
      if (original.description !== uom.description)
        changes.push({
          field: 'Description',
          oldValue: original.description,
          newValue: uom.description,
        });
      if (original.status !== uom.status)
        changes.push({ field: 'Status', oldValue: original.status, newValue: uom.status });
    }
  ...
  if (showForm) {
    return (
      <FormShell
        masterName="Unit of Measure Master"
              ...
              <div className="grid grid-cols-2 gap-6">
                <div>
                  <label
                    ...
                    style={{ color: 'var(--color-mercury-grey)' }}
                  >
                    UOM Code <span style={{ color: 'var(--color-error)' }}>*</span>
                  </label>
                ...
                <div>
                  <label
                    ...
                    style={{ color: 'var(--color-mercury-grey)' }}
                  >
                    UOM Name <span style={{ color: 'var(--color-error)' }}>*</span>
                  </label>
                  <div className="relative">
                    <Type
                      className="absolute left-3 top-1/2 transform -translate-y-1/2 w-4 h-4"
                ...
                >
                  ...
                  <td className="px-6 py-4">
                    <span
                      ...
                      style={{
                        backgroundColor:
                          uom.status === 'Active' ? 'var(--color-teal-tint)' : '#E5E7EB',
                        color:
                          uom.status === 'Active'
                            ? 'var(--color-teal)'
                  ...
                  <td className="px-6 py-4">
                    <span
                      className="px-3 py-1 rounded-full text-sm"
                      style={getApprovalBadgeStyle(uom.approvalStatus)}
                    >
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-2">
                      ...
                      {(uom.approvalStatus === 'Pending Approval' ||
                        uom.approvalStatus === 'Changes Requested' ||
                        uom.approvalStatus === 'Draft') && (
                        <button
                          onClick={() => handleReview(uom)}
                          className="p-2 rounded-lg transition-colors"
