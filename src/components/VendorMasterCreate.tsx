...
function duplicateVendorErrorMessage(
  ...
  const duplicateName = duplicate.legalName || duplicate.name || duplicate.vendor_legal_name || 'existing vendor';
...
const SPOC_COLUMNS: SimpleColumn[] = [
  ...
  { key: 'state', label: 'State', type: 'select', options: INDIAN_STATES.map(s => ({ value: s, label: s })) },
...
const BANK_COLUMNS: SimpleColumn[] = [
  ...
  { key: 'account_number', label: 'Account Number', type: 'text', required: true, placeholder: 'Account number' },
  { key: 'ifsc_code', label: 'IFSC', type: 'text', required: true, placeholder: 'HDFC0001234', width: '130px' },
...
const emptySpoc = () => ({
  spoc_name: '', designation: '', email: '', phone: '',
  is_primary: false, location_label: '', city: '', state: '', pin_code: '',
});

const emptyGst = () => ({
  gstin: '', gst_type: 'regular', gst_state_code: '', state: '',
  city: '', pin_code: '', address: '', spoc_id: '', status: 'active',
});

const emptyBank = () => ({
  account_number: '', ifsc_code: '', bank_name: '', branch_name: '',
  account_type: 'current', currency: 'INR', is_primary: false, status: 'active',
});

const emptyEntityMapping = () => ({
  entity_id: '', gl_code_expense: '', gl_code_expense_desc: '',
  gl_code_cogs: '', gl_code_cogs_desc: '', payment_terms: '',
  credit_days: '', credit_limit: '',
  cost_centre_id: '', profit_centre_id: '',
  block_for_po: false, block_for_po_reason: '',
  block_for_payment: false, block_for_payment_reason: '',
});
...
export function VendorMasterCreate() {
  ...
  const toMasterVendor = useCallback(
    (savedId?: string, effectiveStatus?: string): VendorMaster => {
      ...
      const resolvedVendorCode = vendor.vendor_code || savedId || vendorId || generateSystemVendorCode();
      ...
      return {
        ...
        vendorType: vendor.vendor_type === 'intercompany' || vendor.vendor_type === 'foreign' ? 'Import' : 'Domestic',
        ...
        msmeCategory: (panCompliance.msme_category
          ? `${panCompliance.msme_category}`.charAt(0).toUpperCase() + `${panCompliance.msme_category}`.slice(1)
        ...
        tdsApplicable: Array.isArray(panCompliance.tds_sections) ? panCompliance.tds_sections.length > 0 : false,
        tdsSection: Array.isArray(panCompliance.tds_sections) && panCompliance.tds_sections.length > 0 ? panCompliance.tds_sections[0] : undefined,
        section206ABApplicable: panCompliance.section_206ab === 'specified_person' || panCompliance.section_206ab === 'non_filer',
      };
    },
    [vendor, vendorId, panCompliance, spocs, gstRegistrations, entityMappings, bankAccounts, bankResults]
  );

  const verifyMsme = useCallback(async () => {
    const udyam = (panCompliance.msme_number || '').toUpperCase().trim();
    if (!udyam) { setMsmeError('Enter Udyam number first'); return; }
    if (!kycConsent) { setMsmeError('Tick the consent checkbox above first.'); return; }
    setMsmeLoading(true); setMsmeError('');
    try {
      const res = await fetch(`${API}/api/kyc/verify-msme`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ udyam_number: udyam, consent: 'Y', reason: 'Vendor onboarding KYC' }),
      });
      const data = await res.json();
      if (data.success) {
        setMsmeResult({ ...data.data, source: data.source, transaction_id: data.transaction_id, mock_mode: data.mock_mode });
    ...
    } catch { setMsmeError('KYC service unavailable'); }
    finally { setMsmeLoading(false); }
  }, [panCompliance.msme_number, panCompliance.msme_category, kycConsent]);

  const verifyBankRow = useCallback(async (idx: number) => {
    ...
    try {
      ...
      const data = await res.json();
      if (data.success) {
        setBankResults(prev => ({ ...prev, [idx]: { ...data.data, source: data.source, transaction_id: data.transaction_id, mock_mode: data.mock_mode } }));
        setBankAccounts(prev => prev.map((r, i) => i === idx ? {
          ...r,
  ...
  const handleSave = useCallback(
    async (status?: string) => {
      ...
      try {
        // Source of truth for /masters/vendor-master list
        const resolvedVendorCode = vendor.vendor_code || generateSystemVendorCode();
        ...
        const duplicateError = duplicateVendorErrorMessage(currentMasterRows, savedId, masterVendor);
        if (duplicateError) {
        ...
        const nextMasterRows =
          existingIdx >= 0
            ? currentMasterRows.map((row: any, idx: number) => (idx === existingIdx ? masterVendor : row))
            : [masterVendor, ...currentMasterRows];
        await saveRelationalMasterRecords('vendor_master', nextMasterRows);

        // Best-effort sync to vendor KYC APIs (non-blocking for master list UX)
        const payload = {
          ...vendor,
        ...
        try {
          ...
          const data = await res.json();
          if (effectiveStatus === 'pending_approval' && !isEdit && data?.success && data?.data?.id) {
            await fetch(`${API}/api/vendors/${data.data.id}/submit`, { method: 'POST' });
          }
        } catch {
          // Keep silent: master table save already succeeded.
        }
    ...
    [vendor, spocs, panCompliance, gstRegistrations, bankAccounts, entityMappings, isEdit, vendorId, navigate, toMasterVendor, addVendor, updateVendorMaster]
  );
  ...
  const renderSelect = (field: string, value: string, options: { value: string; label: string }[], onChange: (f: string, v: string) => void) => (
    <select
      value={value}
      onChange={(e) => onChange(field, e.target.value)}
      style={selectStyle}
    ...
    return (
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '60vh' }}>
        <div style={{ textAlign: 'center' }}>
          <div style={{ width: 40, height: 40, border: '3px solid var(--color-silver)', borderTopColor: 'var(--color-teal)', borderRadius: '50%', animation: 'spin 0.8s linear infinite', margin: '0 auto 12px' }} />
  ...
  return (
    <FormShell
      ...
      subtitle={isEdit ? `Editing ${vendor.vendor_legal_name || 'vendor'}` : 'Add a new vendor to the system'}
      headerExtra={kycVerified ? (
        <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4, padding: '4px 10px', borderRadius: 999, fontSize: 11, fontWeight: 600, backgroundColor: '#E6FBF5', color: '#007D60', border: '0.5px solid #9FE8D0' }}>
          &#10003; KYC Verified
        </span>
      ) : undefined}
    ...
    >
      ...
      >
        ...
        <PxFormField label="Legal Name" required>
          {renderInput('vendor_legal_name', 'Registered company name', vendor.vendor_legal_name, updateVendor)}
        </PxFormField>
        <PxFormField label="Trade Name">
          {renderInput('vendor_trade_name', 'Brand or trade name', vendor.vendor_trade_name, updateVendor)}
        </PxFormField>
        <PxFormField label="Vendor Group">
          {renderInput('vendor_group_name', 'Parent group name', vendor.vendor_group_name, updateVendor)}
        </PxFormField>
      ...
      <FormSection
        ...
        action={<AddRowButton label="Add Contact" onClick={() => setSpocs((prev) => [...prev, emptySpoc()])} />}
        flat
      >
        <SimpleInlineTable
      ...
      <FormSection title="Registered Address" subtitle="Primary registered address" columns={3} icon={<MapPin style={{ width: 20, height: 20, color: 'var(--color-teal)' }} />} flat>
        <PxFormField label="Address Line" colSpan={3}>
          <textarea
            ...
            onFocus={(e) => { (e.target as HTMLTextAreaElement).style.borderColor = 'var(--color-teal)'; }}
            onBlur={(e) => { (e.target as HTMLTextAreaElement).style.borderColor = 'var(--color-silver)'; }}
          />
        </PxFormField>
        ...
        <PxFormField label="State">
          {renderSelect('state', vendor.state, INDIAN_STATES.map((s) => ({ value: s, label: s })), updateVendor)}
        </PxFormField>
      ...
      <FormSection title="Tax & Compliance" subtitle="PAN, TDS, GST, and MSME details" columns={1} icon={<ShieldCheck style={{ width: 20, height: 20, color: 'var(--color-teal)' }} />} flat>
        {/* PAN sub-section */}
        <div style={{ marginBottom: 24 }}>
          <h3 style={{ fontSize: 14, fontWeight: 600, color: 'var(--color-ink)', marginBottom: 12 }}>PAN & Identity</h3>

          {/* Consent banner */}
          <label style={{
            display: 'flex', alignItems: 'flex-start', gap: 10,
            padding: '10px 12px', marginBottom: 12,
            borderRadius: 8, border: `1px solid ${kycConsent ? '#9FE8D0' : 'var(--color-silver)'}`,
            backgroundColor: kycConsent ? '#F0FBF7' : '#FAFBFC', cursor: 'pointer',
          }}>
            <input
              type="checkbox"
              checked={kycConsent}
            ...
            <span style={{ fontSize: 12, color: 'var(--color-ink)', lineHeight: 1.5 }}>
              I confirm that the vendor has provided consent to verify PAN, GSTIN, bank account and Udyam/MSME details via Ongrid Gridlines for onboarding KYC. Required by law before any verification call.
            </span>
          </label>
          ...
          <div className="grid grid-cols-3 gap-6">
            <PxFormField label="PAN Number">
              <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                ...
                <button
                  onClick={async () => {
                    if (!panCompliance.pan || panCompliance.pan.length !== 10) { setKycError('Enter valid 10-char PAN first'); return; }
                    if (!kycConsent) { setKycError('Vendor consent is required before running KYC checks.'); return; }
                    setKycLoading(true); setKycError('');
                    const pan = panCompliance.pan.toUpperCase();
                    const isCompany = pan.charAt(3) === 'C' || pan.charAt(3) === 'L';
                    const endpoint = isCompany ? '/api/kyc/verify-pan-comprehensive' : '/api/kyc/verify-pan';
                    try {
                      const res = await fetch(`${API}${endpoint}`, {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json' },
                        body: JSON.stringify({ pan, consent: 'Y', reason: 'Vendor onboarding KYC' }),
                      });
                      const data = await res.json();
                      if (data.success) {
                    ...
                    } catch { setKycError('KYC service unavailable'); }
                    finally { setKycLoading(false); }
                  }}
                  disabled={kycLoading || kycVerified || !kycConsent}
                  style={{
                    height: 30, padding: '0 12px', borderRadius: 6, fontSize: '11.5px', fontWeight: 600,
                    border: 'none', cursor: kycLoading || kycVerified || !kycConsent ? 'not-allowed' : 'pointer',
                    backgroundColor: kycVerified ? '#E6FBF5' : (kycLoading || !kycConsent) ? '#F6F9FC' : '#007D87',
                    color: kycVerified ? '#007D60' : (kycLoading || !kycConsent) ? '#9CA3AF' : '#FFFFFF',
                    whiteSpace: 'nowrap',
                  }}
                  title={!kycConsent ? 'Tick the consent checkbox to enable KYC' : ''}
                >
                  {kycVerified ? '\u2713 Verified' : kycLoading ? 'Verifying...' : 'Verify PAN \u2192'}
                </button>
              </div>
            </PxFormField>
            ...
            <PxFormField label="MSME / Udyam Number" hint={msmeResult ? `Verified via ${msmeResult.source} — ${msmeResult.enterprise_name}` : (msmeError || undefined)}>
              <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                <div style={{ flex: 1 }}>
                  {renderInput('msme_number', 'UDYAM-XX-00-0000000', panCompliance.msme_number, updatePan)}
                </div>
                <button
                  onClick={verifyMsme}
                  ...
                  style={{
                    height: 30, padding: '0 12px', borderRadius: 6, fontSize: '11.5px', fontWeight: 600,
                    border: 'none',
                    cursor: msmeLoading || !panCompliance.msme_number || !kycConsent ? 'not-allowed' : 'pointer',
                    backgroundColor: msmeResult ? '#E6FBF5' : (msmeLoading || !kycConsent) ? '#F6F9FC' : '#007D87',
                    color: msmeResult ? '#007D60' : (msmeLoading || !kycConsent) ? '#9CA3AF' : '#FFFFFF',
                    whiteSpace: 'nowrap',
                  }}
          ...
          {/* KYC Result Panel */}
          {kycResult && (
            <div style={{ border: '1.5px solid #9FE8D0', borderRadius: 12, backgroundColor: '#E6FBF5', padding: 16, marginTop: 12, marginBottom: 12 }}>
              {kycMockMode && (
                <div style={{
                  padding: '6px 10px', marginBottom: 10, borderRadius: 6, fontSize: 11, fontWeight: 600,
                  backgroundColor: '#FEF3C7', color: '#92400E', border: '1px solid #FCD34D',
                }}>
                  ⚠ Mock mode — Ongrid API key not configured. Set ONGRID_API_KEY to use live verification.
                </div>
              )}
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12, flexWrap: 'wrap', gap: 8 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  <span style={{ color: '#007D60', fontSize: 16 }}>{'\u2713'}</span>
                  <span style={{ fontWeight: 600, color: '#007D60', fontSize: 14 }}>KYC Verified — {kycResult.name}</span>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 11, color: '#6E7A82' }}>
                  <span style={{
                    padding: '2px 8px', borderRadius: 10, fontSize: 10, fontWeight: 600,
                    backgroundColor: '#FFFFFF', color: '#007D87', border: '1px solid #9FE8EE',
                  }}>
                    {kycSource || 'ongrid'}
                  </span>
                  {kycTxnId && <span title={kycTxnId}>txn: {String(kycTxnId).slice(0, 12)}…</span>}
                </div>
              </div>
              ...
              {(() => {
                ...
                return (
                  <div style={{
                    padding: '6px 10px', marginBottom: 10, borderRadius: 6, fontSize: 11, fontWeight: 500,
                    backgroundColor: '#FEF2F2', color: '#B91C1C', border: '1px solid #FECACA',
                  }}>
                    ⚠ {mismatches.length} GSTIN(s) belong to a different PAN — please review before auto-filling.
                  </div>
                );
              })()}
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, fontSize: 12 }}>
                <div><span style={{ color: '#6E7A82' }}>Entity Type:</span> <strong>{kycResult.entity_type}</strong></div>
                <div><span style={{ color: '#6E7A82' }}>PAN Status:</span> <strong style={{ color: '#007D60' }}>{kycResult.pan_status}</strong></div>
                <div><span style={{ color: '#6E7A82' }}>Incorporation:</span> <strong>{kycResult.date_of_incorporation || '\u2014'}</strong></div>
                <div><span style={{ color: '#6E7A82' }}>CIN:</span> <strong>{kycResult.cin || '\u2014'}</strong></div>
                {kycResult.directors?.length > 0 && (
                  <div style={{ gridColumn: '1/3' }}><span style={{ color: '#6E7A82' }}>Directors:</span> <strong>{kycResult.directors.join(', ')}</strong></div>
                )}
                {kycResult.gstin_list?.length > 0 && (
                  <div style={{ gridColumn: '1/3' }}>
                    <span style={{ color: '#6E7A82' }}>GST Registrations:</span> <strong>{kycResult.gstin_list.length} found</strong>
                    <div style={{ marginTop: 4 }}>
                      {kycResult.gstin_list.map((g: any, i: number) => (
                        <span key={i} style={{ display: 'inline-block', margin: '4px 4px 0 0', padding: '2px 8px', borderRadius: 4, fontSize: 11, backgroundColor: '#D6F7F9', color: '#007D87' }}>
                ...
                <button
                  onClick={() => {
                    ...
                    if (kycResult.gstin_list?.length) {
                      setGstRegistrations(kycResult.gstin_list.map((g: any) => ({
                        gstin: g.gstin,
                        gst_type: g.gst_type || 'registered_regular',
                        state: g.state || '',
                        gst_state_code: g.gstin?.substring(0, 2) || '',
                        status: g.status || 'active',
                        city: '', pin_code: '', address: '', spoc_id: '',
                      })));
                    }
                  }}
                  style={{ padding: '6px 14px', borderRadius: 6, fontSize: 12, fontWeight: 600, backgroundColor: '#007D87', color: '#FFFFFF', border: 'none', cursor: 'pointer' }}
                >
          ...
          {kycError && (
            <div style={{ color: '#DC2626', fontSize: 12, marginTop: 8, padding: '8px 12px', backgroundColor: '#FEE2E2', borderRadius: 8 }}>
              {kycError}
            </div>
          )}
        </div>
        ...
        <div>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
            <h3 style={{ fontSize: 14, fontWeight: 600, color: 'var(--color-ink)', margin: 0 }}>Vendor Locations &amp; GST Details</h3>
            <AddRowButton label="Add Location" onClick={() => setGstRegistrations((prev) => [...prev, emptyGst()])} />
          </div>
          <SimpleInlineTable
            columns={GST_COLUMNS}
      ...
      <FormSection
        ...
        action={<AddRowButton label="Add Bank Account" onClick={() => setBankAccounts((prev) => [...prev, emptyBank()])} />}
        flat
      >
        <SimpleInlineTable
        ...
        {bankAccounts.length > 0 && (
          <div style={{ marginTop: 16, padding: '12px 14px', borderRadius: 10, backgroundColor: '#FAFBFC', border: '1px solid var(--color-silver)' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
              <h4 style={{ fontSize: 13, fontWeight: 600, color: 'var(--color-ink)', margin: 0 }}>Penny-Drop Verification</h4>
              <span style={{ fontSize: 11, color: 'var(--color-mercury-grey)' }}>Verify each account with ₹1 penny drop via Ongrid</span>
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {bankAccounts.map((row, idx) => {
                ...
                const nameMatch = result && vendor.vendor_legal_name
                  ? result.account_holder_name?.toUpperCase().replace(/[^A-Z]/g, '').includes(vendor.vendor_legal_name.toUpperCase().replace(/[^A-Z]/g, '').slice(0, 8))
                  : null;
                return (
                  <div key={idx} style={{
                    ...
                    <span style={{ fontSize: 12, color: 'var(--color-ink)', fontWeight: 600, minWidth: 170 }}>
                      {row.account_number || <span style={{ color: 'var(--color-mercury-grey)', fontWeight: 400 }}>(no account #)</span>}
                    </span>
                    <span style={{ fontSize: 12, color: 'var(--color-mercury-grey)', minWidth: 120 }}>
                      {row.ifsc_code || '—'}
                    </span>
                    {result ? (
                      <>
                        ...
                        <span style={{ fontSize: 10, padding: '2px 8px', borderRadius: 10, backgroundColor: '#FFFFFF', color: '#007D87', border: '1px solid #9FE8EE', fontWeight: 600 }}>
                          {result.source}
                        </span>
                        {result.mock_mode && (
                          <span style={{ fontSize: 10, padding: '2px 8px', borderRadius: 10, backgroundColor: '#FEF3C7', color: '#92400E', fontWeight: 600 }}>
                            mock
                          </span>
                        )}
                        {nameMatch === false && (
                          <span style={{ fontSize: 10, padding: '2px 8px', borderRadius: 10, backgroundColor: '#FEF2F2', color: '#B91C1C', border: '1px solid #FECACA', fontWeight: 600 }}>
                            ⚠ name mismatch
                          </span>
                        )}
                      </>
                    ...
                    <button
                      ...
                      disabled={loading || !!result || !row.account_number || !row.ifsc_code || !kycConsent}
                      style={{
                        height: 28, padding: '0 12px', borderRadius: 6, fontSize: 11, fontWeight: 600, border: 'none',
                        cursor: loading || !!result || !row.account_number || !row.ifsc_code || !kycConsent ? 'not-allowed' : 'pointer',
                        backgroundColor: result ? '#E6FBF5' : (loading || !kycConsent || !row.account_number || !row.ifsc_code) ? '#F6F9FC' : '#007D87',
                        color: result ? '#007D60' : (loading || !kycConsent || !row.account_number || !row.ifsc_code) ? '#9CA3AF' : '#FFFFFF',
                        whiteSpace: 'nowrap',
                      }}
                      title={!kycConsent ? 'Tick the consent checkbox in Tax & Compliance first' : ''}
                    >
      ...
      <FormSection
        ...
        action={<AddRowButton label="Add Entity Mapping" onClick={() => setEntityMappings((prev) => [...prev, emptyEntityMapping()])} />}
        flat
      >
        <SimpleInlineTable
