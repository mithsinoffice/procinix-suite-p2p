...
export function TaxCodeMaster() {
  ...
  const [taxCodes, setTaxCodes] = useIncrementalMasterRecords<TaxCode>('tax_code_master', [
    ...
    },
    {
      id: '2',
      taxCode: '6115',
      description: 'Pantyhose, tights, stockings and leggings',
      taxRate: '12%',
      status: 'Active',
      approvalStatus: 'Approved',
    },
    {
      id: '3',
      taxCode: '6201',
      description: "Men's or boys' overcoats and jackets",
      taxRate: '12%',
      status: 'Active',
    ...
    },
    {
      id: '4',
      taxCode: '6203',
      description: "Men's or boys' suits and trousers",
      taxRate: '12%',
      status: 'Active',
      approvalStatus: 'Approved',
    },
    {
      id: '5',
      taxCode: '998314',
      description: 'Business support service (SAC)',
      taxRate: '18%',
      status: 'Active',
  ...
  const filteredTaxCodes = useMemo(() => {
    return taxCodes.filter((tax) => {
      const haystack = [tax.taxCode, tax.description ?? '', tax.taxRate ?? '']
        .join(' ')
        .toLowerCase();
      const matchesSearch = haystack.includes(searchTerm.toLowerCase());
      const matchesStatus = statusFilter.length === 0 || statusFilter.includes(tax.status ?? '');
      const matchesApproval =
        approvalFilter.length === 0 || approvalFilter.includes(tax.approvalStatus);
      return matchesSearch && matchesStatus && matchesApproval;
    });
  }, [taxCodes, searchTerm, statusFilter, approvalFilter]);

  const hasActiveFilters =
    searchTerm.trim().length > 0 || statusFilter.length > 0 || approvalFilter.length > 0;
    ...
    if (tax.originalData) {
      ...
      if (original.description !== tax.description) {
        changes.push({
          ...
          newValue: tax.description ?? '',
        });
      }
      if (original.taxRate !== tax.taxRate) {
        changes.push({
          field: 'Tax Rate',
          oldValue: String(original.taxRate ?? ''),
          newValue: String(tax.taxRate ?? ''),
        });
      }
      ...
      >
        <FormSection title="Tax Code Details" columns={2}>
          <PxFormField
            label="Tax Code"
            required
            filled={String(taxCode).trim().length > 0}
            hint="HSN / SAC code"
          >
            <input
              type="text"
          ...
          <PxFormField
            label="Tax Rate"
            required
            filled={String(taxRate).trim().length > 0}
            hint="Composite GST rate"
          >
            <input
              type="text"
              value={taxRate}
              ...
              <div className="grid grid-cols-2 gap-6">
                <div>
                  <label
                    ...
                    style={{ color: 'var(--color-mercury-grey)' }}
                  >
                    Tax Code <span style={{ color: 'var(--color-error)' }}>*</span>
                  </label>
                ...
                <div>
                  <label
                    ...
                    style={{ color: 'var(--color-mercury-grey)' }}
                  >
                    Tax Rate <span style={{ color: 'var(--color-error)' }}>*</span>
                  </label>
                  <div className="relative">
                    <Percent
                      className="absolute left-3 top-1/2 transform -translate-y-1/2 w-4 h-4"
      ...
      >
        <div className="overflow-x-auto">
          <div style={{ minWidth: '1120px' }}>
            ...
            >
              {['Tax Code', 'Description', 'Tax Rate', 'Status', 'Approval Status', 'Action'].map(
                (column) => (
                  <div
                    key={column}
            ...
            <div>
              {filteredTaxCodes.map((tax, index) => (
                ...
                >
                  ...
                  <div>
                    <span
                      className="px-3 py-1.5 rounded-full text-xs"
                      style={{ ...getStatusBadgeStyle(tax.approvalStatus), fontWeight: 700 }}
                    >
                      {tax.approvalStatus}
                    </span>
                  </div>
                  <div className="flex items-center justify-end gap-2">
                    {tax.approvalStatus === 'Pending Approval' && (
                      <PremiumActionButton
                        label="Review tax code"
                        icon={<Eye className="w-4 h-4" />}
                        tone="teal"
                        onClick={() => handleReview(tax)}
                      />
                    )}
                    <PremiumActionButton
                      label="Edit tax code"
              ...
              {filteredTaxCodes.length === 0 && (
                <div className="px-8 py-16 text-center">
                  ...
                  >
                    No tax codes match the current filters
                  </p>
