...
export function POUpdate() {
  ...
  const handleSaveAndNotify = () => {
    ...
    alert(
      'Milestone updates saved successfully!\n\nNotifications sent to:\n• Procurement Manager\n• Approver\n• Finance Team\n• Vendor'
    );
  };
  ...
  return (
    <div className="p-8" style={{ backgroundColor: 'var(--color-cloud)', minHeight: '100vh' }}>
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p style={{ color: 'var(--color-mercury-grey)' }}>
              Update delivery milestone progress and notify stakeholders
            </p>
          </div>
        </div>
      </div>
      ...
      >
        ...
        <div>
          <p style={{ color: 'var(--color-ink)' }}>
            <strong>Update Delivery Milestones:</strong> Use this screen to track actual completion
            dates for sourcing and production milestones.
          </p>
          <p className="text-sm mt-1" style={{ color: 'var(--color-mercury-grey)' }}>
            After updating, click "Save & Notify Stakeholders" to automatically calculate delays and
      ...
      >
        <div className="flex items-start gap-3 mb-6">
          ...
          <div className="flex-1">
            ...
            <p className="text-sm" style={{ color: '#5E35B1' }}>
              Update actual completion dates and track delays in real-time
            </p>
          </div>
        </div>
          ...
          <>
            <div className="mb-6 space-y-4">
              {milestones.map((milestone) => {
                ...
                const delay = calculateMilestoneDelay(
                      ...
                      <div className="mt-3 flex items-center justify-between">
                        <div className="flex items-center gap-4">
                          <span className="text-sm" style={{ color: 'var(--color-mercury-grey)' }}>
                            Delay:{' '}
                            <span style={{ color: delay > 0 ? 'var(--color-error)' : '#4CAF50' }}>
                          ...
                          {!milestone.actualDate && milestone.estimatedDate && (
                            ...
                            >
                              Expected:{' '}
                              {new Date(milestone.estimatedDate).toLocaleDateString('en-IN')}
                            </span>
                          )}
                        </div>
                      </div>
              ...
              <div className="flex items-center justify-between mb-3">
                ...
                <div className="text-right">
                  <p className="text-sm" style={{ color: 'var(--color-mercury-grey)' }}>
                    {milestones.filter((m) => m.actualDate).length} of {milestones.length}{' '}
                    milestones completed
                  </p>
                </div>
              </div>
              <p className="text-xs" style={{ color: 'var(--color-mercury-grey)' }}>
                This delay metric is automatically calculated based on estimated vs actual
                completion dates
              </p>
            </div>
          </>
        ...
        >
          <p className="text-sm" style={{ color: 'var(--color-mercury-grey)' }}>
            <strong style={{ color: 'var(--color-ink)' }}>Notification Recipients:</strong>{' '}
            Procurement Manager, Approver, Finance Team & Vendor
          </p>
          <button
