...
export function GLCodeMappingSection({
  itemId,
  entityMappings,
  onMappingsChange,
}: GLCodeMappingSectionProps) {
  ...
  const updateMapping = useCallback(
    (entityId: string, field: keyof GLMapping, value: string) => {
      setMappings((prev) => {
  ...
  const copyToAll = useCallback(() => {
    ...
    setMappings((prev) => {
      const updated = prev.map((m) => {
        ...
        return {
          ...m,
          expense_gl_code: source.expense_gl_code,
          expense_gl_description: source.expense_gl_description,
          cogs_gl_code: source.cogs_gl_code,
          cogs_gl_description: source.cogs_gl_description,
          input_tax_gl_code: source.input_tax_gl_code,
          input_tax_gl_description: source.input_tax_gl_description,
          stock_gl_code: source.stock_gl_code,
          stock_gl_description: source.stock_gl_description,
          cost_centre: source.cost_centre,
          profit_centre: source.profit_centre,
        };
      });
      onMappingsChange(updated.filter((m) => selectedIds.has(m.entity_id)));
      return updated;
  ...
  const fullyConfigured = mappings.filter(
    (m) => selectedIds.has(m.entity_id) && isRowFull(m)
  ).length;
  const partiallyConfigured = mappings.filter(
    (m) => selectedIds.has(m.entity_id) && isRowConfigured(m) && !isRowFull(m)
  ).length;
  const configuredCount = mappings.filter(
    (m) => selectedIds.has(m.entity_id) && isRowConfigured(m)
  ).length;
  ...
  const getInputStyle = (value: string, fieldKey: string): React.CSSProperties => ({
    ...
    ...(focusedField === fieldKey
      ? { borderColor: '#00A9B7', boxShadow: '0 0 0 2px rgba(0,169,183,0.1)' }
      : {}),
  });
  ...
  return (
    <div style={{ marginTop: '1.5rem' }}>
      ...
      {showInfo && (
        ...
        >
          Map each entity to the appropriate GL accounts for expense booking, COGS, input tax
          credit, and stock valuation. These mappings drive automatic journal entries when
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 900 }}>
            ...
            <tbody>
              {allEntities.map((entity: any, idx: number) => {
                ...
                const m =
                  mappings.find((r) => r.entity_id === entity.id) ||
                  emptyMapping(entity.id, entity.name);
                ...
                return (
                  <tr
                    key={entity.id}
                    style={{ borderTop: '0.5px solid #E1E6EA', opacity: isActive ? 1 : 0.45 }}
                  >
                    {/* Entity cell */}
                    <td style={{ padding: '10px 16px', whiteSpace: 'nowrap' }}>
                      <div className="flex items-center gap-2.5">
                        ...
                        <div>
                          ...
                          {!isActive ? (
                            <div style={{ fontSize: 10, color: '#B0B8C1', fontStyle: 'italic' }}>
                              not selected above
                            </div>
                          ) : (
                            <div style={{ fontSize: 10, color: '#8B95A1' }}>
                              {entity.country || 'IN'} &middot; {entity.currency || 'INR'}
                            </div>
                          )}
                        </div>
                      </div>
                    ...
                    <td style={{ padding: '8px 12px' }}>
                      <input
                        ...
                        onChange={(e) =>
                          updateMapping(entity.id, 'expense_gl_code', e.target.value)
                        }
                    ...
                    <td style={{ padding: '8px 12px' }}>
                      <input
                        ...
                        onChange={(e) =>
                          updateMapping(entity.id, 'input_tax_gl_code', e.target.value)
                        }
