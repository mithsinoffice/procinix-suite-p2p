...
export function CFODesk() {
  ...
  const kpiData = {
    ...
    complianceScore: {
      value: ReportDataService.getComplianceScore(),
      unit: '%',
      trend: 2.1,
      target: 95,
    },
  ...
  const getPaymentStatusColor = (status: string) => {
    switch (status) {
      case 'Approved':
        return { bg: 'var(--color-teal-tint)', text: 'var(--color-teal)' };
      case 'Pending':
        return { bg: '#FFF9E6', text: '#F59E0B' };
      case 'Overdue':
        return { bg: '#FFE8EA', text: '#EF4444' };
      default:
        return { bg: 'var(--color-cloud)', text: 'var(--color-mercury-grey)' };
    }
  };
        ...
        <div className="flex items-start justify-between mb-3">
          <div className="flex-1">
            ...
            <div className="flex items-baseline gap-2">
              <span
                className="text-2xl"
                style={{ color: isWarning ? '#EF4444' : 'var(--color-ink)' }}
              >
          ...
          {Icon && (
            ...
            >
              <Icon
                className="w-5 h-5"
                style={{ color: isWarning ? '#EF4444' : 'var(--color-teal)' }}
              />
            </div>
          )}
        </div>
          ...
          <div className="flex items-center gap-1 text-xs">
            {isPositive ? (
              <TrendingUp
                ...
                style={{
                  color:
                    title.includes('Variance') && value < 0
                      ? '#10B981'
                      : isWarning || title.includes('Outflow')
                        ? '#EF4444'
                        : '#10B981',
                }}
              />
              ...
              <TrendingDown
                ...
                style={{
                  color:
                    title.includes('Variance') || title.includes('Commitments')
                      ? '#10B981'
                      : '#EF4444',
                }}
              />
  ...
  const AlertCard = ({ title, count, icon: Icon, color, description }: any) => {
    return (
      <div
        className="bg-white rounded-lg p-5"
        style={{ border: `1px solid ${color}30`, boxShadow: '0 1px 3px rgba(0,0,0,0.05)' }}
      >
    ...
    <div className="p-8" style={{ backgroundColor: 'var(--color-cloud)', minHeight: '100vh' }}>
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm" style={{ color: 'var(--color-mercury-grey)' }}>
              Financial Control & Risk Governance • FY 2024-25 • Last updated:{' '}
      ...
      <div className="grid grid-cols-5 gap-4 mb-8">
        ...
        <KPICard
          ...
          icon={
            kpiData.complianceScore.value >= kpiData.complianceScore.target
              ? CheckCircle
              : AlertTriangle
          }
      ...
      <div className="mb-6">
        ...
        <div className="grid grid-cols-2 gap-6 mb-6">
          ...
          >
            <h3 className="text-sm mb-4" style={{ color: 'var(--color-ink)' }}>
              Budget vs Actual vs Committed Spend by Department
            </h3>
          ...
          >
            <h3 className="text-sm mb-4" style={{ color: 'var(--color-ink)' }}>
              Monthly Spend Trend (FY 2024-25)
            </h3>
            <ResponsiveContainer width="100%" height={300}>
        ...
        <div className="grid grid-cols-2 gap-6">
          ...
          >
            <h3 className="text-sm mb-4" style={{ color: 'var(--color-ink)' }}>
              Cash Outflow Forecast from Open POs
            </h3>
            <ResponsiveContainer width="100%" height={280}>
              <AreaChart data={cashOutflowForecast}>
                ...
                <Legend
                  formatter={(value) => {
                    const labels: any = {
                      forecast: 'Forecasted Outflow',
                      committed: 'Committed Outflow',
                    };
                    return (
                      <span style={{ color: 'var(--color-ink)', fontSize: '12px' }}>
                        {labels[value]}
      ...
      >
        <div className="px-6 py-4 border-b" style={{ borderColor: 'var(--color-silver)' }}>
          <h3 className="text-sm" style={{ color: 'var(--color-ink)' }}>
            Open PO Commitments & Payment Schedule
          </h3>
          <p className="text-xs mt-1" style={{ color: 'var(--color-mercury-grey)' }}>
            Upcoming payments requiring cash flow management • Total:{' '}
            {formatCurrency(commitmentsData.reduce((sum, item) => sum + item.amount, 0))}
          </p>
        </div>
