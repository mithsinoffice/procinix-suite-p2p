    ...
    <div className="bg-white rounded-xl p-6 mb-6" style={{ border: '2px solid var(--color-silver)' }}>
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-xl mb-1" style={{ color: 'var(--color-ink)', fontWeight: '700' }}>GST Determination</h2>
            <p className="text-sm" style={{ color: 'var(--color-mercury-grey)' }}>Auto-calculated GST type and tax split</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <Info className="w-4 h-4" style={{ color: 'var(--color-mercury-grey)' }} />
          <span className="text-xs" style={{ color: 'var(--color-mercury-grey)' }} title="GST type is determined based on Place of Supply and Ship-to State. If both states are same, CGST+SGST applies. Otherwise, IGST applies.">
      ...
      {/* GSTIN Information */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6 p-4 rounded-lg" style={{ backgroundColor: 'var(--color-cloud)' }}>
        <div>
          <label className="block text-xs mb-1" style={{ color: 'var(--color-mercury-grey)', fontWeight: '600' }}>Company GSTIN</label>
          <input
            type="text"
            value={companyGSTIN}
          ...
          />
          <p className="text-xs mt-1" style={{ color: 'var(--color-mercury-grey)' }}>State: {companyState || 'N/A'}</p>
        </div>
        <div>
          <label className="block text-xs mb-1" style={{ color: 'var(--color-mercury-grey)', fontWeight: '600' }}>Supplier GSTIN</label>
          <input
            type="text"
            value={vendorGSTNumber}
            disabled
            className="w-full px-3 py-2 rounded-lg text-sm"
            style={{ border: '1px solid var(--color-silver)', backgroundColor: '#FFFFFF', color: 'var(--color-mercury-grey)' }}
          />
          <p className="text-xs mt-1" style={{ color: 'var(--color-mercury-grey)' }}>State: {supplierState || 'N/A'}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-6">
        <div>
          ...
          <select
            ...
            style={{
              border: placeOfSupply ? '2px solid var(--color-teal)' : '2px solid var(--color-silver)',
              color: 'var(--color-ink)'
            }}
            required
          >
        ...
        <div>
          ...
          {selectedPO && (
            <p className="text-xs mt-1" style={{ color: 'var(--color-mercury-grey)' }}>Derived from PO</p>
          )}
        </div>
        <div>
          <label className="block text-sm mb-2" style={{ color: 'var(--color-ink)', fontWeight: '600' }}>Supply Type</label>
          <select
            value={supplyType}
        ...
        <div>
          <label className="block text-sm mb-2" style={{ color: 'var(--color-ink)', fontWeight: '600' }}>Special Category</label>
      ...
      {placeOfSupply && shipToState && (
        <div className="p-4 rounded-lg mb-4" style={{ backgroundColor: gstType === 'CGST+SGST' ? '#DCFCE7' : '#DBEAFE', border: gstType === 'CGST+SGST' ? '2px solid #10B981' : '2px solid #3B82F6' }}>
          ...
          {/* GST Override Panel */}
          {showGSTOverride && (
            <div className="mt-4 p-4 rounded-lg" style={{ backgroundColor: '#FFFFFF', border: '1px solid var(--color-silver)' }}>
              <h4 className="text-sm mb-3" style={{ color: 'var(--color-ink)', fontWeight: '600' }}>Override GST Type (Requires Justification)</h4>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-3">
                <div>
                  <label className="block text-xs mb-1" style={{ color: 'var(--color-mercury-grey)' }}>New GST Type</label>
                  <select
                    onChange={(e) => {
                ...
                <div>
                  <label className="block text-xs mb-1" style={{ color: 'var(--color-mercury-grey)' }}>Override Reason</label>
                  <select
              ...
              <div>
                <label className="block text-xs mb-1" style={{ color: 'var(--color-mercury-grey)' }}>Comments (Mandatory)</label>
                <textarea
