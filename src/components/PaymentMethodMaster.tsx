...
export function PaymentMethodMaster() {
  ...
  const [records, setRecords] = useIncrementalMasterRecords<PaymentMethod>('payment_method_master', [
    { id: '1', methodCode: 'NEFT', methodName: 'National Electronic Fund Transfer', description: 'Bank-to-bank electronic fund transfer', processingTime: 'T+1', maxAmount: 0, currency: 'INR', isOnline: true, status: 'Active', approvalStatus: 'Approved' },
    { id: '2', methodCode: 'RTGS', methodName: 'Real Time Gross Settlement', description: 'Real-time high-value transfers, min 2,00,000', processingTime: 'T+0', maxAmount: 0, currency: 'INR', isOnline: true, status: 'Active', approvalStatus: 'Approved' },
    { id: '3', methodCode: 'IMPS', methodName: 'Immediate Payment Service', description: 'Instant interbank transfer up to 5 lakh', processingTime: 'T+0', maxAmount: 500000, currency: 'INR', isOnline: true, status: 'Active', approvalStatus: 'Approved' },
    { id: '4', methodCode: 'UPI', methodName: 'Unified Payments Interface', description: 'Instant mobile payment up to 1 lakh', processingTime: 'T+0', maxAmount: 100000, currency: 'INR', isOnline: true, status: 'Active', approvalStatus: 'Approved' },
    { id: '5', methodCode: 'CHQ', methodName: 'Cheque/DD', description: 'Physical cheque or demand draft', processingTime: '3-5 days', maxAmount: 0, currency: 'INR', isOnline: false, status: 'Active', approvalStatus: 'Approved' },
    { id: '6', methodCode: 'WIRE', methodName: 'International Wire Transfer', description: 'Cross-border wire transfer via SWIFT', processingTime: 'T+2', maxAmount: 0, currency: 'INR', isOnline: true, status: 'Active', approvalStatus: 'Approved' },
    { id: '7', methodCode: 'ACH', methodName: 'Automated Clearing House', description: 'Batch electronic transfers', processingTime: 'T+1', maxAmount: 0, currency: 'INR', isOnline: true, status: 'Active', approvalStatus: 'Approved' },
  ]);
  ...
  const filteredRecords = useMemo(() => {
    return records.filter((r) => {
      const haystack = [r.methodCode, r.methodName, r.description, r.processingTime].join(' ').toLowerCase();
  ...
  const handleSubmit = (approvalStatus: PaymentMethod['approvalStatus'] = 'Pending Approval') => {
    if (isEditMode && editingId) {
      ...
      const updated: PaymentMethod = {
        id: editingId, methodCode, methodName, description, processingTime, maxAmount, currency, isOnline, status, approvalStatus, originalData: originalRecord, entityMappings,
      };
      setRecords(records.map(d => d.id === editingId ? updated : d));
    } else {
      const newRec: PaymentMethod = {
        id: Date.now().toString(), methodCode, methodName, description, processingTime, maxAmount, currency, isOnline, status, approvalStatus, entityMappings,
      };
  ...
  const resetForm = () => {
    setMethodCode(''); setMethodName(''); setDescription(''); setProcessingTime('');
    setMaxAmount(0); setCurrency('INR'); setIsOnline(true); setStatus('Active');
  ...
  const handleEdit = (rec: PaymentMethod) => {
    ...
    setMethodCode(rec.methodCode || ''); setMethodName(rec.methodName || '');
    setDescription(rec.description || ''); setProcessingTime(rec.processingTime || '');
    setMaxAmount(rec.maxAmount || 0); setCurrency(rec.currency || 'INR');
    setIsOnline(rec.isOnline ?? true); setStatus(rec.status || 'Active');
  ...
  const handleReview = (rec: PaymentMethod) => {
    ...
    if (rec.originalData) {
      ...
      if (o.methodCode !== rec.methodCode) changes.push({ field: 'Method Code', oldValue: o.methodCode, newValue: rec.methodCode });
      if (o.methodName !== rec.methodName) changes.push({ field: 'Method Name', oldValue: o.methodName, newValue: rec.methodName });
      if (o.description !== rec.description) changes.push({ field: 'Description', oldValue: o.description, newValue: rec.description });
      if (o.processingTime !== rec.processingTime) changes.push({ field: 'Processing Time', oldValue: o.processingTime, newValue: rec.processingTime });
      if (o.maxAmount !== rec.maxAmount) changes.push({ field: 'Max Amount', oldValue: String(o.maxAmount), newValue: String(rec.maxAmount) });
  ...
  const handleApprove = async () => {
    if (currentReviewRecord) {
      const nextRecords = await applyMasterApprovalAction('payment_method_master', records, currentReviewRecord.id, 'approve');
  ...
  const handleReject = async () => {
    if (currentReviewRecord) {
      const nextRecords = await applyMasterApprovalAction('payment_method_master', records, currentReviewRecord.id, 'reject');
      setRecords(nextRecords);
    }
    setShowApprovalModal(false); setCurrentReviewRecord(null);
  };
  ...
  const handleRequestInfo = async () => {
    if (currentReviewRecord) {
      ...
      const nextRecords = await applyMasterApprovalAction('payment_method_master', records, currentReviewRecord.id, 'request_info', comments);
      setRecords(nextRecords);
    }
    setShowApprovalModal(false); setCurrentReviewRecord(null);
  };
  ...
  if (showForm) {
    return (
      <FormShell masterName="Payment Method Master"
      ...
      >
        <FormSection title="Payment Method Details" columns={2}>
          <PxFormField label="Method Code" required filled={!!methodCode.trim()}>
            <input type="text" value={methodCode} onChange={(e) => setMethodCode(e.target.value)} placeholder="e.g., NEFT" className="px-input" />
          </PxFormField>
          <PxFormField label="Method Name" required filled={!!methodName.trim()}>
            <input type="text" value={methodName} onChange={(e) => setMethodName(e.target.value)} placeholder="e.g., National Electronic Fund Transfer" className="px-input" />
          </PxFormField>
          <PxFormField label="Description" filled={!!description.trim()}>
            <input type="text" value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Method description" className="px-input" />
          </PxFormField>
          <PxFormField label="Processing Time" required filled={!!processingTime.trim()}>
            <input type="text" value={processingTime} onChange={(e) => setProcessingTime(e.target.value)} placeholder="e.g., T+0, T+1" className="px-input" />
          </PxFormField>
          <PxFormField label="Max Amount (0 = unlimited)" filled={maxAmount > 0}>
            <input type="number" value={maxAmount} onChange={(e) => setMaxAmount(Number(e.target.value))} placeholder="0" className="px-input" />
          </PxFormField>
          ...
          <PxFormField label="Online Method" filled>
            <select value={isOnline ? 'Yes' : 'No'} onChange={(e) => setIsOnline(e.target.value === 'Yes')} className="px-select">
  ...
  return (
    <MasterPageShell masterName="Payment Method Master" description="Manage payment methods and channels">
          ...
          <table className="w-full">
            <thead style={{ backgroundColor: 'var(--color-cloud)' }}>
              <tr>
                ...
                <th className="px-6 py-4 text-left text-sm" style={{ color: 'var(--color-mercury-grey)' }}>Method Name</th>
                <th className="px-6 py-4 text-left text-sm" style={{ color: 'var(--color-mercury-grey)' }}>Processing Time</th>
                <th className="px-6 py-4 text-left text-sm" style={{ color: 'var(--color-mercury-grey)' }}>Max Amount</th>
                <th className="px-6 py-4 text-left text-sm" style={{ color: 'var(--color-mercury-grey)' }}>Online</th>
            ...
            <tbody>
              {filteredRecords.map((rec, index) => (
                <tr key={rec.id} style={{ borderTop: index === 0 ? 'none' : '1px solid var(--color-silver)' }}>
                  <td className="px-6 py-4" style={{ color: 'var(--color-ink)' }}>{rec.methodCode}</td>
                  <td className="px-6 py-4" style={{ color: 'var(--color-ink)' }}>{rec.methodName}</td>
                  <td className="px-6 py-4" style={{ color: 'var(--color-mercury-grey)' }}>{rec.processingTime}</td>
                  <td className="px-6 py-4" style={{ color: 'var(--color-mercury-grey)' }}>{rec.maxAmount === 0 ? 'Unlimited' : rec.maxAmount.toLocaleString('en-IN')}</td>
                  <td className="px-6 py-4" style={{ color: 'var(--color-mercury-grey)' }}>{rec.isOnline ? 'Yes' : 'No'}</td>
