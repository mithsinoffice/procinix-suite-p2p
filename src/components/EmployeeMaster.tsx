...
export function EmployeeMaster() {
  ...
  const [employees, setEmployees, isHydrating, persistEmployees] =
    useIncrementalMasterRecords<Employee>('employee_master', [
      {
        id: '1',
        empCode: 'EMP001',
        ...
        baseEntity: 'Subko Coffee Private Limited',
        baseLocation: 'Mumbai HQ',
        reportingManager: 'CFO Office',
        costCenter: 'CC-PROC-001',
        profitCenter: 'PC-HQ-001',
        status: 'Active',
        defaultFunctionalContext: 'Procurement Desk',
      ...
      },
      {
        ...
        profitCenter: 'PC-HQ-001',
        status: 'Active',
        defaultFunctionalContext: 'Accounts Payable',
        approvalStatus: 'Approved',
      },
      {
        id: '3',
        ...
        baseEntity: 'Subko Coffee Private Limited',
        baseLocation: 'Bengaluru Warehouse',
        reportingManager: 'Operations Head',
        costCenter: 'CC-WH-001',
        profitCenter: 'PC-SUPPLY-001',
        status: 'Active',
        defaultFunctionalContext: 'GRN / SRN',
        approvalStatus: 'Approved',
      },
      {
        id: '4',
        empCode: 'EMP004',
        empName: 'Sneha Verma',
        email: 'sneha.verma@procinix.ai',
        phone: '+91 98765 43213',
        department: 'Quality',
        designation: 'Quality Analyst',
        baseEntity: 'Subko Coffee Private Limited',
        baseLocation: 'Mumbai Roastery',
        reportingManager: 'Operations Head',
        costCenter: 'CC-QA-001',
        profitCenter: 'PC-OPS-001',
        status: 'Inactive',
        defaultFunctionalContext: 'Quality Review',
        approvalStatus: 'Approved',
      },
      {
        id: '5',
        ...
        email: 'vikram.singh@procinix.ai',
        phone: '+91 98765 43214',
        department: 'Procurement',
        designation: 'Buyer',
        baseEntity: 'Subko Coffee Private Limited',
        baseLocation: 'Delhi Branch',
        reportingManager: 'Rajesh Kumar',
        costCenter: 'CC-PROC-002',
        profitCenter: 'PC-NORTH-001',
        status: 'Active',
        defaultFunctionalContext: 'Purchase Requisition',
  ...
  const sortedEmployees = useMemo(() => {
    ...
    return [...employees].sort((left, right) => {
      ...
      const codeDiff = left.empCode.localeCompare(right.empCode, undefined, {
        sensitivity: 'base',
      });
      if (codeDiff !== 0) {
        return codeDiff;
  ...
  const filteredEmployees = useMemo(() => {
    return sortedEmployees.filter((employee) => {
      ...
      const matchesApproval =
        approvalFilter.length === 0 || approvalFilter.includes(employee.approvalStatus);
  ...
  const entityOptions = useMemo(
    () => entities.filter((entity) => entity.isActive).map((entity) => entity.name),
    [entities]
  );
  const departmentOptions = useMemo(() => {
    const liveDepartments = departments
      .map((item: any) => item.name || item.departmentName || item.department || '')
      .filter(Boolean);
    return [
      ...new Set([...liveDepartments, ...sortedEmployees.map((employee) => employee.department)]),
    ];
  }, [departments, sortedEmployees]);
  const costCenterOptions = useMemo(() => {
    const liveCostCentres = costCentres
      .map((item: any) => item.code || item.name || '')
      .filter(Boolean);
    return [
      ...new Set([
        ...liveCostCentres,
        ...sortedEmployees.map((employee) => employee.costCenter || '').filter(Boolean),
      ]),
    ];
  }, [costCentres, sortedEmployees]);
  const profitCenterOptions = useMemo(() => {
    const liveProfitCentres = profitCentres
      .map((item: any) => item.code || item.name || '')
      .filter(Boolean);
    return [
      ...new Set([
        ...liveProfitCentres,
        ...sortedEmployees.map((employee) => employee.profitCenter || '').filter(Boolean),
      ]),
    ];
  }, [profitCentres, sortedEmployees]);
  const managerOptions = useMemo(
    () => [...new Set(sortedEmployees.map((employee) => employee.empName))],
    [sortedEmployees]
  );
  ...
  const functionalContextOptions = useMemo(
    () => [
      ...new Set([
        'Chanakya Desk',
        'Procurement Desk',
  ...
  const handleSubmit = async (approvalStatus: Employee['approvalStatus'] = 'Pending Approval') => {
    ...
    if (
      !isEditMode &&
      employees.some(
        (employee) => employee.empCode.trim().toLowerCase() === empCode.trim().toLowerCase()
      )
    ) {
  ...
  const handleReview = (employee: Employee) => {
    ...
    if (employee.originalData) {
      ...
      if (original.empCode !== employee.empCode) {
        changes.push({
          field: 'Employee Code',
          oldValue: original.empCode,
          newValue: employee.empCode,
        });
      }
      if (original.empName !== employee.empName) {
        changes.push({
          field: 'Employee Name',
          oldValue: original.empName,
          newValue: employee.empName,
        });
      }
      ...
      if ((original.designation || '') !== (employee.designation || '')) {
        changes.push({
          ...
          newValue: employee.designation || '-',
        });
      }
      if ((original.baseEntity || '') !== (employee.baseEntity || '')) {
        changes.push({
          field: 'Base Entity',
          oldValue: original.baseEntity || '-',
          newValue: employee.baseEntity || '-',
        });
      }
      if ((original.baseLocation || '') !== (employee.baseLocation || '')) {
        changes.push({
          field: 'Base Location / Branch',
          oldValue: original.baseLocation || '-',
          newValue: employee.baseLocation || '-',
        });
      }
      if ((original.reportingManager || '') !== (employee.reportingManager || '')) {
        changes.push({
          field: 'Reporting Manager',
          oldValue: original.reportingManager || '-',
          newValue: employee.reportingManager || '-',
        });
      }
      if ((original.costCenter || '') !== (employee.costCenter || '')) {
        changes.push({
          field: 'Cost Center',
          oldValue: original.costCenter || '-',
          newValue: employee.costCenter || '-',
        });
      }
      if ((original.profitCenter || '') !== (employee.profitCenter || '')) {
        changes.push({
          field: 'Profit Center',
          oldValue: original.profitCenter || '-',
          newValue: employee.profitCenter || '-',
        });
      }
      if ((original.defaultFunctionalContext || '') !== (employee.defaultFunctionalContext || '')) {
        changes.push({
          field: 'Default Functional Context',
          oldValue: original.defaultFunctionalContext || '-',
          newValue: employee.defaultFunctionalContext || '-',
        });
      }
  ...
  return showForm ? (
    ...
    >
      ...
      <FormSection title="Settings" columns={2}>
        <PxFormField
          ...
          required
          filled={!!defaultFunctionalContext}
        >
          <select
    ...
    <MasterPageShell
      masterName="Employee Master"
      description="Manage employee records with approval workflow"
    >
        ...
        filters={[
          {
            key: 'department',
            label: 'Department',
            options: [...new Set(sortedEmployees.map((e) => e.department))],
              ...
              {filteredEmployees.map((emp, index) => (
                <div
                  ...
                  style={{
                    gridTemplateColumns: '1.3fr 1.8fr 2fr 1.3fr 1.2fr 1fr 1.3fr 0.9fr',
                    borderBottom:
                      index === filteredEmployees.length - 1 ? 'none' : '1px solid #EDF3F7',
                ...
                >
                  ...
                  <div>
                    <span
                      ...
                      style={{
                        backgroundColor:
                          emp.status === 'Active' ? 'var(--color-teal-tint)' : '#FFE8EA',
                        color: emp.status === 'Active' ? 'var(--color-teal)' : 'var(--color-error)',
                        fontWeight: 700,
                      }}
                    >
                      {emp.status}
                  ...
                  <div>
                    <span
                      className="px-3 py-1.5 rounded-full text-xs"
                      style={{ ...getStatusBadgeStyle(emp.approvalStatus), fontWeight: 700 }}
                    >
                      {emp.approvalStatus}
                    </span>
                  </div>
