...
export function QuantityAmendmentForm({ lineItems, onChange }: QuantityAmendmentFormProps) {
  const [newQtys, setNewQtys] = useState<Record<string, number>>(() => {
    ...
    lineItems.forEach((item) => { initial[item.id] = item.currentQty; });
    return initial;
  });
  ...
  const warnings = useMemo(() => {
    ...
    lineItems.forEach((item) => {
      ...
      if (newQty < item.receivedQty && newQty > 0) {
        msgs.push(`${item.itemName}: New qty (${newQty}) is below received qty (${item.receivedQty})`);
      }
    });
    return msgs;
  }, [lineItems, newQtys]);
  ...
  const summary = useMemo(() => {
    ...
    const newTotal = lineItems.reduce((sum, i) => sum + (newQtys[i.id] ?? i.currentQty) * i.unitPrice, 0);
    ...
    <div>
      ...
      {hasCancellation && (
        <div style={{
          ...
          background: 'var(--color-error-light)', borderRadius: 12, marginBottom: 16,
      ...
      {warnings.length > 0 && (
        <div style={{
          ...
          <AlertTriangle size={18} style={{ color: 'var(--color-warning-dark)', flexShrink: 0, marginTop: 2 }} />
          ...
          {lineItems.map((item) => {
            ...
            return (
              <tr key={item.id} style={{
                ...
                <td style={{
                  padding: '8px 12px', color: 'var(--color-ink)', fontWeight: 500,
                  textDecoration: isCancelled ? 'line-through' : 'none',
                }}>
                  {item.itemName}
                </td>
                <td style={{ padding: '8px 12px', color: 'var(--color-slate)' }}>{item.currentQty}</td>
                <td style={{ padding: '8px 12px', color: 'var(--color-teal)' }}>{item.receivedQty}</td>
                ...
                <td style={{
                  ...
                  color: change > 0 ? 'var(--color-success)' : change < 0 ? 'var(--color-error)' : 'var(--color-slate)',
                }}>
                  {change > 0 ? '+' : ''}{change}
                </td>
              </tr>
      ...
      <div style={{
        ...
        <div>
          <div style={{ fontSize: 11, color: 'var(--color-slate)', marginBottom: 4 }}>Original Value</div>
          <div style={{ fontSize: 15, fontWeight: 600, color: 'var(--color-ink)' }}>{fmt(summary.originalTotal)}</div>
        </div>
        <div>
          <div style={{ fontSize: 11, color: 'var(--color-slate)', marginBottom: 4 }}>New Value</div>
          <div style={{ fontSize: 15, fontWeight: 600, color: 'var(--color-ink)' }}>{fmt(summary.newTotal)}</div>
        </div>
        <div>
          <div style={{ fontSize: 11, color: 'var(--color-slate)', marginBottom: 4 }}>Net Change</div>
          <div style={{
