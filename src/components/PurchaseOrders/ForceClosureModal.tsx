...
export function ForceClosureModal({ poId, poNumber, onClose, onSuccess }: ForceClosureModalProps) {
  ...
  const handleSubmit = async () => {
    ...
    setError('');
    try {
      await mysqlApiRequest(`/api/purchase-orders/${poId}/force-close`, {
        method: 'POST',
  ...
  const statusColor = (status: string) => {
    switch (status.toLowerCase()) {
      ...
      case 'pending': return 'var(--color-error)';
      ...
      >
        ...
        <div style={{
          ...
          <button onClick={onClose} style={{
            ...
            color: 'var(--color-slate)', padding: 4,
          }}>
            <X size={20} />
          </button>
        </div>
        ...
        <div style={{ padding: '20px 24px' }}>
          ...
          <div style={{
            ...
            backgroundColor: 'var(--color-error-light)', borderRadius: 12, marginBottom: 20,
          }}>
          ...
          {preview && (
            <>
              ...
              <div style={{
                ...
                padding: 16, background: 'var(--color-cloud)', borderRadius: 12,
                marginBottom: 20, border: '1px solid var(--color-silver)',
              }}>
                <div>
                  <div style={{ fontSize: 11, color: 'var(--color-slate)', marginBottom: 4 }}>PO Number</div>
                  <div style={{ fontSize: 14, fontWeight: 600, color: 'var(--color-ink)' }}>
                    {preview.poNumber || poNumber}
                  </div>
                </div>
                <div>
                  <div style={{ fontSize: 11, color: 'var(--color-slate)', marginBottom: 4 }}>Total Value</div>
                  <div style={{ fontSize: 14, fontWeight: 600, color: 'var(--color-ink)' }}>{fmt(preview.totalValue)}</div>
                </div>
                <div>
                  <div style={{ fontSize: 11, color: 'var(--color-slate)', marginBottom: 4 }}>Received</div>
                  <div style={{ fontSize: 14, fontWeight: 600, color: 'var(--color-success)' }}>{fmt(preview.receivedValue)}</div>
                </div>
                <div>
                  <div style={{ fontSize: 11, color: 'var(--color-slate)', marginBottom: 4 }}>Pending</div>
                  <div style={{ fontSize: 14, fontWeight: 600, color: 'var(--color-warning-dark)' }}>{fmt(preview.pendingValue)}</div>
                </div>
              </div>
                  ...
                  <tbody>
                    {preview.lineItems.map((item) => (
                      <tr key={item.id} style={{ borderBottom: '1px solid var(--color-silver)' }}>
                        <td style={{ padding: '8px 12px', color: 'var(--color-ink)' }}>{item.itemName}</td>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 16, marginBottom: 20 }}>
                ...
                <div style={{ display: 'flex', gap: 24 }}>
                  <label style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer', fontSize: 14, color: 'var(--color-ink)' }}>
                    <input type="checkbox" checked={notifyVendor} onChange={(e) => setNotifyVendor(e.target.checked)} />
                    Notify Vendor
                  </label>
                  <label style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer', fontSize: 14, color: 'var(--color-ink)' }}>
                    <input type="checkbox" checked={accrualReversal} onChange={(e) => setAccrualReversal(e.target.checked)} />
              ...
              <div style={{
                ...
                <div>
                  <div style={{ fontSize: 11, color: 'var(--color-slate)', marginBottom: 4 }}>Budget to be Released</div>
                  <div style={{ fontSize: 16, fontWeight: 600, color: 'var(--color-success)' }}>{fmt(preview.budgetToRelease)}</div>
                </div>
                <div>
                  <div style={{ fontSize: 11, color: 'var(--color-slate)', marginBottom: 4 }}>Accrual Impact</div>
                  <div style={{ fontSize: 16, fontWeight: 600, color: 'var(--color-warning-dark)' }}>{fmt(preview.accrualImpact)}</div>
                </div>
              </div>
            </>
          )}
