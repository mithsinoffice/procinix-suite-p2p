...
export function PriceAmendmentForm({ lineItems, onChange }: PriceAmendmentFormProps) {
  const [newPrices, setNewPrices] = useState<Record<string, number>>(() => {
    ...
    lineItems.forEach((item) => {
      initial[item.id] = item.currentPrice;
    });
    return initial;
  });
  ...
  const summary = useMemo(() => {
    ...
    const newTotal = lineItems.reduce(
      (sum, i) => sum + (newPrices[i.id] ?? i.currentPrice) * i.quantity,
      0
    );
    ...
    <div style={{ display: 'flex', gap: 20, alignItems: 'flex-start' }}>
      ...
      <div style={{ flex: 1, overflowX: 'auto' }}>
        {requiresApproval && (
          ...
          >
            <AlertTriangle
              size={18}
              style={{ color: 'var(--color-warning-dark)', flexShrink: 0 }}
            />
      ...
      >
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          <div>
            <div style={{ fontSize: 11, color: 'var(--color-slate)', marginBottom: 2 }}>
              Original Total
            </div>
          ...
          <div style={{ borderTop: '1px solid var(--color-silver)', paddingTop: 12 }}>
            ...
            <div
              style={{
                ...
                marginTop: 2,
                color:
                  Math.abs(summary.pctChange) > 10 ? 'var(--color-error)' : 'var(--color-slate)',
              }}
            >
              {summary.pctChange > 0 ? '+' : ''}
