...
const REGION_OPTIONS: Array<'GCC' | 'India' | 'South East Asia'> = [
  'GCC',
  'India',
  'South East Asia',
];
const GCC_COUNTRIES = ['UAE', 'Saudi Arabia', 'Qatar', 'Kuwait', 'Bahrain', 'Oman'];
const SOUTH_EAST_ASIA_COUNTRIES = [
  'Singapore',
  'Malaysia',
  'Thailand',
  'Indonesia',
...
export function EntityMaster() {
  ...
  const uniqueCountries = [
    ...new Set(baseEntities.map((e: any) => e.country).filter(Boolean)),
  ].sort();
  const [entities, setEntities] = useIncrementalMasterRecords<EntityRecord>(
    'entity_master',
    baseEntities as EntityRecord[]
  );
  ...
  const approvalsParams = useMemo(() => {
    ...
    const fromApprovals =
      sp.get('fromApprovals') === '1' || Boolean(approvalContextFromStorage?.fromApprovals);
  ...
  const isApproverReview = Boolean(
    approvalsParams.fromApprovals && approvalsParams.approvalId && approvalsParams.recordId
  );
  ...
  const filteredEntities = useMemo(() => {
    return entities.filter((entity) => {
      ...
      const matchesApproval =
        approvalFilter.length === 0 || approvalFilter.includes(entity.approvalStatus ?? 'Approved');
  ...
  const handleReview = (entity: EntityRecord) => {
    ...
    if (original) {
      if (original.code !== entity.code)
        changes.push({ field: 'Entity Code', oldValue: original.code, newValue: entity.code });
      if (original.legalName !== entity.legalName)
        changes.push({
          field: 'Legal Name',
          oldValue: original.legalName,
          newValue: entity.legalName,
        });
      if (original.country !== entity.country)
        changes.push({ field: 'Country', oldValue: original.country, newValue: entity.country });
      if ((original.region || '') !== (entity.region || ''))
        changes.push({
          field: 'Region',
          oldValue: original.region || '',
          newValue: entity.region || '',
        });
      if (original.currency !== entity.currency)
        changes.push({ field: 'Currency', oldValue: original.currency, newValue: entity.currency });
      if (original.taxRegime !== entity.taxRegime)
        changes.push({
          field: 'Tax Regime',
          oldValue: original.taxRegime,
          newValue: entity.taxRegime,
        });
      if (original.isActive !== entity.isActive)
        changes.push({
          field: 'Status',
          oldValue: original.isActive ? 'Active' : 'Inactive',
          newValue: entity.isActive ? 'Active' : 'Inactive',
        });
      if ((original.legalEntityType || '') !== (entity.legalEntityType || ''))
        changes.push({
          field: 'Legal Entity Type',
          oldValue: original.legalEntityType || '',
          newValue: entity.legalEntityType || '',
        });
      if ((original.businessLicenseNumber || '') !== (entity.businessLicenseNumber || ''))
        changes.push({
          field: 'Business License Number',
          oldValue: original.businessLicenseNumber || '',
          newValue: entity.businessLicenseNumber || '',
        });
      if ((original.incorporationDate || '') !== (entity.incorporationDate || ''))
        changes.push({
          field: 'Incorporation Date',
          oldValue: original.incorporationDate || '',
          newValue: entity.incorporationDate || '',
        });
      if ((original.natureOfBusiness || '') !== (entity.natureOfBusiness || ''))
        changes.push({
          field: 'Nature of Business',
          oldValue: original.natureOfBusiness || '',
          newValue: entity.natureOfBusiness || '',
        });
      if ((original.taxRegistrationNumber || '') !== (entity.taxRegistrationNumber || ''))
        changes.push({
          field: 'Tax Registration Number',
          oldValue: original.taxRegistrationNumber || '',
          newValue: entity.taxRegistrationNumber || '',
        });
      if ((original.panNumber || '') !== (entity.panNumber || ''))
        changes.push({
          field: 'PAN Number',
          oldValue: original.panNumber || '',
          newValue: entity.panNumber || '',
        });
      if ((original.msmeNumber || '') !== (entity.msmeNumber || ''))
        changes.push({
          field: 'MSME Number',
          oldValue: original.msmeNumber || '',
          newValue: entity.msmeNumber || '',
        });
      if ((original.registeredAddress || '') !== (entity.registeredAddress || ''))
        changes.push({
          field: 'Registered Address',
          oldValue: original.registeredAddress || '',
          newValue: entity.registeredAddress || '',
        });
      if ((original.city || '') !== (entity.city || ''))
        changes.push({ field: 'City', oldValue: original.city || '', newValue: entity.city || '' });
      if ((original.state || '') !== (entity.state || ''))
        changes.push({
          field: 'State',
          oldValue: original.state || '',
          newValue: entity.state || '',
        });
      if ((original.pincode || '') !== (entity.pincode || ''))
        changes.push({
          field: 'Pincode',
          oldValue: original.pincode || '',
          newValue: entity.pincode || '',
        });
      if ((original.bankName || '') !== (entity.bankName || ''))
        changes.push({
          field: 'Bank Name',
          oldValue: original.bankName || '',
          newValue: entity.bankName || '',
        });
      if ((original.bankAccountNumber || '') !== (entity.bankAccountNumber || ''))
        changes.push({
          field: 'Account Number',
          oldValue: original.bankAccountNumber || '',
          newValue: entity.bankAccountNumber || '',
        });
      if ((original.bankIfsc || '') !== (entity.bankIfsc || ''))
        changes.push({
          field: 'IFSC Code',
          oldValue: original.bankIfsc || '',
          newValue: entity.bankIfsc || '',
        });
      if ((original.bankBranch || '') !== (entity.bankBranch || ''))
        changes.push({
          field: 'Bank Branch',
          oldValue: original.bankBranch || '',
          newValue: entity.bankBranch || '',
        });
    }
  ...
  if (showForm) {
    const scrutinizerInsights: Array<{
    ...
    if (country === 'India' && panNumber.trim() && panNumber.trim().length !== 10) {
      scrutinizerInsights.push({
        ...
        detail:
          'PAN is typically 10 characters. Re-check before approving if this is a key compliance field.',
      });
    }
    ...
    return (
      <FormShell
        ...
        draftStatus={
          isApproverReview
            ? entities.find((entity) => entity.id === editingId)?.approvalStatus ||
              'Pending Approval'
            : editingId
              ? 'Draft'
              : 'New'
          ...
          isApproverReview ? (
            <div className="flex items-center gap-3">
              ...
              <button
                ...
                style={{
                  backgroundColor: approvalActionBusy ? 'var(--color-silver)' : 'var(--color-teal)',
                  fontWeight: 700,
                }}
              >
          ...
          <div className="lg:col-span-2 min-w-0">
            <FormSection title="Entity Details" columns={2}>
              ...
              <PxFormField label="Legal Name" required filled={!!legalName.trim()}>
                <input
                  ...
                  className="px-input"
                  disabled={isApproverReview}
                />
              </PxFormField>
              <PxFormField label="Region" required filled={!!region}>
                <select
                  value={region}
                  onChange={(e) => setRegion(e.target.value as 'GCC' | 'India' | 'South East Asia')}
                  className="px-select"
                  disabled={isApproverReview}
                >
                  ...
                  >
                    ...
                    {uniqueCountries.map((c: string) => (
                      <option key={c} value={c}>
                        {countryFlags[c] ? `${countryFlags[c]} ${c}` : c}
                      </option>
                    ))}
                    {country && !uniqueCountries.includes(country) && (
                      <option value={country}>
                        {countryFlags[country] ? `${countryFlags[country]} ${country}` : country}
                      </option>
                    )}
            ...
            <FormSection title="Tax & Compliance" columns={2}>
              <PxFormField
                ...
                filled={!!taxRegistrationNumber.trim()}
                hint="GST/VAT registration number"
              >
                <input
                  type="text"
                ...
                <div className="mt-5 pt-5" style={{ borderTop: '1px solid var(--color-silver)' }}>
                  ...
                  <div className="flex gap-3">
                    ...
                    <button
                      ...
                      style={{
                        backgroundColor: approvalActionBusy
                          ? 'var(--color-silver)'
                          : 'var(--color-teal)',
                        fontWeight: 800,
                      }}
  ...
  return (
    <MasterPageShell
      masterName="Entity Master"
      description="Manage legal entities, companies, and branches"
    >
                ...
                >
                  ...
                  <td className="px-6 py-4">
                    <span
                      ...
                      style={getStatusBadgeStyle(entity.isActive)}
                    >
                  ...
                  <td className="px-6 py-4">
                    <span
                      className="px-3 py-1 rounded-full text-sm"
                      style={getApprovalBadgeStyle(entity.approvalStatus)}
                    >
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-2">
                      ...
                      {(entity.approvalStatus === 'Pending Approval' ||
                        entity.approvalStatus === 'Changes Requested' ||
      ...
      >
        <p className="text-sm" style={{ color: '#D97706' }}>
          ℹ️ Entity Master is populated from the canonical entity registry. Showing{' '}
