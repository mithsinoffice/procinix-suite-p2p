...
export function PermissionRBACDemo() {
  ...
  const permissionCategories = {
    'AP Automation': currentRole.permissions.filter(p => 
      p.startsWith('AP_INVOICE') || p.startsWith('PAYMENT_RUN') || 
      p.startsWith('PURCHASE_ORDER') || p.startsWith('GRN') || 
      p.startsWith('VENDOR') || p.startsWith('CASHFLOW')
    ),
    'AR Automation': currentRole.permissions.filter(p => 
      p.startsWith('AR_INVOICE') || p.startsWith('CUSTOMER') || 
      p.startsWith('COLLECTION') || p.startsWith('CREDIT_MGMT')
    ),
    'R2R Automation': currentRole.permissions.filter(p => 
      p.startsWith('GENERAL_LEDGER') || p.startsWith('RECONCILIATION') || 
      p.startsWith('FINANCIAL_CLOSE') || p.startsWith('CONSOLIDATION')
    ),
    'Masters & Reports': currentRole.permissions.filter(p => 
      p.startsWith('MASTERS') || p.startsWith('REPORTS')
    ),
    'Global Functions': currentRole.permissions.filter(p => 
      ...
      p.startsWith('SETTINGS') || p.startsWith('AI_CAPTURE')
    )
  };
  ...
  const getPermissionIcon = (permission: string) => {
    ...
    if (permission.includes('.CREATE') || permission.includes('.EDIT') || 
        ...
        permission.includes('.EXECUTE') || permission.includes('.POST')) return Edit3;
  ...
  return (
    <div style={{ backgroundColor: 'var(--color-cloud)', minHeight: '100vh' }}>
      <div className="p-8 max-w-7xl mx-auto">
        ...
        {/* Permission Key */}
        <div className="mt-8 p-6 rounded-lg" style={{ backgroundColor: '#DBEAFE', border: '1px solid #3B82F6' }}>
          <h3 className="text-sm mb-3" style={{ color: '#1E3A8A', fontWeight: '600' }}>
            📋 Permission Types Explained
          </h3>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="flex items-center gap-2">
              <div className="w-6 h-6 rounded flex items-center justify-center" style={{ backgroundColor: '#10B98120' }}>
                <Edit3 className="w-3.5 h-3.5" style={{ color: '#10B981' }} />
              </div>
              <div>
                <div className="text-xs" style={{ color: '#1E3A8A', fontWeight: '600' }}>Edit Permissions</div>
                <div className="text-xs" style={{ color: '#1E3A8A' }}>CREATE, EDIT, DELETE, MANAGE</div>
              </div>
            </div>
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <div className="text-xs" style={{ color: '#1E3A8A' }}>APPROVE, REJECT</div>
              </div>
            </div>
            <div className="flex items-center gap-2">
              <div className="w-6 h-6 rounded flex items-center justify-center" style={{ backgroundColor: '#F59E0B20' }}>
                <Eye className="w-3.5 h-3.5" style={{ color: '#F59E0B' }} />
              </div>
              <div>
                <div className="text-xs" style={{ color: '#1E3A8A', fontWeight: '600' }}>View Permissions</div>
                <div className="text-xs" style={{ color: '#1E3A8A' }}>VIEW, READ</div>
              </div>
            </div>
          </div>
          <div className="mt-4 pt-4" style={{ borderTop: '1px solid #3B82F6' }}>
            <p className="text-xs" style={{ color: '#1E3A8A' }}>
              <strong>💡 Tip:</strong> Switch roles above and watch the left navigation automatically update. 
              Items you can only view show an eye icon (👁️). Items without any permission are completely hidden from the navigation.
            </p>
          </div>
        </div>
      </div>
