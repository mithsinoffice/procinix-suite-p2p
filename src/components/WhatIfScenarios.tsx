    ...
    <div className="min-h-screen bg-[#F6F9FC]">
      ...
      <div className="bg-white border-b border-[#E1E6EA]">
        <div className="px-6 py-4">
          <div>
            <h1 className="text-[#0A0F14]">What-If & Scenario Analysis</h1>
            <p className="text-[#6E7A82] text-sm">Model budget scenarios with adjustable parameters and impact analysis</p>
          </div>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-1 space-y-6">
            ...
            <div className="bg-white rounded-lg border border-[#E1E6EA] p-6">
              <h2 className="text-[#0A0F14] mb-4">Current Budget State</h2>
              <div className="space-y-3">
                <div>
                  <p className="text-xs text-[#6E7A82] mb-1">Total Budget</p>
                  <p className="text-xl text-[#0A0F14]">₹{(totalBudget / 1000000).toFixed(2)}M</p>
                </div>
                <div>
                  <p className="text-xs text-[#6E7A82] mb-1">Committed</p>
                  <p className="text-lg text-blue-600">₹{(totalCommitted / 1000000).toFixed(2)}M</p>
                </div>
                <div>
                  <p className="text-xs text-[#6E7A82] mb-1">Actual</p>
                  <p className="text-lg text-purple-600">₹{(totalActual / 1000000).toFixed(2)}M</p>
                </div>
                <div>
                  <p className="text-xs text-[#6E7A82] mb-1">Available</p>
            ...
            {/* Scenario Builder */}
            <div className="bg-white rounded-lg border border-[#E1E6EA] p-6">
              <h2 className="text-[#0A0F14] mb-4">Scenario Builder</h2>

              <div className="space-y-4">
                <div>
                  <label className="block text-sm text-[#0A0F14] mb-2">Scenario Name</label>
                  <input
                    type="text"
                    value={scenarioName}
                ...
                <div>
                  <label className="block text-sm text-[#0A0F14] mb-2">Scenario Type</label>
                  <select
                    value={scenarioType}
                ...
                <div>
                  <label className="block text-sm text-[#0A0F14] mb-2">Time Horizon</label>
                  <select
                    value={timeHorizon}
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full"
                  />
                  <div className="flex justify-between text-xs text-[#6E7A82] mt-1">
                    <span>-50%</span>
                    <span>0%</span>
                    <span>+50%</span>
                  </div>
                ...
                {/* Quick Presets */}
                <div>
                  <p className="text-sm text-[#0A0F14] mb-2">Quick Presets</p>
                ...
                <button
                  ...
                  className="w-full px-4 py-2 bg-[#00A9B7] text-white rounded-lg hover:bg-[#007D87] transition-colors flex items-center justify-center gap-2"
                >
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="grid grid-cols-4 gap-4">
              <div className="bg-white rounded-lg border border-[#E1E6EA] p-4">
                <p className="text-xs text-[#6E7A82] mb-1">Projected Budget</p>
                <p className="text-2xl text-[#0A0F14]">₹{(projectedBudget / 1000000).toFixed(2)}M</p>
              ...
              <div className="bg-white rounded-lg border border-[#E1E6EA] p-4">
                <p className="text-xs text-[#6E7A82] mb-1">Projected Committed</p>
              ...
              <div className="bg-white rounded-lg border border-[#E1E6EA] p-4">
                <p className="text-xs text-[#6E7A82] mb-1">Projected Available</p>
              ...
              <div className="bg-white rounded-lg border border-[#E1E6EA] p-4">
                <p className="text-xs text-[#6E7A82] mb-1">Breach Risk</p>
                ...
                <p className="text-xs text-[#6E7A82] mt-1">{utilizationPercent.toFixed(1)}% utilized</p>
              </div>
            </div>

            {/* Scenario Comparison Chart */}
            <div className="bg-white rounded-lg border border-[#E1E6EA] p-6">
              <h2 className="text-[#0A0F14] mb-4">Scenario Comparison - Current vs Projected</h2>
            ...
            {/* Budget Burn Projection */}
            <div className="bg-white rounded-lg border border-[#E1E6EA] p-6">
              <h2 className="text-[#0A0F14] mb-4">Budget Burn Projection - Cumulative %</h2>
              <ResponsiveContainer width="100%" height={300}>
                <LineChart data={burnProjection}>
                  <CartesianGrid strokeDasharray="3 3" stroke="#E1E6EA" />
                  <XAxis dataKey="month" stroke="#6E7A82" style={{ fontSize: '12px' }} />
                  <YAxis stroke="#6E7A82" style={{ fontSize: '12px' }} domain={[0, 100]} label={{ value: 'Utilization %', angle: -90, position: 'insideLeft', style: { fontSize: '12px', fill: '#6E7A82' } }} />
                  <Tooltip 
                    contentStyle={{ backgroundColor: '#fff', border: '1px solid #E1E6EA', borderRadius: '8px' }}
                    formatter={(value: number) => `${value.toFixed(1)}%`}
                  />
                  <Legend wrapperStyle={{ fontSize: '12px' }} />
                  <Line type="monotone" dataKey="current" stroke="#00A9B7" strokeWidth={2} dot={{ fill: '#00A9B7', r: 4 }} name="Current Burn" />
            ...
            <div className="bg-white rounded-lg border border-[#E1E6EA]">
              <div className="px-6 py-4 border-b border-[#E1E6EA]">
                <h2 className="text-[#0A0F14]">Saved Scenarios</h2>
              </div>
              <div className="divide-y divide-[#E1E6EA]">
                {scenarios.map(scenario => (
                  <div key={scenario.id} className="px-6 py-4 hover:bg-[#F6F9FC]">
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        <div className="flex items-center gap-3 mb-2">
                          <h3 className="text-[#0A0F14]">{scenario.scenarioName}</h3>
                        ...
                        <div className="flex items-center gap-6 text-sm text-[#6E7A82]">
                      ...
                      <div className="flex gap-2">
                        <button className="p-2 hover:bg-white rounded transition-colors">
                          <Copy className="w-4 h-4 text-[#6E7A82]" />
                        </button>
                        <button className="p-2 hover:bg-white rounded transition-colors">
