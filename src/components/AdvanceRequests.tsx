...
import { 
  ...
  FileText, ArrowUpRight, WalletCards
} from 'lucide-react';
...
export function AdvanceRequests() {
  ...
  const filteredRequests = useMemo(() => advanceRequests.filter(request => {
    const matchesSearch = 
    ...
    const matchesPayment = paymentFilter.length === 0 || paymentFilter.includes(request.paymentStatus);
    ...
    return matchesSearch && matchesStatus && matchesType && matchesPayment && matchesPriority && matchesDateRange;
  }), [advanceRequests, filterDateRange.from, filterDateRange.to, paymentFilter, priorityFilter, searchTerm, statusFilter, typeFilter]);
            ...
            <div style={{ minWidth: '1500px' }}>
              <div className="grid gap-4 px-6 py-4" style={{ gridTemplateColumns: '1.7fr 1.7fr 1fr 1.4fr 1fr 1fr 1fr 1fr 1fr 0.9fr', borderBottom: '1px solid #E8F0F4' }}>
                <div className="space-y-2">
                  ...
                  {hasActiveFilters && (
                    <button type="button" onClick={() => { setSearchTerm(''); setStatusFilter([]); setTypeFilter([]); setPaymentFilter([]); setPriorityFilter([]); setFilterDateRange({ from: '', to: '' }); }} className="flex items-center gap-2 px-3 py-2 rounded-xl text-sm" style={{ backgroundColor: '#FFF5F5', border: '1px solid #FED7D7', color: '#C53030', fontWeight: 600 }}>
                      Clear Filters
                    </button>
                  )}
                </div>
                ...
                <div className="flex items-start">
                  <PremiumFilterMenu label="Type" options={['PO-based', 'On-Account']} selected={typeFilter} onToggle={(value) => setTypeFilter((current) => toggleMultiSelect(current, value))} />
                </div>
                <div />
                <div />
                <div />
                <div className="flex items-start">
                  <PremiumFilterMenu label="Status" options={['Draft', 'Submitted', 'Approved', 'Rejected', 'Cancelled']} selected={statusFilter} onToggle={(value) => setStatusFilter((current) => toggleMultiSelect(current, value))} />
                </div>
                <div className="flex items-start">
                  <PremiumFilterMenu label="Payment" options={['Pending', 'In Queue', 'Processed', 'Failed']} selected={paymentFilter} onToggle={(value) => setPaymentFilter((current) => toggleMultiSelect(current, value))} />
                </div>
                <div className="flex items-start">
                  <PremiumFilterMenu label="Priority" options={['Low', 'Medium', 'High', 'Critical']} selected={priorityFilter} onToggle={(value) => setPriorityFilter((current) => toggleMultiSelect(current, value))} />
                </div>
                <div />
              </div>
                ...
                {filteredRequests.length > 0 ? (
                  filteredRequests.map(request => (
                    <div key={request.id} className="grid gap-4 px-6 py-4 hover:bg-[#F8FCFE]" style={{ gridTemplateColumns: '1.7fr 1.7fr 1fr 1.4fr 1fr 1fr 1fr 1fr 1fr 0.9fr', borderBottom: '1px solid #EDF3F7' }}>
                      ...
                      <div>
                        <span className={`px-2 py-1 rounded text-xs ${getPaymentStatusBadge(request.paymentStatus)}`}>
                          {request.paymentStatus}
                        </span>
                      </div>
                      <div>
                        <span className={`px-2 py-1 rounded text-xs ${getPriorityBadge(request.priority)}`}>
                          {request.priority}
                        </span>
                      </div>
                      <div className="flex items-center justify-end gap-2">
                          <PremiumActionButton label="View request" icon={<Eye className="w-4 h-4" />} tone="teal" onClick={() => alert(`View details for ${request.requestNumber}`)} />
                          {request.status === 'Draft' && (
                            <PremiumActionButton label="Edit request" icon={<Edit className="w-4 h-4" />} tone="violet" onClick={() => navigate(`/ap/advance-request-form?edit=${request.id}`)} />
                          )}
                          <PremiumActionButton label="Open request" icon={<ArrowUpRight className="w-4 h-4" />} tone="blue" onClick={() => alert(`Open ${request.requestNumber}`)} />
                      </div>
                    </div>
                  ))
                ) : (
