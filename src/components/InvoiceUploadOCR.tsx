...
import { 
  ...
  ZoomIn, ZoomOut, RotateCw, Download, Eye, X, AlertTriangle,
  Info, Shield, TrendingUp, DollarSign, Package, Calendar,
  User, Building2, Hash, FileCheck, ChevronRight, ChevronLeft,
...
export function InvoiceUploadOCR() {
  ...
  const generateMockOCRData = (): OCRData => {
    ...
    return {
      ...
      lineItems: [
        {
          id: '1',
          description: { value: 'Software Licenses - Annual Subscription', confidence: 'High', isEdited: false },
  ...
  const generateOCRInsights = (data: OCRData) => {
    ...
    Object.entries(data).forEach(([key, value]) => {
      if (value && typeof value === 'object' && 'confidence' in value && value.confidence === 'Low') {
    ...
    if (data.vendorGSTIN.value && data.vendorGSTIN.value.length === 15) {
      insights.push({
        ...
        explanation: 'GSTIN format validated. State code: ' + data.vendorGSTIN.value.substring(0, 2),
        confidence: 95,
        impact: 'low',
    ...
    if (data.poNumber.value) {
      insights.push({
        ...
        explanation: 'System will attempt to auto-match this invoice to the referenced PO and validate against GRN.',
  ...
  const performValidation = () => {
    ...
    issues.push({
      ...
      description: 'Invoice INV-2025-00123 from this vendor has similar characteristics to invoice submitted on Jan 8, 2025.',
    ...
    issues.push({
      ...
      description: 'This is an MSME registered vendor. Payment should be processed within 45 days as per MSMED Act.',
      action: 'view-msme',
    ...
    <div className="max-w-4xl mx-auto">
      <div className="bg-white rounded-lg border border-gray-200 p-8">
        <div className="text-center mb-6">
          ...
          <p className="text-gray-600">Upload a PDF or image of the supplier invoice for automatic data extraction</p>
        </div>
        ...
        {!uploadedFile ? (
          ...
          >
            ...
            <p className="text-gray-700 mb-2">Drag and drop your invoice here, or click to browse</p>
      ...
      <div className="flex gap-6 h-[calc(100vh-250px)]">
        ...
        <div className="w-1/2 bg-white rounded-lg border border-gray-200 overflow-auto">
          <div className="p-6 border-b border-gray-200">
            ...
            <p className="text-sm text-gray-600">Review and edit extracted data. Fields with low confidence are highlighted.</p>
          </div>
          ...
          <div className="p-6 space-y-6">
            ...
            {/* Line Items */}
            <div>
              <h4 className="text-sm text-gray-500 mb-4">Line Items ({ocrData.lineItems.length})</h4>
              <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div className="col-span-2">
                        <ExtractedFieldInput
                          ...
                          onUpdate={(value) => updateExtractedField(`lineItems.${index}.description`, value)}
                        />
                      </div>
                      <ExtractedFieldInput
                        label="HSN/SAC"
                        field={item.hsnSac}
                        onUpdate={(value) => updateExtractedField(`lineItems.${index}.hsnSac`, value)}
                      />
                      <ExtractedFieldInput
                        label="Quantity"
                        field={item.qty}
                      ...
                      <ExtractedFieldInput
                        label="Amount"
                        field={item.amount}
                        onUpdate={(value) => updateExtractedField(`lineItems.${index}.amount`, value)}
                        type="number"
                      />
                      <ExtractedFieldInput
                        label="GST Rate (%)"
                        field={item.gstRate}
                        onUpdate={(value) => updateExtractedField(`lineItems.${index}.gstRate`, value)}
                        type="number"
                      />
                      <ExtractedFieldInput
                        label="GST Amount"
                        field={item.gstAmount}
                        onUpdate={(value) => updateExtractedField(`lineItems.${index}.gstAmount`, value)}
                        type="number"
                      />
                    </div>
                  </div>
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            {vendorMatches.length > 0 && (
              <div className="flex items-center space-x-2">
                ...
                <span className="text-sm text-green-600">{vendorMatches[0].matchConfidence}% Match</span>
              </div>
            )}
          </div>
              ...
              {vendorMatches.map((match) => (
                <label
                  ...
                  className={`flex items-start p-3 border rounded cursor-pointer hover:bg-gray-50 ${
                    selectedVendorMatch === match.vendorCode ? 'border-teal-500 bg-teal-50' : 'border-gray-200'
                  }`}
                >
                  <input
                    type="radio"
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            {poMatches.length > 0 && (
              <div className="flex items-center space-x-2">
                <div className="w-2 h-2 bg-green-500 rounded-full"></div>
                <span className="text-sm text-green-600">{poMatches[0].matchConfidence}% Match</span>
              </div>
            )}
          </div>
            ...
            <div className="space-y-3">
              {poMatches.map((match) => (
                <label
                  key={match.poNumber}
                  className={`flex items-start p-3 border rounded cursor-pointer hover:bg-gray-50 ${
                    selectedPOMatch === match.poNumber ? 'border-teal-500 bg-teal-50' : 'border-gray-200'
                  }`}
                >
                  <input
                    type="radio"
                  ...
                  <div className="ml-3 flex-1">
                    ...
                    <div className="flex items-center justify-between mt-1">
                      ...
                      <p className="text-xs text-gray-400">Open: ₹{match.openAmount.toLocaleString('en-IN')}</p>
                    </div>
                  </div>
                </label>
              ))}
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            {grnMatches.length > 0 && (
              <div className="flex items-center space-x-2">
                <div className="w-2 h-2 bg-green-500 rounded-full"></div>
                <span className="text-sm text-green-600">{grnMatches[0].matchConfidence}% Match</span>
              </div>
            )}
          </div>
            ...
            <div className="space-y-3">
              {grnMatches.map((match) => (
                <label
                  key={match.grnNumber}
                  className={`flex items-start p-3 border rounded cursor-pointer hover:bg-gray-50 ${
                    selectedGRNMatch === match.grnNumber ? 'border-teal-500 bg-teal-50' : 'border-gray-200'
                  }`}
                >
                  <input
                    type="radio"
        ...
        <div className="p-6">
          <div className="space-y-4">
            {validationIssues.map((issue, index) => {
              ...
              return (
                <div key={index} className={`p-4 border rounded-lg ${config.bgColor} ${config.borderColor}`}>
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              ...
              <div>
                ...
                <p className="text-sm text-gray-600">Upload invoice → Extract data → Validate → Complete</p>
              </div>
            </div>
            <div className="flex items-center space-x-2">
...
function ExtractedFieldInput({ label, field, onUpdate, icon, type = 'text' }: ExtractedFieldInputProps) {
