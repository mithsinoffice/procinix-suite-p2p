...
export function OCRBanner({
  ...
  const invoiceNumber =
    scores.fields?.invoice_number?.value || scores.fields?.invoice_number?.ocr_raw || 'Invoice';
  const vendorName =
    scores.fields?.vendor_name?.value || scores.fields?.vendor_name?.ocr_raw || 'Unknown vendor';
  return (
    <div className="ocr-banner">
      <div className="ocr-banner-icon">
        <Sparkles size={16} />
      </div>
      <div className="ocr-banner-content">
        <div className="ocr-banner-title">
          Gemini AI OCR — invoice processed · {pct}% overall confidence
        </div>
        <div className="ocr-banner-subtitle">
      ...
      <div className="ocr-banner-actions">
        ...
        <button type="button" className="btn-secondary" onClick={onReviewConflicts}>
          Review conflicts →
        </button>
      </div>
    </div>
