...
function toCorrection(
  fieldName: string,
  ocrScore: OCRFieldScore | undefined,
  value: string,
  ...
  return (
    <div className={`ocr-field-wrapper ${statusClass}`}>
      <div className="ocr-field-label-row">
        ...
        {ocrScore ? (
          <MatchScoreBadge confidence={ocrScore.confidence} status={ocrScore.match_status} />
        ) : null}
      </div>
      {children}
      {ocrScore?.match_status === 'matched' && (
        <div className="ocr-field-help">
          ✓ OCR extracted: "{ocrScore.value || ocrScore.ocr_raw || ''}"
        </div>
      )}
      {!collapsed &&
        (ocrScore?.match_status === 'conflict' || ocrScore?.match_status === 'low_confidence') && (
          <div className="ocr-conflict-panel">
            <div className="ocr-conflict-title">
              {ocrScore.match_status === 'conflict'
                ? '⚠ OCR extracted:'
                : '✗ Low confidence — OCR extracted:'}{' '}
              {ocrScore.ocr_raw || '—'}
            </div>
            ...
            <div className="ocr-action-row">
              <button
                ...
                onClick={() =>
                  selectedCandidate &&
                  applyAndRecord(selectedCandidate.value, 'custom', 'Use selected OCR candidate')
                }
              ...
              <button
                ...
                onClick={() =>
                  selectedCandidate &&
                  applyAndRecord(selectedCandidate.value, 'custom', 'Teach AI this mapping')
                }
                disabled={!selectedCandidate}
              >
                ✦ Teach AI this mapping
              </button>
              <button type="button" className="btn-secondary" onClick={() => setCollapsed(true)}>
                Skip
              </button>
            </div>
          </div>
