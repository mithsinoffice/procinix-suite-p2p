...
export function OCRScoreCards({ scores }: { scores: OCRInvoiceScores }) {
  ...
  const confidenceClass = confidence > 80 ? 'ocr-score-good' : confidence >= 50 ? 'ocr-score-mid' : 'ocr-score-low';
  ...
  return (
    <div className="ocr-score-cards">
      ...
      <div className="ocr-score-card">
        ...
        <div className="ocr-score-value">{scores.fields_conflicted + scores.fields_low_confidence}</div>
        <div className="ocr-score-subtext">{reviewFields.length ? reviewFields.join(', ') : 'No fields'}</div>
      </div>
      ...
      <div className="ocr-score-card">
        ...
        <div className="ocr-score-subtext">{scores.touchless_eligible ? 'No conflicts detected' : 'Blocked by unresolved fields'}</div>
      </div>
    </div>
  );
}
