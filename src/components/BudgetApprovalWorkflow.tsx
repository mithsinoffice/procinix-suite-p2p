    ...
    <div className="min-h-screen bg-[#F6F9FC]">
      ...
      <div className="bg-white border-b border-[#E1E6EA]">
        <div className="px-6 py-4">
          <div>
            <h1 className="text-[#0A0F14]">Budget Approval Workflow</h1>
            <p className="text-[#6E7A82] text-sm">Multi-level approval tracking with SLA monitoring</p>
          </div>
        </div>
      </div>
      ...
      <div className="p-6">
        ...
        <div className="grid grid-cols-4 gap-4 mb-6">
          ...
          <div className="bg-white rounded-lg border border-[#E1E6EA] p-4">
            <div className="flex items-center justify-between mb-2">
              <span className="text-sm text-[#6E7A82]">Rejected</span>
              <XCircle className="w-4 h-4 text-red-500" />
            </div>
            <div className="text-2xl text-[#0A0F14]">{stats.rejected}</div>
          </div>
          
          <div className="bg-white rounded-lg border border-[#E1E6EA] p-4">
            <div className="flex items-center justify-between mb-2">
              <span className="text-sm text-[#6E7A82]">Overdue SLA</span>
              <AlertCircle className="w-4 h-4 text-red-500" />
            </div>
            <div className="text-2xl text-[#0A0F14]">{stats.overdue}</div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 gap-4 mb-6">
          {approvalBudgets.map(budget => (
            <div key={budget.id} className="bg-white rounded-lg border border-[#E1E6EA] overflow-hidden">
              {/* Budget Header */}
              <div className="px-6 py-4 border-b border-[#E1E6EA] bg-[#F6F9FC]">
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-2">
                      <h3 className="text-[#0A0F14]">{budget.budgetName}</h3>
                  ...
                  <button
                    ...
                    className="px-3 py-2 border border-[#E1E6EA] rounded-lg hover:bg-white transition-colors flex items-center gap-2"
                  >
              ...
              {selectedBudget === budget.id && (
                <div className="px-6 py-4">
                  <div className="relative">
                    {/* Workflow Line */}
                    <div className="absolute left-6 top-8 bottom-8 w-0.5 bg-[#E1E6EA]"></div>
                    ...
                    <div className="space-y-6">
                      {budget.approvalWorkflow.map((step, index) => (
                        <div key={index} className="relative flex gap-4">
                          ...
                          {/* Step Content */}
                          <div className="flex-1 bg-[#F6F9FC] rounded-lg p-4">
                            <div className="flex items-start justify-between mb-3">
                              <div>
                                <div className="flex items-center gap-2 mb-1">
                                  <h4 className="text-[#0A0F14]">Level {step.level}: {step.approverRole}</h4>
                                  {step.overdue && (
                                ...
                                <div className="flex items-center gap-2 text-sm text-[#6E7A82]">
                            ...
                            <div className="grid grid-cols-3 gap-4 text-sm">
                              <div>
                                <p className="text-[#6E7A82] mb-1">SLA Due</p>
                                <div className="flex items-center gap-2">
                                  <Calendar className="w-4 h-4 text-[#6E7A82]" />
                                  <span className={step.overdue ? 'text-red-600' : 'text-[#0A0F14]'}>
                                    {step.slaDue}
                                  </span>
                                </div>
                              </div>
                              <div>
                                <p className="text-[#6E7A82] mb-1">SLA Hours</p>
                                <span className="text-[#0A0F14]">{step.slaHours}h</span>
                              </div>
                              <div>
                                <p className="text-[#6E7A82] mb-1">Action Date</p>
                                <span className="text-[#0A0F14]">{step.actionDate || 'Pending'}</span>
                              </div>
                            </div>

                            {step.comments && (
                              <div className="mt-3 pt-3 border-t border-[#E1E6EA]">
                                <div className="flex items-start gap-2">
                                  <MessageSquare className="w-4 h-4 text-[#6E7A82] mt-0.5" />
                                  <div>
                                    <p className="text-xs text-[#6E7A82] mb-1">Comments</p>
                                    <p className="text-sm text-[#0A0F14]">{step.comments}</p>
                                  </div>
                                </div>
                              </div>
                            )}
                            ...
                            {step.status === 'Pending' && (
                              <div className="mt-3 pt-3 border-t border-[#E1E6EA] flex gap-2">
                                ...
                                <button className="px-3 py-1.5 border border-[#E1E6EA] rounded text-sm hover:bg-white transition-colors flex items-center gap-2">
        ...
        {approvalBudgets.length === 0 && (
          <div className="bg-white rounded-lg border border-[#E1E6EA] p-12 text-center">
            ...
            <p className="text-[#6E7A82] mb-1">No budgets pending approval</p>
