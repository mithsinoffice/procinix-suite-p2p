...
export function WorkflowManagement() {
  ...
  const filteredWorkflows = workflows.filter((workflow) => {
    ...
    const matchesCategory =
      categoryFilter.length === 0 || categoryFilter.includes(workflow.workflowCategory ?? 'Forms');
    const matchesStatus = statusFilter.length === 0 || statusFilter.includes(workflow.status);
    return matchesSearch && matchesCategory && matchesStatus;
  });
  const hasActiveFilters =
    searchTerm.trim().length > 0 || categoryFilter.length > 0 || statusFilter.length > 0;
      ...
      >
        <div className="flex items-center justify-between gap-4 flex-wrap">
          ...
          <div style={{ flex: 1, minWidth: '240px' }}>
            ...
            <h2 className="text-lg" style={{ color: 'var(--color-ink)', margin: 0 }}>
            ...
            >
              ...
              <div className="flex items-start">
                <PremiumFilterMenu
                  ...
                  onToggle={(value) =>
                    setCategoryFilter((current) => toggleMultiSelect(current, value))
                  }
                />
              </div>
              <div />
