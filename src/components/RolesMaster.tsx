...
export function RolesMaster() {
  ...
  const filteredRoles = useMemo(() => {
    return roles.filter((role) => {
      ...
      const matchesApproval =
        approvalFilter.length === 0 || approvalFilter.includes(role.approvalStatus ?? 'Approved');
      return matchesSearch && matchesStatus && matchesApproval;
    });
  }, [roles, searchTerm, statusFilter, approvalFilter]);

  const getAssignedUserCount = (role: Role) =>
    users.filter((user) => {
      ...
      setRoles(
        roles.map((r) =>
          r.id === selectedRole.id
            ? { ...r, ...formData, approvalStatus: 'Pending', entityMappings }
            : r
        )
      );
      setShowCreateModal(false);
  ...
  const handleReview = (role: Role) => {
    ...
    if (role.originalData) {
      ...
      if (original.roleCode !== role.roleCode)
        changes.push({ field: 'Role Code', oldValue: original.roleCode, newValue: role.roleCode });
      if (original.roleName !== role.roleName)
        changes.push({ field: 'Role Name', oldValue: original.roleName, newValue: role.roleName });
      if (original.description !== role.description)
        changes.push({
          field: 'Description',
          oldValue: original.description,
          newValue: role.description,
        });
      if (original.status !== role.status)
        changes.push({ field: 'Status', oldValue: original.status, newValue: role.status });
      if (JSON.stringify(original.permissions) !== JSON.stringify(role.permissions))
        changes.push({
          field: 'Permissions',
          oldValue: original.permissions.join(', '),
      ...
      >
        <FormSection title="Role Details" columns={2}>
          <PxFormField
            label="Role Code"
            required
            filled={!!formData.roleCode.trim()}
            hint="Unique role identifier"
          >
            <input
              type="text"
          ...
          <PxFormField
            label="Permissions"
            required
            filled={formData.permissions.length > 0}
            hint={`${formData.permissions.length} selected`}
          >
            <div
            ...
            >
              {availablePermissions.map((permission) => (
                <label
                  ...
                  style={{
                    backgroundColor: formData.permissions.includes(permission)
                      ? 'var(--color-teal-tint)'
                      : 'transparent',
                    border: formData.permissions.includes(permission)
                ...
                >
                  ...
                  />
                  <span style={{ fontSize: '13px', color: 'var(--color-ink)' }}>{permission}</span>
                </label>
              ))}
            </div>
          </PxFormField>
            ...
            <tbody>
              {filteredRoles.map((role) => (
                <tr key={role.id} style={{ borderBottom: '1px solid var(--color-silver)' }}>
                  ...
                  <td style={{ padding: '16px' }}>{getStatusBadge(role.status)}</td>
                  <td style={{ padding: '16px' }}>
                    <div className="flex items-center gap-2">
