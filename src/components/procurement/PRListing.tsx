...
import { FileText, Search, Eye, Edit, Copy, Clock, Download, RefreshCw, Package, ArrowUpRight, ClipboardList } from 'lucide-react';
...
export function PRListing() {
  ...
  // Apply filters
  const filteredPRs = useMemo(() => currentPRs.filter(pr => {
    ...
    const matchesRisk = selectedRiskLevels.length === 0 || selectedRiskLevels.includes(pr.aiRiskLevel);
    
    const matchesAmount = (!amountRange.min || pr.totalAmount >= parseFloat(amountRange.min) * 100000) &&
                         (!amountRange.max || pr.totalAmount <= parseFloat(amountRange.max) * 100000);
    
    return matchesSearch && matchesPRType && matchesStatus && matchesEntity && matchesRisk && matchesAmount;
  }), [amountRange.max, amountRange.min, currentPRs, searchTerm, selectedEntities, selectedPRTypes, selectedRiskLevels, selectedStatuses]);
    ...
    <div style={{ backgroundColor: 'var(--color-cloud)', minHeight: '100vh' }}>
      ...
      <div className="bg-white px-8 py-6" style={{ borderBottom: '1px solid var(--color-silver)' }}>
        <div className="flex items-center justify-between mb-4">
          <div>
            <h1 className="text-2xl mb-2" style={{ color: 'var(--color-ink)', margin: 0 }}>Purchase Requisitions</h1>
            <p className="text-sm" style={{ color: 'var(--color-mercury-grey)', margin: 0 }}>
              {selectedTab === 'my-prs' ? 'View and manage your PR requests' : 'View team PR requests'}
            </p>
          </div>
          <div className="flex items-center gap-3">
            {filteredPRs.filter(pr => pr.status === 'Approved' && !pr.linkedPO).length > 0 && (
              <button 
                ...
                style={{ backgroundColor: '#FFFFFF', borderColor: 'var(--color-teal)', color: 'var(--color-teal)' }}
                onClick={handleBulkConvertToPO}
              >
                <Package className="w-4 h-4 inline mr-2" />
                Convert {filteredPRs.filter(pr => pr.status === 'Approved' && !pr.linkedPO).length} PRs to PO
              </button>
            )}
            <button 
      ...
      <div className="p-8">
        ...
        <div className="bg-white rounded-lg mb-6" style={{ border: '1px solid var(--color-silver)' }}>
          <div className="flex items-center gap-4 p-4" style={{ borderBottom: '1px solid var(--color-silver)' }}>
            <button
              onClick={() => setSelectedTab('my-prs')}
              className="px-4 py-2 rounded-lg text-sm transition-colors"
              style={{
                backgroundColor: selectedTab === 'my-prs' ? 'var(--color-teal-tint)' : 'transparent',
                color: selectedTab === 'my-prs' ? 'var(--color-teal)' : 'var(--color-mercury-grey)',
                border: selectedTab === 'my-prs' ? '1px solid var(--color-teal)' : '1px solid transparent',
            ...
            <button
              ...
              className="px-4 py-2 rounded-lg text-sm transition-colors"
              style={{
                backgroundColor: selectedTab === 'team-prs' ? 'var(--color-teal-tint)' : 'transparent',
                color: selectedTab === 'team-prs' ? 'var(--color-teal)' : 'var(--color-mercury-grey)',
                border: selectedTab === 'team-prs' ? '1px solid var(--color-teal)' : '1px solid transparent',
        ...
        <div className="grid grid-cols-5 gap-6 mb-8">
          ...
          <div className="bg-white p-6 rounded-lg" style={{ border: '1px solid var(--color-silver)' }}>
            <p className="text-sm mb-2" style={{ color: 'var(--color-mercury-grey)' }}>Pending</p>
        ...
        <div className="rounded-[28px] overflow-hidden" style={{ backgroundColor: '#FFFFFF', border: '1px solid var(--color-fog)', boxShadow: '0 24px 52px rgba(15, 23, 42, 0.07)' }}>
          <div className="p-6" style={{ borderBottom: '1px solid var(--color-silver)' }}>
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className="w-12 h-12 rounded-2xl flex items-center justify-center" style={{ background: 'linear-gradient(135deg, #E7F4FF 0%, #D7EBFF 100%)', boxShadow: '0 14px 30px rgba(37, 99, 235, 0.12)' }}>
                ...
                <div>
                  ...
                  <h3 className="text-base" style={{ color: 'var(--color-ink)', margin: 0, fontWeight: '600' }}>
                    {selectedTab === 'my-prs' ? 'My Purchase Requisitions' : 'Team Purchase Requisitions'}
                  </h3>
                </div>
              </div>
          ...
          <div className="overflow-x-auto">
            <div style={{ minWidth: '1680px' }}>
              <div className="grid gap-4 px-6 py-4" style={{ gridTemplateColumns: '1.6fr 1fr 1.2fr 1.1fr 1fr 1fr 1fr 1.2fr 1fr 1fr 1fr', borderBottom: '1px solid #E8F0F4' }}>
                <div className="space-y-2">
                  <div className="relative w-full">
                    <Search className="w-4 h-4 absolute left-4 top-1/2 -translate-y-1/2" style={{ color: 'var(--color-mercury-grey)' }} />
                    <input value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} placeholder="Search PR..." className="w-full pl-11 pr-4 py-2.5 rounded-2xl text-sm" style={{ backgroundColor: '#F8FBFD', border: '1px solid var(--color-fog)', color: 'var(--color-ink)' }} />
                  </div>
                  {hasActiveFilters && (
                    <button type="button" onClick={() => { setSearchTerm(''); setSelectedPRTypes([]); setSelectedStatuses([]); setSelectedEntities([]); setSelectedRiskLevels([]); setAmountRange({ min: '', max: '' }); setDateRange({ from: '', to: '' }); }} className="flex items-center gap-2 px-3 py-2 rounded-xl text-sm" style={{ backgroundColor: '#FFF5F5', border: '1px solid #FED7D7', color: '#C53030', fontWeight: 600 }}>
                ...
                <div className="flex items-start">
                  <PremiumFilterMenu label="Type" options={['Catalogue', 'Regular', 'Service', 'Kit/Bundle', 'Asset/CAPEX', 'Blanket']} selected={selectedPRTypes} onToggle={(value) => setSelectedPRTypes((current) => toggleMultiSelect(current, value) as PRType[])} />
                </div>
                <div className="flex items-start">
                  <PremiumFilterMenu label="Entity" options={[...new Set(currentPRs.map((pr) => pr.entity))]} selected={selectedEntities} onToggle={(value) => setSelectedEntities((current) => toggleMultiSelect(current, value))} />
                </div>
                <div />
                <div />
                <div />
                <div />
                <div className="flex items-start">
                  <PremiumFilterMenu label="Status" options={['Draft', 'Pending Approval', 'In Review', 'Approved', 'Rejected', 'Converted to PO']} selected={selectedStatuses} onToggle={(value) => setSelectedStatuses((current) => toggleMultiSelect(current, value) as PRStatus[])} />
                </div>
                <div />
                <div className="flex items-start">
                  <PremiumFilterMenu label="AI Risk" options={['Low', 'Medium', 'High']} selected={selectedRiskLevels} onToggle={(value) => setSelectedRiskLevels((current) => toggleMultiSelect(current, value) as AIRiskLevel[])} />
                </div>
                <div />
              </div>

              <div className="grid gap-4 px-6 py-4" style={{ gridTemplateColumns: '1.6fr 1fr 1.2fr 1.1fr 1fr 1fr 1fr 1.2fr 1fr 1fr 1fr', background: 'linear-gradient(180deg, #F8FBFD 0%, #F3F8FB 100%)', borderBottom: '1px solid #E4EDF2' }}>
                {['PR', 'Type', 'Entity', 'Requestor', 'Department', 'Need By', 'Amount', 'Status', 'Approver', 'AI Risk', 'Action'].map((column) => (
                ...
                {filteredPRs.map((pr) => {
                  ...
                  return (
                    <div key={pr.id} className="grid gap-4 px-6 py-4 hover:bg-[#F8FCFE]" style={{ gridTemplateColumns: '1.6fr 1fr 1.2fr 1.1fr 1fr 1fr 1fr 1.2fr 1fr 1fr 1fr', borderBottom: '1px solid #EDF3F7' }}>
                      <div>
                        <div>
                          <p className="text-sm mb-1" style={{ color: 'var(--color-ink)', fontWeight: '600', margin: 0 }}>{pr.id}</p>
                          {pr.linkedPO && (
                            <p className="text-xs" style={{ color: 'var(--color-teal)', margin: 0, cursor: 'pointer' }}>→ {pr.linkedPO}</p>
                          )}
                        </div>
                      </div>
                      <div>
                      ...
                      <div className="text-sm" style={{ color: 'var(--color-ink)' }}>{pr.entity}</div>
                      <div className="text-sm" style={{ color: 'var(--color-ink)' }}>{pr.requestor}</div>
                      <div className="text-sm" style={{ color: 'var(--color-mercury-grey)' }}>{pr.department}</div>
                      <div className="text-sm" style={{ color: 'var(--color-mercury-grey)' }}>{pr.needByDate}</div>
                      <div className="text-sm text-right" style={{ color: 'var(--color-ink)', fontWeight: '600' }}>{formatCurrency(pr.totalAmount)}</div>
                      <div className="text-center">
                        <span className="px-3 py-1 rounded-full text-xs inline-block" style={{ backgroundColor: statusStyle.bg, color: statusStyle.color, fontWeight: '500' }}>
                          {pr.status}
                        </span>
                      </div>
                      <div className="text-sm" style={{ color: 'var(--color-mercury-grey)' }}>{pr.nextApprover}</div>
                      <div className="text-center">
                      ...
                      <div className="flex items-center justify-end gap-2">
                          <PremiumActionButton label="View PR" icon={<Eye className="w-4 h-4" />} tone="teal" onClick={() => navigate(`/procurement/pr/detail/${pr.id}`)} />
                          {pr.status === 'Draft' && (
                            <PremiumActionButton label="Edit PR" icon={<Edit className="w-4 h-4" />} tone="violet" />
                          )}
                          {pr.status === 'Pending Approval' && selectedTab === 'my-prs' && (
                            <button className="px-2 py-1 rounded text-xs" style={{ backgroundColor: 'var(--color-error-light)', color: 'var(--color-error-dark)' }}>
                              Withdraw
                            </button>
                          )}
                          ...
                          <PremiumActionButton label="Duplicate PR" icon={<Copy className="w-4 h-4" />} tone="slate" />
                          <PremiumActionButton label="Open PR" icon={<ArrowUpRight className="w-4 h-4" />} tone="blue" onClick={() => navigate(`/procurement/pr/detail/${pr.id}`)} />
                      </div>
                    </div>
                  );
                })}
