    ...
    <div style={{ backgroundColor: '#F6F9FC', minHeight: '100vh' }}>
      ...
      <div className="bg-white px-8 py-6" style={{ borderBottom: '1px solid #E1E6EA' }}>
        <div className="flex items-center justify-between mb-4">
          <div>
            <h1 className="text-2xl mb-2" style={{ color: '#0A0F14', margin: 0 }}>Purchase Requisitions</h1>
          ...
          <div className="flex items-center gap-3">
            {filteredPRs.filter(pr => pr.status === 'Approved' && !pr.linkedPO).length > 0 && (
              <button 
                ...
                style={{ backgroundColor: '#FFFFFF', borderColor: '#00A9B7', color: '#00A9B7' }}
      ...
      <div className="p-8">
        ...
        <div className="bg-white rounded-lg mb-6" style={{ border: '1px solid #E1E6EA' }}>
          <div className="flex items-center gap-4 p-4" style={{ borderBottom: '1px solid #E1E6EA' }}>
            <button
              onClick={() => setSelectedTab('my-prs')}
              className="px-4 py-2 rounded-lg text-sm transition-colors"
              style={{
                backgroundColor: selectedTab === 'my-prs' ? '#E8F7F8' : 'transparent',
                color: selectedTab === 'my-prs' ? '#00A9B7' : '#6E7A82',
                border: selectedTab === 'my-prs' ? '1px solid #00A9B7' : '1px solid transparent',
            ...
            <button
              ...
              className="px-4 py-2 rounded-lg text-sm transition-colors"
              style={{
                backgroundColor: selectedTab === 'team-prs' ? '#E8F7F8' : 'transparent',
                color: selectedTab === 'team-prs' ? '#00A9B7' : '#6E7A82',
                border: selectedTab === 'team-prs' ? '1px solid #00A9B7' : '1px solid transparent',
        ...
        <div className="grid grid-cols-5 gap-6 mb-8">
          ...
          <div className="bg-white p-6 rounded-lg" style={{ border: '1px solid #E1E6EA' }}>
            <p className="text-sm mb-2" style={{ color: '#6E7A82' }}>Pending</p>
              ...
              <div className="grid gap-4 px-6 py-4" style={{ gridTemplateColumns: '1.6fr 1fr 1.2fr 1.1fr 1fr 1fr 1fr 1.2fr 1fr 1fr 1fr', borderBottom: '1px solid #E8F0F4' }}>
                <div className="space-y-2">
                  <div className="relative w-full">
                    ...
                    <input value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} placeholder="Search PR..." className="w-full pl-11 pr-4 py-2.5 rounded-2xl text-sm" style={{ backgroundColor: '#F8FBFD', border: '1px solid #D7E3EA', color: '#0A0F14' }} />
                  </div>
                  {hasActiveFilters && (
                    ...
                    <div key={pr.id} className="grid gap-4 px-6 py-4 hover:bg-[#F8FCFE]" style={{ gridTemplateColumns: '1.6fr 1fr 1.2fr 1.1fr 1fr 1fr 1fr 1.2fr 1fr 1fr 1fr', borderBottom: '1px solid #EDF3F7' }}>
                      <div>
                        <div>
                          ...
                          {pr.linkedPO && (
                            <p className="text-xs" style={{ color: '#00A9B7', margin: 0, cursor: 'pointer' }}>→ {pr.linkedPO}</p>
                          )}
                        </div>
                      </div>
                      <div>
                      ...
                      <div className="text-sm" style={{ color: '#0A0F14' }}>{pr.entity}</div>
                      <div className="text-sm" style={{ color: '#0A0F14' }}>{pr.requestor}</div>
                      <div className="text-sm" style={{ color: '#6E7A82' }}>{pr.department}</div>
                      <div className="text-sm" style={{ color: '#6E7A82' }}>{pr.needByDate}</div>
                      <div className="text-sm text-right" style={{ color: '#0A0F14', fontWeight: '600' }}>{formatCurrency(pr.totalAmount)}</div>
                      ...
                      <div className="text-sm" style={{ color: '#6E7A82' }}>{pr.nextApprover}</div>
                      <div className="text-center">
                      ...
                      <div className="flex items-center justify-end gap-2">
                          ...
                          {pr.status === 'Pending Approval' && selectedTab === 'my-prs' && (
                            <button className="px-2 py-1 rounded text-xs" style={{ backgroundColor: '#FEE2E2', color: '#DC2626' }}>
                              Withdraw
                            </button>
                          )}
