...
export function RegularPRForm() {
  ...
  const completeness = useMemo(() => {
    const fields = [selectedEntity, selectedDepartment, deliveryLocation, needByDate, businessJustification];
        ...
        <div className="grid grid-cols-3 gap-6">
          ...
          <div className="col-span-2 space-y-6">
            ...
            <FormSection title="PR Header Information" columns={2}>
              ...
              <PxFormField label="Need-by Date" required>
                <input type="date" value={needByDate} onChange={(e) => setNeedByDate(e.target.value)} className="px-input" />
              </PxFormField>
              <PxFormField label="Business Justification" required colSpan={2}>
                <textarea value={businessJustification} onChange={(e) => setBusinessJustification(e.target.value)} className="px-input" rows={2} placeholder="Explain why these items are needed..." style={{ height: 'auto' }}></textarea>
              </PxFormField>
            </FormSection>
            ...
            <div className="bg-white p-6 rounded-lg" style={{ border: '1px solid var(--color-silver)' }}>
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-base" style={{ color: 'var(--color-ink)', fontWeight: '600' }}>Line Items</h3>
                <button 
              ...
              {lineItems.length === 0 ? (
                <div className="text-center py-12" style={{ backgroundColor: 'var(--color-cloud)', borderRadius: '8px' }}>
                  <FileText className="w-12 h-12 mx-auto mb-3" style={{ color: 'var(--color-mercury-grey)' }} />
                  <p className="text-sm mb-1" style={{ color: 'var(--color-ink)' }}>No items added yet</p>
                  <p className="text-sm" style={{ color: 'var(--color-mercury-grey)' }}>Click "Add Line Item" to start building your PR</p>
                </div>
              ) : (
                <div className="overflow-x-auto">
                  <table className="w-full">
                    <thead style={{ backgroundColor: 'var(--color-cloud)' }}>
                      <tr>
                        <th className="px-3 py-3 text-left text-xs" style={{ color: 'var(--color-mercury-grey)', fontWeight: '600' }}>Item Details</th>
                        <th className="px-3 py-3 text-left text-xs" style={{ color: 'var(--color-mercury-grey)', fontWeight: '600' }}>Vendor</th>
                        <th className="px-3 py-3 text-center text-xs" style={{ color: 'var(--color-mercury-grey)', fontWeight: '600' }}>Qty</th>
                        <th className="px-3 py-3 text-right text-xs" style={{ color: 'var(--color-mercury-grey)', fontWeight: '600' }}>Unit Price</th>
                        <th className="px-3 py-3 text-center text-xs" style={{ color: 'var(--color-mercury-grey)', fontWeight: '600' }}>Price Variance</th>
                        <th className="px-3 py-3 text-right text-xs" style={{ color: 'var(--color-mercury-grey)', fontWeight: '600' }}>Total</th>
                        <th className="px-3 py-3 text-center text-xs" style={{ color: 'var(--color-mercury-grey)', fontWeight: '600' }}>Action</th>
                      </tr>
                    </thead>
                        ...
                        return (
                          <tr key={item.id}>
                            <td className="px-3 py-4">
                              ...
                              {item.itemName && (
                                <div>
                                  <p className="text-xs" style={{ color: 'var(--color-ink)', fontWeight: '600' }}>{item.itemName}</p>
                                  <p className="text-xs" style={{ color: 'var(--color-mercury-grey)' }}>{item.description}</p>
                                  <p className="text-xs mt-1" style={{ color: 'var(--color-mercury-grey)' }}>HSN: {item.hsnCode} | GST: {item.gstRate}%</p>
                                </div>
                              )}
                            </td>
                            <td className="px-3 py-4">
                            ...
                            <td className="px-3 py-4">
                              ...
                              />
                              <p className="text-xs text-center mt-1" style={{ color: 'var(--color-mercury-grey)' }}>{item.uom}</p>
                            </td>
                            <td className="px-3 py-4">
                              <input
                                type="number"
                              ...
                              {item.lastPurchasePrice && item.lastPurchasePrice > 0 && (
                                <p className="text-xs text-right mt-1" style={{ color: 'var(--color-mercury-grey)' }}>
                              ...
                              {item.priceVariance !== undefined && item.priceVariance !== 0 ? (
                                <div className="flex items-center justify-center gap-1">
                                  {item.priceVariance > 0 ? (
                                    <TrendingUp className="w-4 h-4" style={{ color: 'var(--color-error-dark)' }} />
                                  ) : (
                                    <TrendingDown className="w-4 h-4" style={{ color: 'var(--color-success-dark)' }} />
                                  )}
                                  <span 
                                    className="text-xs"
                                    style={{ color: item.priceVariance > 0 ? 'var(--color-error-dark)' : 'var(--color-success-dark)', fontWeight: '600' }}
                                  >
                            ...
                            <td className="px-3 py-4 text-right">
                              <p className="text-sm" style={{ color: 'var(--color-ink)', fontWeight: '600' }}>{formatCurrency(totals.total)}</p>
                              <p className="text-xs" style={{ color: 'var(--color-mercury-grey)' }}>incl. GST</p>
                            </td>
                            <td className="px-3 py-4 text-center">
                              <button 
              ...
              <div className="bg-white p-6 rounded-lg" style={{ border: '2px solid var(--color-teal)' }}>
                ...
                <div className="grid grid-cols-4 gap-4">
                  <div className="text-center p-4 rounded-lg" style={{ backgroundColor: 'var(--color-cloud)' }}>
                    <p className="text-sm mb-2" style={{ color: 'var(--color-mercury-grey)' }}>Total Items</p>
                    <p className="text-2xl" style={{ color: 'var(--color-ink)', fontWeight: '600' }}>{lineItems.length}</p>
                  </div>
                  <div className="text-center p-4 rounded-lg" style={{ backgroundColor: 'var(--color-cloud)' }}>
                    <p className="text-sm mb-2" style={{ color: 'var(--color-mercury-grey)' }}>Unique Vendors</p>
                    <p className="text-2xl" style={{ color: 'var(--color-ink)', fontWeight: '600' }}>{uniqueVendors}</p>
                  </div>
                  <div className="text-center p-4 rounded-lg" style={{ backgroundColor: 'var(--color-cloud)' }}>
                    <p className="text-sm mb-2" style={{ color: 'var(--color-mercury-grey)' }}>Subtotal</p>
                    <p className="text-xl" style={{ color: 'var(--color-ink)', fontWeight: '600' }}>{formatCurrency(grandTotal.subtotal)}</p>
                  </div>
                  <div className="text-center p-4 rounded-lg" style={{ backgroundColor: 'var(--color-teal-tint)' }}>
                    <p className="text-sm mb-2" style={{ color: 'var(--color-teal)' }}>Grand Total</p>
          ...
          <div className="space-y-6">
            ...
            {lineItems.some(item => item.priceVariance && Math.abs(item.priceVariance) > 10) && (
              <div className="bg-white p-6 rounded-lg" style={{ border: '2px solid var(--color-warning-dark)' }}>
                <div className="flex items-center gap-2 mb-4">
                  <AlertTriangle className="w-5 h-5" style={{ color: 'var(--color-warning-dark)' }} />
                  <h3 className="text-base" style={{ color: 'var(--color-warning-dark)', fontWeight: '600' }}>Price Variance Alert</h3>
                </div>
                <div className="space-y-2">
                  {lineItems
                    .filter(item => item.priceVariance && Math.abs(item.priceVariance) > 10)
                    .map(item => (
                      <div key={item.id} className="p-3 rounded-lg" style={{ backgroundColor: 'var(--color-warning-light)' }}>
                        <p className="text-sm mb-1" style={{ color: 'var(--color-ink)', fontWeight: '600' }}>{item.itemName}</p>
            ...
            <div className="bg-white p-6 rounded-lg" style={{ border: '1px solid var(--color-silver)' }}>
              <div className="flex items-center gap-2 mb-4">
                ...
                <h3 className="text-base" style={{ color: 'var(--color-ink)', fontWeight: '600' }}>Procurement Guidelines</h3>
              </div>
              <ul className="space-y-3">
                <li className="flex items-start gap-2">
                  <CheckCircle className="w-4 h-4 mt-0.5 flex-shrink-0" style={{ color: 'var(--color-success-dark)' }} />
                  <span className="text-sm" style={{ color: 'var(--color-mercury-grey)' }}>Get <strong>3 quotes</strong> for items over ₹50,000</span>
                </li>
                <li className="flex items-start gap-2">
                  <CheckCircle className="w-4 h-4 mt-0.5 flex-shrink-0" style={{ color: 'var(--color-success-dark)' }} />
                  <span className="text-sm" style={{ color: 'var(--color-mercury-grey)' }}>Price variance <strong>&gt;10%</strong> needs CFO approval</span>
                </li>
                <li className="flex items-start gap-2">
                  <CheckCircle className="w-4 h-4 mt-0.5 flex-shrink-0" style={{ color: 'var(--color-success-dark)' }} />
                  <span className="text-sm" style={{ color: 'var(--color-mercury-grey)' }}>Vendor must be <strong>approved</strong> in master</span>
                </li>
                <li className="flex items-start gap-2">
                  <CheckCircle className="w-4 h-4 mt-0.5 flex-shrink-0" style={{ color: 'var(--color-success-dark)' }} />
                  <span className="text-sm" style={{ color: 'var(--color-mercury-grey)' }}>Items must be <strong>budgeted</strong> for current FY</span>
                </li>
              </ul>
            </div>
            ...
            {uniqueVendors > 1 && (
              <div className="bg-white p-6 rounded-lg" style={{ border: '1px solid var(--color-silver)' }}>
                <h3 className="text-base mb-4" style={{ color: 'var(--color-ink)', fontWeight: '600' }}>Multi-Vendor Impact</h3>
                <div className="space-y-3">
                  <div className="p-3 rounded-lg" style={{ backgroundColor: 'var(--color-warning-light)' }}>
                    <p className="text-sm" style={{ color: 'var(--color-warning-dark)', fontWeight: '600' }}>⚠️ Multiple POs Required</p>
                    <p className="text-xs mt-1" style={{ color: 'var(--color-mercury-grey)' }}>
              ...
              <div className="bg-white p-6 rounded-lg" style={{ border: '1px solid var(--color-silver)' }}>
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    <span className="text-sm" style={{ color: 'var(--color-mercury-grey)' }}>Department Budget</span>
                    <span className="text-sm" style={{ color: 'var(--color-ink)', fontWeight: '600' }}>₹15,00,000</span>
                  </div>
                  <div className="flex items-center justify-between">
