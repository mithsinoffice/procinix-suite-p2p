...
describe('getInvoiceEntries', () => {
  it('returns credit entries for processed invoices', async () => {
    const db = makeDb({
      invoices: [
        { id: 'i1', invoice_number: 'INV-001', invoice_date: '2026-01-15', total_amount: 10000, lifecycle_state: 'Processed' },
        { id: 'i2', invoice_number: 'INV-002', invoice_date: '2026-01-20', total_amount: 5000, lifecycle_state: 'Processed' },
      ],
    });
...
describe('getPaymentEntries', () => {
  it('returns debit entries for confirmed payments', async () => {
    const db = makeDb({
      payments: [
        { id: 'p1', payment_date: '2026-01-25', amount: 8000, utr: 'UTR123', status: 'confirmed', invoice_id: 'i1', invoice_number: 'INV-001' },
      ],
    });
  ...
  it('uses fallback doc_ref when UTR is null', async () => {
    const db = makeDb({
      payments: [
        { id: 'abcd1234-5678', payment_date: '2026-01-25', amount: 3000, utr: null, status: 'confirmed', invoice_id: 'i1', invoice_number: 'INV-001' },
      ],
    });
...
describe('getVendorLedger — combined entries sorted by date', () => {
  it('merges and sorts invoice + payment entries chronologically', async () => {
    const db = makeDb({
      invoices: [
        { id: 'i1', invoice_number: 'INV-001', invoice_date: '2026-01-15', total_amount: 10000, lifecycle_state: 'Processed' },
        { id: 'i2', invoice_number: 'INV-002', invoice_date: '2026-01-25', total_amount: 5000, lifecycle_state: 'Processed' },
      ],
      payments: [
        { id: 'p1', payment_date: '2026-01-20', amount: 8000, utr: 'UTR1', status: 'confirmed', invoice_id: 'i1', invoice_number: 'INV-001' },
      ],
    });

    const ledger = await getVendorLedger({ vendorId: 'v1', entityId: 'e1', fromDate: '2026-01-01', toDate: '2026-01-31', db });
  ...
  it('invoices sort before payments on same date', async () => {
    const db = makeDb({
      ...
      payments: [
        { id: 'p1', payment_date: '2026-01-15', amount: 10000, utr: 'UTR1', status: 'confirmed', invoice_id: 'i1', invoice_number: 'INV-001' },
      ],
    });
...
describe('getVendorLedger — closing balance', () => {
  it('computes closing = opening + credits - debits', async () => {
    const db = makeDb({
      openingBalance: 5000,
      invoices: [
        { id: 'i1', invoice_number: 'INV-001', invoice_date: '2026-01-10', total_amount: 20000, lifecycle_state: 'Processed' },
        { id: 'i2', invoice_number: 'INV-002', invoice_date: '2026-01-20', total_amount: 15000, lifecycle_state: 'Processed' },
      ],
      payments: [
        { id: 'p1', payment_date: '2026-01-15', amount: 12000, utr: 'UTR1', status: 'confirmed', invoice_id: 'i1', invoice_number: 'INV-001' },
      ],
    });
  ...
  it('closing can go negative (overpayment)', async () => {
    const db = makeDb({
      openingBalance: 0,
      payments: [
        { id: 'p1', payment_date: '2026-01-15', amount: 5000, utr: 'UTR1', status: 'confirmed', invoice_id: 'i1', invoice_number: 'INV-001' },
      ],
    });
...
describe('getVendorLedger — single source', () => {
  ...
  it('works with payment entries only', async () => {
    const db = makeDb({
      payments: [
        { id: 'p1', payment_date: '2026-01-20', amount: 3000, utr: 'UTR1', status: 'confirmed', invoice_id: 'i1', invoice_number: 'INV-001' },
      ],
    });
...
describe('getVendorLedger — entity scoping', () => {
  it('throws when entityId is not provided', async () => {
    const db = makeDb();
    await expect(
      getVendorLedger({ vendorId: 'v1', entityId: null, fromDate: '2026-01-01', toDate: '2026-01-31', db })
  ...
  it('throws when entityId is empty string', async () => {
    const db = makeDb();
    await expect(
      getVendorLedger({ vendorId: 'v1', entityId: '', fromDate: '2026-01-01', toDate: '2026-01-31', db })
    ).rejects.toThrow('entityId is required');
  });
});
